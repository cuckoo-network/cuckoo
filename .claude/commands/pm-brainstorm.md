...
6. **Emit the proposal as text only.** Open with a **numbered summary list** of every candidate (milestones and inbox notes together, in proposed priority order) — one line each: `N. <title> (wN, ~size) — one-line outcome`, so the user can pick by number before reading further. Then give the full detail: the target workstream, each proposed milestone (task table + definition of done + source + goal linkage + expected outcome + why-now rationale) and/or inbox note, in the same numbered order as the summary so numbers stay addressable in the rest of the conversation. Propose **implementation tasks only**: `/pm` appends the standing closing tasks (Render parity when the milestone is feature dev/a fix touching REST/GraphQL/MCP/UI, then Simplify, then Test coverage, then Closeout) itself when it materializes, so do not include them — but do flag in the proposal whether you expect Render parity to apply, so `/pm` and the user aren't guessing. Do **not** write files.
