# Welcome to Cuckoo Brand Assets

Learn how to use brand assets and showcase content from Cuckoo technologies and programs.

<div className="row">
  <div className="col">
    <h2>Brand name</h2>
  </div>
  <div className="col">
    <p>Cuckoo</p>
  </div>
</div>

<br />

<div className="row">
  <div className="col">
    <h2>Font family</h2>
  </div>
  <div className="col">
    <a href="https://fonts.google.com/specimen/Inter" target="_blank">
      -apple-system, Inter, "system-ui", "Segoe UI", Roboto, "Helvetica Neue", Ubuntu, sans-serif, "Microsoft YaHei"
    </a>
  </div>
</div>

<br />

<div className="row">
  <div className="col">
    <h2>Primary color</h2>
  </div>
  <div className="col"  >
    <div style={{ width: '20px', height: '20px', backgroundColor: '#3b82f6', marginRight: '10px' }}></div>
    #3b82f6
  </div>
</div>


<br />

<div className="row">
  <div className="col"><h2>Logo</h2></div>
  <div className="col">
    <div>Icon only PNG 👇</div>
    <img style={{width: "48px"}} src="/img/favicon.png"/>
    <div>Icon only PNG 👇</div>
    <img style={{width: "48px"}} src="/img/favicon-black.png"/>
        <div>Icon only SVG 👇</div>
        <img style={{width: "48px"}} src="/img/favicon.svg"/>

    <div>Icon + Text PNG 👇</div>
    <img src="https://cuckoo-network.b-cdn.net/white-full-logo.png"/>
    <div>Icon + Text SVG 👇</div>
    <img src="https://cuckoo-network.b-cdn.net/white-full-logo.svg"/>
    <div>Icon + Text PNG 👇 white text</div>
    <img style={{backgroundColor: "white", padding: "8px"}}
         src="https://cuckoo-network.b-cdn.net/black-full-logo.png"/>
    <div>Icon + Text SVG 👇 white text</div>
    <img style={{backgroundColor: "white", padding: "8px"}}
         src="https://cuckoo-network.b-cdn.net/black-full-logo.svg"/>

    <div>Preview 👇</div>
    <img
         src="https://cuckoo-network.b-cdn.net/cuckoo-social-card.png"/>
  </div>
</div>

<br />

<div className="row">
  <div className="col">
    <h2>Download</h2>
  </div>
  <div className="col">
    <p>
      <a href="https://cuckoo-network.b-cdn.net/cuckoo-brand-assets.zip">
        Click to download all
      </a>
    </p>
  </div>
</div>

<br />

<div className="row">
  <div className="col">
    <h2>Any question?</h2>
  </div>
  <div className="col">
    <p>
      Hi, there! 👋 We are open to collaborations and partnerships to bring AI + Web3
      to everyone on this planet. Please feel free to{" "}
      <a href="https://cuckoo.network/tg">contact us via Discord.</a>
    </p>
  </div>
</div>
