...
- ✅ opensandbox-controller chart **vendored** (`charts/opensandbox-controller`, 0.2.0); renders with a digest-pinned Bex image carrying snapshot-namespace and terminal-Pod projection patches.
- ✅ kpack `v0.18.0` release manifest **vendored with its SHA-256 recorded**; `charts/kpack` adds the Paketo store/stack/cluster-builder and the local Zot registry alias.
