...
#   BEX_GITHUB_APP_CLIENT_ID    OAuth client id (required to prove the callback
#                               user administers the installation)
#   BEX_GITHUB_APP_CLIENT_SECRET OAuth client secret (out-of-band)
#   BEX_GITHUB_WEBHOOK_SECRET   the app's webhook HMAC secret (optional but needed
#                               for zero-config push-to-deploy)
#
# The Secret's key names are what lego/operator/config/api/deployment.yaml
...
require BEX_GITHUB_APP_CLIENT_ID 1
require BEX_GITHUB_APP_CLIENT_SECRET 1
...
if [ "${DRY_RUN:-}" = "1" ]; then
  ...
  echo "would apply secret $NS/bex-github-app (keys: app-id private-key slug client-id client-secret webhook-secret)"
  exit 0
fi
