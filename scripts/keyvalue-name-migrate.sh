#!/usr/bin/env bash
# Backfill KeyValue.spec.name without changing metadata.name. The latter is the
...
# The KeyValue sibling of scripts/postgres-name-migrate.sh (w9/m6, mirroring
# w9/m3) — same idempotent, non-destructive contract.
set -euo pipefail
...
usage() {
  cat <<'EOF'
Usage: scripts/keyvalue-name-migrate.sh [--apply] [--namespace NS | --all-namespaces]
...
if [ "$ALL_NAMESPACES" -eq 1 ]; then
  objects="$(kubectl get keyvalues.app.bex.co -A -o json)"
else
  objects="$(kubectl -n "$NAMESPACE" get keyvalues.app.bex.co -o json)"
fi
...
if [ -n "$duplicates" ]; then
  echo "error: duplicate key-value display names exist (namespace, tenant, name, ids):" >&2
  echo "$duplicates" >&2
  exit 1
fi
...
if [ -z "$pending" ]; then
  echo "key-value display-name backfill: already complete in $scope_label"
  exit 0
fi

count="$(wc -l <<<"$pending" | tr -d ' ')"
echo "key-value display-name backfill: $count object(s) pending in $scope_label"
while IFS=$'\t' read -r namespace id; do
  ...
  kubectl -n "$namespace" patch keyvalues.app.bex.co "$id" --type=merge -p "$body" >/dev/null
...
if [ "$APPLY" -eq 0 ]; then
  ...
  echo "key-value display-name backfill complete in $scope_label"
fi
