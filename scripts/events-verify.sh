...
# TRUTHFUL 1:1 record of what happened to a service, composed from rows bex
# already writes, and it can never carry a secret:
#
...
#      as the actor.
#   2. ORDERING: the whole feed is newest-first across BOTH sources (deploys rows
#      and audit rows interleaved), and the lifecycle actions appear in the
...
# Requires: kubectl, curl, jq, go; the mock cluster with auth (Hydra),
...
DB_NS="$(kubectl get cluster.postgresql.cnpg.io -A -o jsonpath='{range .items[?(@.metadata.name=="bex-db")]}{.metadata.namespace}{end}')"
[ -n "$DB_NS" ] || fail "no bex-db CNPG cluster found — the control-plane store is what this feed reads"
...
# --- run the operator + bex-api (store on: the feed's two sources live there) --
...
start_api "BEX_CP_DB_URI=$DB_URI" "BEX_CP_ADDR=:18092" "BEX_CP_TOKEN=$CP_TOKEN" "BEX_CP_RESYNC=5s" \
  "BEX_OPENBAO_URL=http://$BAO" "BEX_OPENBAO_JWT_PATH=$jwt_file"
...
echo "==> performing the sequence: suspend → resume → scale → env-var write → deploy"
# Render's own codes: suspend/resume/scale are accepted (202), not completed (200).
...
request POST "/v1/services/$SVC/deploys" ''            ; expect 201 "trigger deploy"
...
for t in deploy_started deploy_ended suspender_added suspender_removed instance_count_changed env_vars_changed; do
  n="$(types | grep -cx "$t" || true)"
  case "$t" in
    deploy_started) want=2 ;; # the create's first deploy + the explicit trigger
    *) want=1 ;;
  esac
  [ "$n" = "$want" ] || fail "$t appears $n time(s), want exactly $want — the feed must be 1:1 with what happened"
done
pass "correspondence: every action appears exactly once (deploy_started twice: create + trigger)"
...
# Newest-first across BOTH sources, and the lifecycle verbs in reverse order.
...
pass "ordering: newest-first across deploys + audit rows, lifecycle in reverse order"
...
gqlq="{\"query\":\"{ serviceEvents(serviceId: \\\"$SVC\\\", startTime: \\\"$SINCE\\\", limit: 100) { id type timestamp details { deployId deployStatus actor triggeredByUser } } }\"}"
...
pass "redaction: the planted value appears in NO response (REST + GraphQL), and the env-var write is still visible as env_vars_changed"
...
echo
pass "service events verified: 1:1 with what happened, newest-first, cursor-stable, and value-free"
