...
actual_version="$(cd lego/cli && go list -m -f '{{.Version}}' "$UPSTREAM_MODULE")"
[ "$actual_version" = "$EXPECTED_VERSION" ] \
  || fail "$UPSTREAM_MODULE version is $actual_version, want $EXPECTED_VERSION; update $PIN_RECORD and review compatibility"

grep -Fq '"github.com/render-oss/cli/cmd"' lego/cli/main.go \
  || fail "lego/cli/main.go no longer imports upstream cmd package"
if find lego/cli -type f -path '*/cmd/*' -print -quit | grep -q .; then
  fail "lego/cli contains a copied cmd tree; keep command implementation upstream"
fi
