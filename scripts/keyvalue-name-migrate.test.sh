#!/usr/bin/env bash
# Pure-bash regression tests for keyvalue-name-migrate.sh. A shell-function
...
# no-sensitive-output behavior. The KeyValue sibling of
# postgres-name-migrate.test.sh.
set -euo pipefail
cd "$(dirname "$0")/.."

tmp="$(mktemp -d)"
trap 'rm -rf "$tmp"' EXIT
export MOCK_KEYVALUE_STATE="$tmp/keyvalues.json"
export MOCK_PATCH_LOG="$tmp/patches.tsv"
: >"$MOCK_PATCH_LOG"

kubectl() {
  local args=("$@") id="" payload="" i
  if [[ " $* " == *" get keyvalues.app.bex.co "* ]]; then
    command cat "$MOCK_KEYVALUE_STATE"
    return
  fi
  if [[ " $* " == *" patch keyvalues.app.bex.co "* ]]; then
    for ((i = 0; i < ${#args[@]}; i++)); do
      if [ "${args[$i]}" = "keyvalues.app.bex.co" ]; then id="${args[$((i + 1))]}"; fi
    ...
    ' "$MOCK_KEYVALUE_STATE" >"$MOCK_KEYVALUE_STATE.next"
    command mv "$MOCK_KEYVALUE_STATE.next" "$MOCK_KEYVALUE_STATE"
...
write_valid_state() {
  jq -nc '{apiVersion:"app.bex.co/v1alpha1",kind:"KeyValueList",items:[
    {metadata:{namespace:"tenant",name:"legacy-a",labels:{"bex.co/tenant":"tea-a"}},spec:{plan:"free",testSensitiveValue:"DO-NOT-PRINT"}},
    {metadata:{namespace:"tenant",name:"red-other-workspace",labels:{"bex.co/tenant":"tea-b"}},spec:{name:"legacy-a",plan:"free"}},
    {metadata:{namespace:"tenant",name:"red-current",labels:{"bex.co/tenant":"tea-a"}},spec:{name:"current",plan:"free"}}
  ]}' >"$MOCK_KEYVALUE_STATE"
  : >"$MOCK_PATCH_LOG"
}
...
write_valid_state
out="$(bash scripts/keyvalue-name-migrate.sh --namespace tenant)"
...
out="$(bash scripts/keyvalue-name-migrate.sh --apply --namespace tenant)"
...
assert "state now carries the explicit display name" jq -e '.items[] | select(.metadata.name=="legacy-a") | .spec.name=="legacy-a"' "$MOCK_KEYVALUE_STATE" >/dev/null
out="$(bash scripts/keyvalue-name-migrate.sh --apply --namespace tenant)"
...
: >"$MOCK_PATCH_LOG"
if bash scripts/keyvalue-name-migrate.sh --apply --namespace tenant >"$tmp/invalid.out" 2>&1; then
...
jq -nc '{apiVersion:"app.bex.co/v1alpha1",kind:"KeyValueList",items:[
  {metadata:{namespace:"tenant",name:"red-one",labels:{"bex.co/tenant":"tea-a"}},spec:{name:"duplicate"}},
  {metadata:{namespace:"tenant",name:"red-two",labels:{"bex.co/tenant":"tea-a"}},spec:{name:"duplicate"}}
]}' >"$MOCK_KEYVALUE_STATE"
: >"$MOCK_PATCH_LOG"
if bash scripts/keyvalue-name-migrate.sh --apply --namespace tenant >"$tmp/duplicate.out" 2>&1; then
...
if [ "$fails" -eq 0 ]; then
  echo "PASS: key-value name migration dry-run/apply/idempotence/preflight"
  exit 0
fi
