...
- **Top-level LIST/CREATE routes use a content-shaped `pendingComponent`** — `ListPageSkeleton` (agents, blueprints, env-groups, webhooks, notifications, usage) / `FormPageSkeleton` (`*.new`), both in `common/components/detail-skeletons.tsx` — instead of falling back to the bare `RoutePending` spinner (w9/m69). They render the page's own header+grid/form shape inside the persistent shell's content region, and only mount when a nav is slow enough to pass `defaultPendingMs` (a prefetched/cached nav — w9/m68 — skips them). Redirect-only routes (`static.index`, `static.new`) have no `component`/pending state, so they get none.
- The detail-route `pendingMs: 0` + component-as-pending config was **audited against the persistent shell (w9/m69) and retained**: rendering the detail frame's skeleton at 0ms is still the right behavior (the shell persists, but the frame's header/tabs skeleton should appear immediately, not after `defaultPendingMs`). Don't remove it, and don't cargo-cult `pendingMs: 0` onto a list route — a list route wants its `defaultPendingMs`-gated `ListPageSkeleton`, not an instant one.
