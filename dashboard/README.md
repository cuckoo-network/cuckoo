...
- After sign-up → email verification, a new user lands on the **payment wall** (`src/routes/setup.payment.tsx`, `src/features/onboarding/`) — hosted bex requires a card on file before any resource use ([ADR075](../docs/ADR075-user-onboarding.md) D7). A pending workspace invitation goes to the shared `/invite` review page first; acceptance or an existing membership selects the invited workspace before billing readiness is evaluated (ADR024). The wall forwards straight through when the workspace needs no card (gate off, exempt, already bound), and `PaymentSetupGate` sends a still-unbound workspace back to it from any app route.
