...
describe("useApiKeys", () => {
  it("maps apiKeys to views, dropping nulls and keys without an id", () => {
    mockUseQuery.mockReturnValue({
      data: {
        apiKeys: [
          { __typename: "ApiKey", id: "key-1", name: "deploy-agent", createdAt: "2026-07-01T00:00:00Z" },
          null,
          { __typename: "ApiKey", id: null, name: "orphan", createdAt: null },
        ],
      },
      loading: false,
      error: undefined,
    ...
    expect(result.current.keys).toEqual([
      { id: "key-1", name: "deploy-agent", createdAt: "2026-07-01T00:00:00Z" },
    ]);
  });

  it("never requests or surfaces a secret field — the view type has none", () => {
    mockUseQuery.mockReturnValue({
      data: { apiKeys: [{ __typename: "ApiKey", id: "key-1", name: "x", createdAt: null }] },
      loading: false,
      error: undefined,
      refetch: vi.fn(),
    });
