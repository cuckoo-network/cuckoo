  ...
  return (
    <Card>
      <CardHeader className="flex flex-row items-center justify-between pb-2">
        <CardTitle className="text-base">{t("blueprints.validateTitle")}</CardTitle>
        <Button
          size="sm"
          variant="outline"
      ...
      {ran && (
        <CardContent>
          {loading ? null : result == null ? (
            <p className="text-sm text-muted-foreground">{t("blueprints.validateNoResult")}</p>
          ) : result.valid ? (
            <p className="text-sm font-medium text-green-600">{t("blueprints.validateValid")}</p>
          ) : (
