...
const settingsState: {
  ...
  settings: { deployStarted: true, deploySucceeded: true, deployFailed: true },
  loading: false,
  error: undefined,
};
...
const update = vi.fn();
vi.mock(
  "@/features/notifications/hooks/use-update-notification-settings",
  () => ({
...
describe("NotificationSettingsPanel", () => {
  it("reflects the current preferences in all three switches", () => {
    settingsState.settings = {
      deployStarted: true,
    ...
    expect(switches[0]).toHaveAttribute("aria-checked", "true"); // deployStarted
    expect(switches[1]).toHaveAttribute("aria-checked", "false"); // deploySucceeded
    expect(switches[2]).toHaveAttribute("aria-checked", "true"); // deployFailed
  });
  ...
  it("toggling a switch calls update with the flipped field and the other unchanged", async () => {
    ...
    const switches = screen.getAllByRole("switch");
    await user.click(switches[0]); // flip deployStarted true -> false
  ...
  it("toggling the other switch flips deployFailed and leaves deploySucceeded unchanged", async () => {
    ...
    await user.click(screen.getAllByRole("switch")[2]); // flip deployFailed true -> false
