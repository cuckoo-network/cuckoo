...
| t004 | Special-type samples on prod: static-site, cron-demo, stack-demo                             | 60m | t002             | ◐ cron-demo/stack-demo GREEN; static-site build ✅, publish bug root-caused + **fixed** (`012`), pending operator rollout to verify |
...
- **static-site** — two problems. (1) `--type static_site` went down the Docker build path and failed (`no Dockerfile`); bex's static build is Dockerfile-only (ADR029) but the sample had none and over-promised "no Dockerfile needed". Fixed + shipped: a minimal Dockerfile + honest comment (extract-simulation validated locally); real no-Dockerfile-publish fix filed `.pm/w9/010`. (2) Post-`/ship` the build succeeds but the deploy fails post-build — the **publish Job's** extract initContainer pulls the built image with the **apps-namespace** pull secret, unreachable from the separate `BEX_BUILD_NAMESPACE` (bex-system) it runs in (empty anyway under per-App creds). **Fixed** (`.pm/w9/012`): the publish Job now uses a build-namespace credential (`buildJobPullSecret` → `RegistryBuildPullSecret`, default `bex-registry-pull`), unit-tested (`TestBuildJobPullSecret`); operator + controller envtest suites green. Verifies on prod once the operator rolls onto the fix (needs Argo). static-site is therefore **not yet GREEN on prod** but the defect is fixed, not just filed.
