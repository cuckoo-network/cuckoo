...
**Worker:** worker7 **Goal:** bex's actual REST responses are validated against a pinned copy of Render's OpenAPI spec in CI, with an explicit divergence allowlist — so new parity drift fails a build instead of waiting for the next manual audit. **Status:** todo

## Tasks (in order)

| id   | title                                                     | est | depends_on |
| ---- | --------------------------------------------------------- | --- | ---------- |
| t001 | Pin Render's OpenAPI spec + choose a validator            | 40m | —          |
| t002 | Conformance harness over the CI Postgres/OpenFGA setup    | 60m | t001       |
| t003 | Per-family conformance cases (services/deploys/pg/kv/…)    | 60m | t002       |
| t004 | Divergence allowlist keyed to ADR018 rows                 | 40m | t003       |
| t005 | Wire into `backend-test.yml` + document the pin-update flow| 30m | t003       |
| t006 | Simplify                                                  | 30m | t004, t005 |
| t007 | Test coverage                                             | 30m | t004, t005 |
