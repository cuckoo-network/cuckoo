...
**Worker:** worker7 **Goal:** each tenant App pulls its images with its own registry credential, so a credential leaked from one tenant's pod cannot read any other tenant's images — closing ADR022's recorded shared-credential blast radius before real tenants exist. **Status:** todo

## Tasks (in order)

| id   | title                                                        | est | depends_on |
| ---- | ------------------------------------------------------------ | --- | ---------- |
| t001 | Design the per-App credential scheme (htpasswd + Zot ACLs)   | 45m | —          |
| t002 | Operator mints per-App pull credential + Secret              | 45m | t001       |
| t003 | Zot htpasswd/ACL generation + revocation on App delete       | 45m | t002       |
| t004 | Compose with w6/m29's imagePullSecret attach                 | 30m | t003       |
| t005 | Live cross-tenant pull-denial verification + ADR022 update   | 45m | t004       |
