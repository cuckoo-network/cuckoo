...
**Worker:** worker7 **Goal:** three filed sub-hour security chores land as one shippable chunk (the m10/m37 grouping precedent): Go call-graph vuln scanning joins the CI guard family, a config-toggle rollback can no longer reap live sandbox namespaces, and the static-site publish Job stops holding a wildcard registry credential **Status:** done — **DONE 2026-07-31**

## Tasks (in order)

| id   | title                                                                                     | est | depends_on       | status |
| ---- | ------------------------------------------------------------------------------------------ | --- | ---------------- | ------ |
| t001 | Pinned `govulncheck` CI job over the Go workspace + baseline triage + gate policy (`013`)   | 45m | —                | — **DONE** |
| t002 | Gate `<ws>-sandbox` prune on workspace existence, not the config toggle (`011`)             | 45m | —                | — **DONE** |
| t003 | Repo-scoped publish-Job extract credential: investigate, fix or record (ADR045 Finding 7)   | 45m | —                | — **DONE** |
| t004 | Simplify pass over the changed code                                                         | 20m | t001, t002, t003 | — **DONE** |
| t005 | Test coverage: prune regression, guard durability, credential-scope assertion               | 45m | t001, t002, t003 | — **DONE** |
| t006 | Closeout                                                                                    | 10m | t005             | — **DONE** |

## Outcomes

- **t001** — `.github/workflows/govulncheck.yml` runs pinned `govulncheck@v1.1.4` over all three modules on `lego/**` changes, under the `1.25` Go line the release image builds with (never a newer toolchain that would hide a stdlib vuln prod ships). Gate = fail on any reachable vuln. Baseline triaged clean: GO-2026-5970 fixed by bumping `x/text`→v0.39.0; GO-2026-5856 (crypto/tls) resolved by the `golang:1.25` line (≥1.25.12).
- **t002** — `ReconcileOnce` now always marks a live workspace's `<ws>-sandbox` desired (toggle = create-only), so flipping `BEX_TENANT_SANDBOX_NAMESPACES` off no longer reaps it; regression-tested. ADR045 Finding 6 fixed-in-place.
- **t003** — **investigate → record:** Finding 7 was already resolved by `7e05d80f` (w3/m35). The publish Job pulls with the repo-scoped `reg-pull-<name>` when `BEX_REGISTRY_NS` is set (prod is), mirrored into the build namespace; wildcard is the dev fallback. `TestBuildJobPullSecret` already pins both branches. ADR045 Finding 7 disposition updated with evidence.
