...
**Worker:** worker5 **Goal:** A user can see the workspace's registered blueprints (auto-registered on every repo-backed `deploy`), inspect each manifest, validate it (dry-run), and trigger an idempotent sync — all from the dashboard. **Status:** done

## Tasks (in order)

| id   | title                                                                                                                                 | est | depends_on |
| ---- | ------------------------------------------------------------------------------------------------------------------------------------- | --- | ---------- |
| t001 | Data layer: `features/blueprints/` hooks over `blueprints` (list) + `syncBlueprint` + `validateBlueprint`                            | 35m | —          | — **DONE** |
| t002 | Blueprints list page + route (`/blueprints`) + sidebar nav entry: rows (name/repo/branch/status), empty state explaining auto-register | 45m | t001       | — **DONE** |
| t003 | Blueprint detail: read-only manifest (`bex.yml`) view + "Sync" action with confirm + last-sync status                                 | 40m | t001       | — **DONE** |
| t004 | Validate flow: a "Validate" affordance surfacing per-entry errors from `validateBlueprint` (dry-run, no apply)                        | 35m | t002       | — **DONE** |
| t005 | Empty/loading/error states + `en`/`zh` locale strings                                                                                 | 30m | t002, t003 | — **DONE** |
| t006 | Live verification against the mock cluster: deploy a repo-backed `bex.yml` → appears in list → view manifest → sync → validate        | 30m | t004, t005 | — **DONE** |
| t007 | Render parity — full-surface check (REST/GraphQL/MCP already ship validate/list/sync; confirm the new UI matches)                     | 20m | t006       | — **DONE** |
