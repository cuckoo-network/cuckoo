...
**Worker:** worker7 **Goal:** give the three large surfaces shipped since the last audit window — the Stripe billing lifecycle (money + an unauthenticated public endpoint), the static-site serving/publish plane, and per-tenant namespaces — the same evidence-backed adversarial review the earlier platform got, and produce a fresh follow-up register that becomes w7's next work queue. **Status:** done

## Tasks (in order)

| id   | title                                                                                                | est | depends_on             |            |
| ---- | ---------------------------------------------------------------------------------------------------- | --- | ---------------------- | ---------- |
| t001 | Freeze the audit window + exclusion map                                                              | 45m | —                      | — **DONE** |
| t002 | Billing plane audit: webhook intake, enforcement/recovery, key custody, outbox-seal integrity        | 90m | t001                   | — **DONE** |
| t003 | Static serving/publish plane audit: S3 identity separation, publish Job, alias-authority residuals   | 60m | t001                   | — **DONE** |
| t004 | Tenant-namespace plane audit: NamespaceReconciler authority beyond m35 t013, quota/label/prune paths | 60m | t001                   | — **DONE** |
| t005 | Cross-cutting sweep: in-window secret custody/rotation, RBAC/NetworkPolicy/CI-guard drift            | 60m | t001                   | — **DONE** |
| t006 | Write the register: severities + evidence, one disposition per finding                               | 60m | t002, t003, t004, t005 | — **DONE** |
| t007 | Simplify                                                                                             | 30m | t006                   | — **DONE** |
| t008 | Test coverage: regression tests for every fixed-in-place finding                                     | 45m | t006                   | — **DONE** |
| t009 | Closeout                                                                                             | 15m | t006                   | — **DONE** |

## Outcome (2026-07-30)

Register shipped: [`docs/ADR045-security-review-round3.md`](../../../../docs/ADR045-security-review-round3.md). Four parallel adversarial subagent passes (billing · static · tenant-namespace · cross-cutting) over the 2026-07-20→HEAD window, verified against the code. **10 findings, no critical.** Headline: a **[High] cross-tenant host-hijack** unguarded on the create/blueprint/deploy-manifest paths (the reserved+collision gate lived only on `AddDomain`) — **fixed in place** at the single `writeNewApp` seam with a regression test. Also fixed in place: **[Medium]** billing seal-horizon/rollup-catch-up decoupling (`ClampSealHours` + `usage.CatchupWindow`, tested); **[Medium]** `.env.example` mirror gap (6 Stripe vars); **[Low]** CI guard for a ClusterRoleBinding to the secret-bearing tenant roles. Filed as follow-ups: quota storage/LB caps (→w3/m34), publish-Job pull-cred scope (→w1/m57), `w7/011` (sandbox-toggle prune), `w7/012` (outside-gate route completeness). Backend build + unit tests + lint + gitops-validate green.
