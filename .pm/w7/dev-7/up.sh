#!/usr/bin/env bash
# Bring up the dev-7 isolated environment (workstream w7): its own Kratos +
# Hydra + Mailpit (namespace dev-7-auth) on the shared local kind/CAPD cluster
...
# namespace (dev-7) are duplicated, so this never collides with the shared
...
echo "==> operator RBAC for the dev-7 apps namespace (secrets CRUD — see rbac-dev-7.yaml)"
kubectl apply -f "$ENVDIR/rbac-dev-7.yaml" >/dev/null
...
# scripts/auth-secrets.sh's dsn() helper, dev-7-auth namespace).
dsn() {
...
echo "==> out-of-band kratos/hydra secrets (freshly generated, dev-7-auth only)"
...
echo
echo "dev-7 (workstream w7) is up:"
