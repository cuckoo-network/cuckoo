...
```sh
bash .pm/w7/dev-7/up.sh      # idempotent: creates namespaces, CNPG Clusters,
                               # mailpit, kratos+hydra (helm), port-forwards,
                               # and a local bex-api
bash .pm/w7/dev-7/status.sh  # pod status + http health checks
bash .pm/w7/dev-7/down.sh    # kills local processes, deletes both namespaces
```
...
`up.sh` sets **`BEX_CP_IDENTITY=dev-7`**, so every tenant namespace this
