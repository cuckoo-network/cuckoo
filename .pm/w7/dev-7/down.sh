#!/usr/bin/env bash
# Tear down the dev-7 isolated environment (workstream w7): kills the local
# processes up.sh forked (port-forwards, bex-api) and deletes the
# dev-7-auth / dev-7 namespaces from the shared cluster. Leaves the shared
# cluster itself (and any other workstream's resources) untouched.
#
# Usage: bash .pm/w7/dev-7/down.sh
set -euo pipefail
...
echo "dev-7 down. Namespace deletion continues in the background (kubectl get ns to watch)."
