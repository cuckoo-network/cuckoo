# dev-10 — workstream w10's isolated local dev environment

A dedicated Kratos + Hydra + Mailpit stack (namespace `dev-10-auth`) plus a
dedicated app namespace (`dev-10`), layered onto the **shared** local
...
`dev-10` sidesteps this by giving workstream w10 its own Kratos/Hydra with
its own ports, instead of fighting over the shared one or another
workstream's. `dev-1` .. `dev-10` (`.pm/w1/dev-1` .. `.pm/w10/dev-10`) are
ten identical siblings — one per workstream — differing only in namespace
...
| --- | --- |
| dashboard | `50100` |
| kratos-public (port-forward) | `51100` |
| hydra-admin (port-forward) | `52100` |
| mailpit UI (port-forward) | `53100` |
| bex-api (local process) | `54100` |
| bex-db (port-forward) | `55100` |
| bex-api control-plane API (BEX_CP_ADDR, host-only) | `56100` |
| kratos-admin (port-forward, BEX_KRATOS_ADMIN_URL) | `57100` |
| hydra-public / issuer (port-forward, BEX_OAUTH_ISSUER) | `58100` |
| loki (port-forward, BEX_LOKI_URL) | `59100` |
| mailpit SMTP (port-forward, BEX_SMTP_ADDR) | `60100` |
| OpenSandbox lifecycle server (`agent-up`) | `61100` |
| ssh-gateway agent attach SSE (`agent-up`) | `62100` |
| OpenFGA (`agent-up`) | `63100` |
| OpenBao (`agent-up`) | `64100` |
| ssh-gateway sandbox-exec (`agent-up`) | `65100` |

## Start / status / stop

```sh
bash scripts/dev-env.sh 10 up      # idempotent: creates namespaces, CNPG Clusters,
                               # mailpit, kratos+hydra (helm), port-forwards,
                               # and a local bex-api
bash scripts/dev-env.sh 10 status  # pod status + http health checks
bash scripts/dev-env.sh 10 down    # kills local processes, deletes both namespaces
```

Then start the dashboard separately (kept out of `dev-env.sh 10 up` so it stays in the
foreground / under your own process control — `dev-env.sh 10 up` prints the exact
command):

```sh
cd dashboard && VITE_API_URL=http://localhost:54100/graphql \
  VITE_SSR_API_URL=http://localhost:54100/graphql \
  VITE_KRATOS_PUBLIC_URL=http://localhost:51100 yarn dev --port 50100
```

## Recovering after a restart

`dev-env.sh 10 up` is idempotent — re-running it after a reboot (or after the shared
...
KeyValue/App/Database CRs in the `dev-10` namespace survive as long as the
underlying `bex` cluster (and its PVs) survive.

## Tenant-namespace isolation between concurrent dev-N stacks

`dev-env.sh 10 up` sets **`BEX_CP_IDENTITY=dev-10`**, so every tenant namespace this
control plane provisions is stamped `app.bex.co/control-plane: dev-10` and its
...
`dev-env.sh 10 status` prints every managed tenant namespace with its owner — if a row
shows an owner other than `dev-10` it belongs to another harness and this one
will leave it alone; if *this* harness's rows show anything but `dev-10`, its
...
none of that machinery. `bash scripts/dev-env.sh 10 agent-up` layers it on:
...
| **OpenFGA** (`dev-10-auth`) | `cmd/ssh-gateway` calls `requiredEnv("BEX_OPENFGA_URL")` and has no allow-all hatch; bex-api must share the store or every attach is denied |
| **OpenBao** (`dev-10-auth`, dev mode) | the BYO model key (ADR062) the gateway mints per exchange |
...
stamped `app.bex.co/dev-env: dev-10` and refuses to take it from another
...
bash scripts/dev-env.sh 10 agent-netpol <workspaceId>
...
`bash scripts/dev-env.sh 10 agent-stub` stands up a local Anthropic Messages API
...
The dashboard's `/agents` conversation view will not load in dev-10 on the
...
```sh
cd dashboard && HYDRA_ADMIN_URL=http://localhost:52100 \
  HYDRA_PUBLIC_URL=http://localhost:58100 \
  VITE_API_URL=http://localhost:54100/graphql \
  VITE_SSR_API_URL=http://localhost:54100/graphql \
  VITE_KRATOS_PUBLIC_URL=http://localhost:51100 \
  VITE_KRATOS_SSR_URL=http://localhost:51100 \
  VITE_AGENT_STREAM_URL=http://localhost:62100 \
  yarn dev --port 50100
```
...
```
Access to fetch at 'http://localhost:54100/v1/agent-sessions/<id>/stream'
...
Pointed at `62100` the request reaches the gateway's attach listener directly,
which does allow the ticket header and echoes CORS for the dev-10 dashboard
...
- **The bex operator** — already cluster-scoped in the shared cluster,
  reconciles CRs in every namespace including `dev-10`. No need for a second
...
`values/`, `rbac-dev-10.yaml`) used to live in this directory, copied ten times.
They are now one implementation — **`scripts/dev-env.sh 10 {up|down|status|clean|env}`**
...
to keep before re-running. `scripts/dev-env.sh 10 clean` reclaims `logs/` and
