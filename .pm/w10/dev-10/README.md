...
```sh
bash .pm/w10/dev-10/up.sh      # idempotent: creates namespaces, CNPG Clusters,
                               # mailpit, kratos+hydra (helm), port-forwards,
                               # and a local bex-api
bash .pm/w10/dev-10/status.sh  # pod status + http health checks
bash .pm/w10/dev-10/down.sh    # kills local processes, deletes both namespaces
```
...
`up.sh` sets **`BEX_CP_IDENTITY=dev-10`**, so every tenant namespace this
