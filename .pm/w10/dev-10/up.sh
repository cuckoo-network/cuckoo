#!/usr/bin/env bash
# Bring up the dev-10 isolated environment (workstream w10): its own Kratos +
# Hydra + Mailpit (namespace dev-10-auth) on the shared local kind/CAPD cluster
...
# namespace (dev-10) are duplicated, so this never collides with the shared
...
#
# Usage: bash .pm/w10/dev-10/up.sh
...
echo "==> operator RBAC for the dev-10 apps namespace (secrets CRUD — see rbac-dev-10.yaml)"
kubectl apply -f "$ENVDIR/rbac-dev-10.yaml" >/dev/null
...
# scripts/auth-secrets.sh's dsn() helper, dev-10-auth namespace).
dsn() {
...
echo "==> out-of-band kratos/hydra secrets (freshly generated, dev-10-auth only)"
...
echo
echo "dev-10 (workstream w10) is up:"
