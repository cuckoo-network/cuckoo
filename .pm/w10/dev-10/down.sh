#!/usr/bin/env bash
# Tear down the dev-10 isolated environment (workstream w10): kills the local
# processes up.sh forked (port-forwards, bex-api) and deletes the
# dev-10-auth / dev-10 namespaces from the shared cluster. Leaves the shared
# cluster itself (and any other workstream's resources) untouched.
#
# Usage: bash .pm/w10/dev-10/down.sh
set -euo pipefail
...
echo "dev-10 down. Namespace deletion continues in the background (kubectl get ns to watch)."
