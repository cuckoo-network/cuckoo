...
- [ ] **m16** — Events feed fidelity: required from/to details + auto_deploy discrimination (7 tasks) ← from `/pm-brainstorm` round 10, 2026-07-15 (the ADR018 Service-events row's two closable ◐ divergences — `from`/`to` Render marks required on plan/instance-count/autoscaling events, and the `auto_deploy_enabled`/`_disabled` split — via the maintenance-mode typed-nullable-field precedent, never a generic arguments object)
