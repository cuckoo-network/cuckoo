...
**Worker:** worker3 **Goal:** ship the Vercel-AI-SDK-compatible conversation plane decided in ADR047 § D9 "Target API shape": one ticket-authenticated stream endpoint published under the primary API origin — `POST` submits a live prompt turn, `GET` replays the transcript then splices the live tail (terminal sessions: replay-only) — served by the gateway process under the D3 verbatim-forward guarantee, with the transcript store and the `attach-ticket` mint closing the reconnect/history gaps. **Status:** implementation complete + locally verified (backend 47 pkgs incl. real-Postgres transcript round-trip, gateway attach replay/live/terminal/reject, driver `POST /turn`, namespace gateway-ingress policy; driver 22 tests; `make lint-backend` 0 issues); **awaiting the live-substrate E2E run** (shares the m41 operator-run gate) to satisfy the DoD and close.
