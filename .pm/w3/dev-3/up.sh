#!/usr/bin/env bash
# Bring up the dev-3 isolated environment (workstream w3): its own Kratos +
# Hydra + Mailpit (namespace dev-3-auth) on the shared local kind/CAPD cluster
...
# namespace (dev-3) are duplicated, so this never collides with the shared
...
# scripts/auth-secrets.sh's dsn() helper, dev-3-auth namespace).
dsn() {
...
echo "==> out-of-band kratos/hydra secrets (freshly generated, dev-3-auth only)"
...
echo
echo "dev-3 (workstream w3) is up:"
