...
```sh
bash .pm/w3/dev-3/up.sh      # idempotent: creates namespaces, CNPG Clusters,
                               # mailpit, kratos+hydra (helm), port-forwards,
                               # and a local bex-api
bash .pm/w3/dev-3/status.sh  # pod status + http health checks
bash .pm/w3/dev-3/down.sh    # kills local processes, deletes both namespaces
```
...
`up.sh` sets **`BEX_CP_IDENTITY=dev-3`**, so every tenant namespace this
