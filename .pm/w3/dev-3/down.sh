#!/usr/bin/env bash
# Tear down the dev-3 isolated environment (workstream w3): kills the local
# processes up.sh forked (port-forwards, bex-api) and deletes the
# dev-3-auth / dev-3 namespaces from the shared cluster. Leaves the shared
# cluster itself (and any other workstream's resources) untouched.
#
# Usage: bash .pm/w3/dev-3/down.sh
set -euo pipefail
...
echo "dev-3 down. Namespace deletion continues in the background (kubectl get ns to watch)."
