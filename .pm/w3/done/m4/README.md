...
- App pods are matched by Deployment pod-name shape (`pod=~"<app>-[a-z0-9]+-[a-z0-9]{5}"`) since kubelet metrics carry no pod labels; kube-state-metrics (label joins) stays out of scope — recorded as a documented heuristic beside the Traefik service selector in docs/observability.md.
...
- `cpu_limit`/`memory_limit` intentionally stay single-point (no fabricated limit history) — documented in docs/observability.md.
