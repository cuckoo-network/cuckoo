...
- **Security follow-up (closed):** m35 was planned as a prerequisite but landed concurrently after this milestone closed. Its owner check is applied to exec itself, and its production Cilium/RBAC/admission hardening — per-sandbox network isolation, fail-closed DNS/FQDN/SNI egress, durable owner enforcement, least-privilege lifecycle RBAC, and admission confinement of the sandbox Pod runtime and NamespaceReconciler — is deployed and passed the full ownership/network/DNS-SNI/RBAC/admission/Hubble matrix on the production gVisor/Cilium substrate (2026-07-30, 40 checks, clean disposable-fixture audit). The outstanding sandbox-exec security dependency is satisfied; the sandbox may now expand further.
