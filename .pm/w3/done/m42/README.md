...
## Warm-Pool re-disposition (m32 t003 deferral, re-evaluated at m42 closeout)

Warm Pool stays **deferred as a post-v1 watch item**, not adopted. m32 deferred it as "no per-tenant value + snapshot moot"; the reasoning survives the pause/resume fix and is stronger now. Under ADR043's per-tenant `<ws>-sandbox` scoping (ADR042 D3/D4) a tenant's create cannot draw from a pool that lives in a shared `opensandbox-system` namespace, so a naive warm `Pool` (D1) buys no cross-tenant create-latency win — it would need a per-tenant (or claim-time-move) warm strategy first. And now that rootfs hibernation works, the load-bearing "sleep = free / fast wake" economics (ADR047 D1/D6) are delivered by resume-from-snapshot, not by pre-warmed idle capacity — so the pool's remaining value is narrow cold-start latency, which does not justify per-tenant warm capacity in v1. Recorded as the standing D1 warm-`Pool` watch item in [docs/ADR042-sandbox-cluster-substrate.md](../../../docs/ADR042-sandbox-cluster-substrate.md) (§ watch items); no follow-up milestone opened.
