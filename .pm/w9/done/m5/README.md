...
**Worker:** worker9 **Goal:** `GET /services/{id}/secret-files` honors Render's `cursor`/`limit` contract (today the params are accepted and ignored), and the GraphQL/MCP/dashboard secret-file lists page to match — the w9/m10 stable-name cursor pattern applied to env vars' sibling route. **Status:** done

## Tasks (in order)

| id   | title                                                                                     | est | depends_on |     |
| ---- | -------------------------------------------------------------------------------------------- | --- | ---------- | --- |
| t001 | REST: honor `cursor`/`limit` with the m10 stable-name cursor; conformance suite clean          | 30m | —          | — **DONE** |
| t002 | GraphQL/MCP pagination args to match (the m10 shapes); dashboard list follows                  | 40m | t001       | — **DONE** |
| t003 | Boundary tests (first/last/expired cursor, over-limit) + a verify-script leg if the CLI exercises the route | 20m | t002       | — **DONE** |
| t004 | Render parity — param semantics vs Render's env-vars pagination; three-surface consistency     | 20m | t003       | — **DONE** |
| t005 | Simplify — `/simplify` over the code this milestone changed                                    | 20m | t004       | — **DONE** |
| t006 | Test coverage — meaningful pagination tests beyond t003's boundaries (stability under mutation) | 20m | t004       | — **DONE** |
