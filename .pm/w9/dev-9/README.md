...
| hydra-public / issuer (port-forward, BEX_OAUTH_ISSUER) | `58090` |
| loki (port-forward, BEX_LOKI_URL) | `59090` |
| mailpit SMTP (port-forward, BEX_SMTP_ADDR) | `60090` |
| OpenSandbox lifecycle server (`agent-up`) | `61090` |
| ssh-gateway agent attach SSE (`agent-up`) | `62090` |
| OpenFGA (`agent-up`) | `63090` |
| OpenBao (`agent-up`) | `64090` |
| ssh-gateway sandbox-exec (`agent-up`) | `65090` |

## Start / status / stop

```sh
bash scripts/dev-env.sh 9 up      # idempotent: creates namespaces, CNPG Clusters,
                               # mailpit, kratos+hydra (helm), port-forwards,
                               # and a local bex-api
bash scripts/dev-env.sh 9 status  # pod status + http health checks
bash scripts/dev-env.sh 9 down    # kills local processes, deletes both namespaces
```

Then start the dashboard separately (kept out of `dev-env.sh 9 up` so it stays in the
foreground / under your own process control — `dev-env.sh 9 up` prints the exact
command):

```sh
cd dashboard && VITE_API_URL=http://localhost:54090/graphql \
  VITE_SSR_API_URL=http://localhost:54090/graphql \
  VITE_KRATOS_PUBLIC_URL=http://localhost:51090 yarn dev --port 50090
```

## Recovering after a restart

`dev-env.sh 9 up` is idempotent — re-running it after a reboot (or after the shared
...
`dev-env.sh 9 up` sets **`BEX_CP_IDENTITY=dev-9`**, so every tenant namespace this
...
`dev-env.sh 9 status` prints every managed tenant namespace with its owner — if a row
...
none of that machinery. `bash scripts/dev-env.sh 9 agent-up` layers it on:
...
| **OpenFGA** (`dev-9-auth`) | `cmd/ssh-gateway` calls `requiredEnv("BEX_OPENFGA_URL")` and has no allow-all hatch; bex-api must share the store or every attach is denied |
| **OpenBao** (`dev-9-auth`, dev mode) | the BYO model key (ADR062) the gateway mints per exchange |
...
stamped `app.bex.co/dev-env: dev-9` and refuses to take it from another
...
bash scripts/dev-env.sh 9 agent-netpol <workspaceId>
...
`bash scripts/dev-env.sh 9 agent-stub` stands up a local Anthropic Messages API
...
The dashboard's `/agents` conversation view will not load in dev-9 on the
...
```sh
cd dashboard && HYDRA_ADMIN_URL=http://localhost:52090 \
  HYDRA_PUBLIC_URL=http://localhost:58090 \
  VITE_API_URL=http://localhost:54090/graphql \
  VITE_SSR_API_URL=http://localhost:54090/graphql \
  VITE_KRATOS_PUBLIC_URL=http://localhost:51090 \
  VITE_KRATOS_SSR_URL=http://localhost:51090 \
  VITE_AGENT_STREAM_URL=http://localhost:62090 \
  yarn dev --port 50090
```
...
```
Access to fetch at 'http://localhost:54090/v1/agent-sessions/<id>/stream'
...
Pointed at `62090` the request reaches the gateway's attach listener directly,
which does allow the ticket header and echoes CORS for the dev-9 dashboard
...
`values/`, `rbac-dev-9.yaml`) used to live in this directory, copied ten times.
They are now one implementation — **`scripts/dev-env.sh 9 {up|down|status|clean|env}`**
...
to keep before re-running. `scripts/dev-env.sh 9 clean` reclaims `logs/` and
