#!/usr/bin/env bash
# Tear down the dev-9 isolated environment (workstream w9): kills the local
# processes up.sh forked (port-forwards, bex-api) and deletes the
# dev-9-auth / dev-9 namespaces from the shared cluster. Leaves the shared
# cluster itself (and any other workstream's resources) untouched.
#
# Usage: bash .pm/w9/dev-9/down.sh
set -euo pipefail
...
echo "dev-9 down. Namespace deletion continues in the background (kubectl get ns to watch)."
