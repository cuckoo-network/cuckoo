#!/usr/bin/env bash
# Bring up the dev-9 isolated environment (workstream w9): its own Kratos +
# Hydra + Mailpit (namespace dev-9-auth) on the shared local kind/CAPD cluster
...
# namespace (dev-9) are duplicated, so this never collides with the shared
...
echo "==> operator RBAC for the dev-9 apps namespace (secrets CRUD — see rbac-dev-9.yaml)"
kubectl apply -f "$ENVDIR/rbac-dev-9.yaml" >/dev/null
...
# scripts/auth-secrets.sh's dsn() helper, dev-9-auth namespace).
dsn() {
...
echo "==> out-of-band kratos/hydra secrets (freshly generated, dev-9-auth only)"
...
export BEX_BOOTSTRAP_CLIENT_SECRET="${BEX_BOOTSTRAP_CLIENT_SECRET:-dev-9-bootstrap-secret}"
...
# The internal control-plane listener is loopback-only in this disposable dev
# harness. Production must supply BEX_CP_TOKEN instead.
api_started=0
...
echo
echo "dev-9 (workstream w9) is up:"
