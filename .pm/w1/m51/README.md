...
**Worker:** worker1 **Goal:** make egress numbers true end to end — the router resolver counts Traefik's `websecure-` TLS sibling (where essentially all real traffic flows) so the chart, month-to-date, health source, and billing meter stop measuring only 80→443 redirects; and the usage rollup records each hour's healthy sources instead of deferring the whole hour forever, so prod finally writes `egress_bytes` rows. **Status:** in progress — t001–t003 + t006–t007 done 2026-07-18 (ground truth: Traefik v3.7.5 mints exactly base + `websecure-` sibling per TLS host, prod inventory reconciled; resolver emits both incl. collision-hashed keys; rollup records healthy sources / skips unhealthy / defers only on transport, trichotomy test-pinned; ADR023+ADR018 updated; simplify residual appended to `w1/033`); t004 prod verification + t005 cross-surface spot-check await the deploy, t008 closes after
...
| t004 | Prod verification: HTTPS volume in chart/month-to-date + egress_bytes rows land   | 30m | t002, t003 |
| t005 | Render parity — cross-surface consistency + ADR018/ADR023 notes                   | 30m | t004       |
...
| t008 | Closeout — verify DoD, sync status, move to done                                  | 15m | t006, t007 |
...
- **Goal linkage:** ADR023's metering integrity (bandwidth that is *true*, not merely never-invented) and Render-parity credibility of the bandwidth surfaces; direct continuation of w1/m50.
...
- Reset-tolerant http *accounting* beyond per-source skipping — hours where the websecure counter flaps stay excluded from billing by the per-source health gate (its `increase()` inflation is exactly what the gate exists to block).
