#!/usr/bin/env bash
# Bring up the dev-1 isolated environment (workstream w1): its own Kratos +
# Hydra + Mailpit (namespace dev-1-auth) on the shared local kind/CAPD cluster
...
# namespace (dev-1) are duplicated, so this never collides with the shared
...
echo "==> operator RBAC for the dev-1 apps namespace (secrets CRUD — see rbac-dev-1.yaml)"
kubectl apply -f "$ENVDIR/rbac-dev-1.yaml" >/dev/null
...
# scripts/auth-secrets.sh's dsn() helper, dev-1-auth namespace).
dsn() {
...
echo "==> out-of-band kratos/hydra secrets (freshly generated, dev-1-auth only)"
