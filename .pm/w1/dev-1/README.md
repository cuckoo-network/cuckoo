# dev-1 — workstream w1's isolated local dev environment

A dedicated Kratos + Hydra + Mailpit stack (namespace `dev-1-auth`) plus a
dedicated app namespace (`dev-1`), layered onto the **shared** local
...
`dev-1` sidesteps this by giving workstream w1 its own Kratos/Hydra with
...
| --- | --- |
| dashboard | `50010` |
| kratos-public (port-forward) | `51010` |
| hydra-admin (port-forward) | `52010` |
| mailpit UI (port-forward) | `53010` |
| bex-api (local process) | `54010` |
| bex-db (port-forward) | `55010` |
| bex-api control-plane API (BEX_CP_ADDR, host-only) | `56010` |
| kratos-admin (port-forward, BEX_KRATOS_ADMIN_URL) | `57010` |
| hydra-public / issuer (port-forward, BEX_OAUTH_ISSUER) | `58010` |
| loki (port-forward, BEX_LOKI_URL) | `59010` |
| mailpit SMTP (port-forward, BEX_SMTP_ADDR) | `60010` |
| OpenSandbox lifecycle server (`agent-up`) | `61010` |
| ssh-gateway agent attach SSE (`agent-up`) | `62010` |
| OpenFGA (`agent-up`) | `63010` |
| OpenBao (`agent-up`) | `64010` |
| ssh-gateway sandbox-exec (`agent-up`) | `65010` |
...
KeyValue/App/Database CRs in the `dev-1` namespace survive as long as the
...
none of that machinery. `bash scripts/dev-env.sh 1 agent-up` layers it on:
...
| **OpenFGA** (`dev-1-auth`) | `cmd/ssh-gateway` calls `requiredEnv("BEX_OPENFGA_URL")` and has no allow-all hatch; bex-api must share the store or every attach is denied |
| **OpenBao** (`dev-1-auth`, dev mode) | the BYO model key (ADR062) the gateway mints per exchange |
...
stamped `app.bex.co/dev-env: dev-1` and refuses to take it from another
...
bash scripts/dev-env.sh 1 agent-netpol <workspaceId>
...
`bash scripts/dev-env.sh 1 agent-stub` stands up a local Anthropic Messages API
...
The dashboard's `/agents` conversation view will not load in dev-1 on the
...
```
Access to fetch at 'http://localhost:54010/v1/agent-sessions/<id>/stream'
...
Pointed at `62010` the request reaches the gateway's attach listener directly,
which does allow the ticket header and echoes CORS for the dev-1 dashboard
...
- **The bex operator** — already cluster-scoped in the shared cluster,
  reconciles CRs in every namespace including `dev-1`. No need for a second
