...
**Worker:** worker1 **Goal:** A workspace can create a named project, assign existing services to it, and see them grouped together on the services list across REST/GraphQL/MCP/dashboard — matching Render's project-grouping behavior. **Status:** todo

## Tasks (in order)

| id   | title                                                                                                                                | est | depends_on |
| ---- | ---------------------------------------------------------------------------------------------------------------------------------------- | --- | ---------- |
| t001 | Data model: `projects` table (workspace-scoped: id, name, created-by/at) + a nullable `project_id` column on the apps/services row; mint the id through `lego/backend/internal/id` | 45m | —          |
| t002 | REST: CRUD `/v1/projects` + assign/unassign a service to a project                                                                   | 40m | t001       |
| t003 | GraphQL: mirror t002 (`projects`/`project` queries, `createProject`/`deleteProject`/`assignServiceToProject` mutations)              | 30m | t002       |
| t004 | MCP: `list_/create_/delete_project` + assign tool                                                                                    | 25m | t002       |
| t005 | Dashboard: services list groups by project (Render's dashboard-organization shape — ungrouped services still show flat), a lightweight project-create/rename/delete flow | 1h  | t003, t004 |
| t006 | Live verification: create a project, assign two real services to it, confirm both REST and dashboard reflect the grouping consistently | 30m | t005       |
