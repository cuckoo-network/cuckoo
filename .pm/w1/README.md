...
- [ ] **m33** — Pre-Deploy Command: gate rollout on a setup/migration step (9 tasks) ← from `/pm-brainstorm more` 2026-07-13 (`docs/ADR006-bex-api.md:124` lists `preDeployCommand` as an "ignored" bex.yml field — Render's Deploy section confirmed live via `w5/done/m13`; no overlap with `w6/m21`'s `dockerfilePath`/`startCommand` scope)
