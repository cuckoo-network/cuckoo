...
**Worker:** worker1 **Goal:** The control-plane store (w1/m2) grows real tenancy: a new identity gets a tenant row + membership, API keys bind to their tenant's `workspace:tea-<id>`, every verb authorizes against the caller's workspace instead of `workspace:default`, and prod flips from allow-all to enforced OpenFGA. **Status:** DONE (2026-07-09) — mint-on-first-login, key→tenant binding, workspace-scoped authz, prod OpenFGA flip, simplified + tested.

## Tasks (in order)

| id   | title                                                                                      | est | depends_on |
| ---- | ------------------------------------------------------------------------------------------- | --- | ---------- |
| t001 | Tenant mint on first login: Kratos identity → tenant row + `tenant_members` — **DONE**      | 30m | w1/m2/t007 |
| t002 | Key→tenant binding: OAuth2 client → `tenant_oauth_clients` + FGA membership — **DONE**       | 30m | t001       |
| t003 | Scope `core.Authorize` + list/get to the caller's workspace (retire `default`) — **DONE**    | 30m | t002       |
| t004 | Enable `BEX_OPENFGA_URL` in the prod api Deployment + cross-tenant 403 check — **DONE**      | 25m | t003       |
