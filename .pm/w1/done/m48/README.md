...
**Worker:** worker1 **Goal:** The deploy log feed always has something honest to say from the second a deploy is created — Render-style platform progress lines (`==> Build queued…`, `==> Building from <repo>…`, `==> Your service is live`) synthesized from the deploy lifecycle, visible identically on REST/GraphQL/MCP, the SSE tail, and the dashboard. **Status:** done — 2026-07-17: contract captured (live-deploy-following.md § Platform progress-line contract), `internal/logs/progress.go` synthesis merged into explicit `type=build` history reads (one core verb ⇒ REST/GraphQL/MCP identical) + the SSE tail's subscribe catch-up / wait-loop transitions / post-stream terminal check; live-proven on dev-1: `==> Build queued` streamed at subscribe with the build pod absent, `==> Building from …@main` streamed on the started transition, the deploy page rendered both during `Building` with the storeless banner intact, and the UNMODIFIED official Render CLI (`render logs --type build --tail`) printed the same lines with identical deterministic ids. Loki-backed history reads are unit-pinned (dev-1 is storeless); prod-live check after the next deploy is `w1/032`.

## Tasks (in order)

| id   | title                                                                                                 | est | depends_on |
| ---- | ----------------------------------------------------------------------------------------------------- | --- | ---------- |
| t001 | Capture Render's platform progress-line contract (vocabulary, log type, ids, CLI) — **DONE**          | 30m | —          |
| t002 | Core synthesis: deploy-lifecycle entries merged into windowed `type=build` history reads — **DONE**   | 45m | t001       |
| t003 | SSE tail: emit synthesized progress lines on deploy-status transitions while following — **DONE**     | 45m | t002       |
| t004 | Dashboard: verify the deploy page never renders the silent-window empty state — **DONE**              | 30m | t003       |
| t005 | Render parity: same lines on REST/GraphQL/MCP + official CLI `render logs --type build` — **DONE**    | 30m | t004       |
| t006 | Simplify: `/simplify` over the code this milestone changed — **DONE** (logID reused as follower dedupe key; diff reviewed for reuse/altitude, no other findings) | 20m | t005 |
| t007 | Test coverage: deterministic ids, window edges, no-duplicate merge, terminal-line cases — **DONE**    | 30m | t005       |
| t008 | Closeout: verify DoD, move milestone to done — **DONE**                                               | 10m | t007       |
