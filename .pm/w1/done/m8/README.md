...
- The ladder itself, three times over: operator `tierResources` (`app_controller.go:158`), store `var tiers` + `normalizeTier` (`store/api.go:322-341`), and the docs table (`docs/control-plane.md` §Tiers). All three already mirror Render's published ladder — this milestone **collapses copies**, it does not invent sizes.
