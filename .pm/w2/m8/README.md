...
**Worker:** worker2 **Goal:** an admin connects a GitHub account once (bex GitHub App install); agents and the dashboard can enumerate its repos on all four surfaces. **Status:** todo

## Tasks (in order)

| id   | title                                                                                    | est | depends_on |
| ---- | ---------------------------------------------------------------------------------------- | --- | ---------- |
| t001 | ADR `docs/github-integration.md` — GitHub App model, manifest flow, env vars, trust chain | 40m | —          |
| t002 | `lego/backend/internal/github` client — app JWT, installation tokens, repo listing        | 45m | t001       |
| t003 | Control-plane store: `git_connections` table + connection verbs (authz-gated)             | 40m | t002       |
| t004 | REST: connect · callback · get/delete connection · `GET /v1/repos`                        | 45m | t003       |
| t005 | GraphQL (`gitConnection`/`repos` + mutations) and MCP (`list_repos`, `get_git_connection`) | 40m | t004       |
| t006 | Dashboard: Settings → "Connect GitHub" card (install link, status, disconnect)            | 45m | t005       |
| t007 | Render parity — cross-surface consistency + declare REST/MCP repo surface a bex superset  | 30m | t006       |
| t008 | Simplify — `/simplify` over the milestone's diff                                          | 30m | t007       |
| t009 | Test coverage — connection lifecycle, 503-when-unconfigured, token minting, pagination    | 40m | t007       |
| t010 | Closeout — DoD verified, move to `done/`                                                  | 15m | t009       |
