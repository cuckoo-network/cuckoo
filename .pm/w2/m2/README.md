...
| --- | --- | --- | --- |
| t001 | `Core.Deploy` + `POST /v1/deploy`: {repo, bex.yml} → build-from-git `App` CR | 30m | w1/m2, w1/m5 |
...
`docs/vision.md` pillar 4 (deploy-from-chat) + roadmap item 2 (wake activator + HMAC webhook); `docs/control-plane.md` request flow. **Cross-workstream:** needs w1/m2 (control plane) and w1/m5 (in-cluster builds) first.
