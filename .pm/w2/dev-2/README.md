...
| hydra-public / issuer (port-forward, BEX_OAUTH_ISSUER) | `58020` |
| loki (port-forward, BEX_LOKI_URL) | `59020` |
| mailpit SMTP (port-forward, BEX_SMTP_ADDR) | `60020` |
| OpenSandbox lifecycle server (`agent-up`) | `61020` |
| ssh-gateway agent attach SSE (`agent-up`) | `62020` |
| OpenFGA (`agent-up`) | `63020` |
| OpenBao (`agent-up`) | `64020` |
| ssh-gateway sandbox-exec (`agent-up`) | `65020` |
...
cd dashboard && VITE_API_URL=http://localhost:54020/graphql \
  VITE_SSR_API_URL=http://localhost:54020/graphql \
...
none of that machinery. `bash scripts/dev-env.sh 2 agent-up` layers it on:
...
| **OpenFGA** (`dev-2-auth`) | `cmd/ssh-gateway` calls `requiredEnv("BEX_OPENFGA_URL")` and has no allow-all hatch; bex-api must share the store or every attach is denied |
| **OpenBao** (`dev-2-auth`, dev mode) | the BYO model key (ADR062) the gateway mints per exchange |
...
stamped `app.bex.co/dev-env: dev-2` and refuses to take it from another
...
bash scripts/dev-env.sh 2 agent-netpol <workspaceId>
...
`bash scripts/dev-env.sh 2 agent-stub` stands up a local Anthropic Messages API
...
The dashboard's `/agents` conversation view will not load in dev-2 on the
...
```sh
cd dashboard && HYDRA_ADMIN_URL=http://localhost:52020 \
  HYDRA_PUBLIC_URL=http://localhost:58020 \
  VITE_API_URL=http://localhost:54020/graphql \
  VITE_SSR_API_URL=http://localhost:54020/graphql \
  VITE_KRATOS_PUBLIC_URL=http://localhost:51020 \
  VITE_KRATOS_SSR_URL=http://localhost:51020 \
  VITE_AGENT_STREAM_URL=http://localhost:62020 \
  yarn dev --port 50020
```
...
```
Access to fetch at 'http://localhost:54020/v1/agent-sessions/<id>/stream'
...
Pointed at `62020` the request reaches the gateway's attach listener directly,
which does allow the ticket header and echoes CORS for the dev-2 dashboard
