...
```sh
bash .pm/w2/dev-2/up.sh      # idempotent: creates namespaces, CNPG Clusters,
                               # mailpit, kratos+hydra (helm), port-forwards,
                               # and a local bex-api
bash .pm/w2/dev-2/status.sh  # pod status + http health checks
bash .pm/w2/dev-2/down.sh    # kills local processes, deletes both namespaces
```
...
`up.sh` sets **`BEX_CP_IDENTITY=dev-2`**, so every tenant namespace this
