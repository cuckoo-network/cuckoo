#!/usr/bin/env bash
# Bring up the dev-2 isolated environment (workstream w2): its own Kratos +
# Hydra + Mailpit (namespace dev-2-auth) on the shared local kind/CAPD cluster
...
# namespace (dev-2) are duplicated, so this never collides with the shared
...
echo "==> operator RBAC for the dev-2 apps namespace (secrets CRUD — see rbac-dev-2.yaml)"
kubectl apply -f "$ENVDIR/rbac-dev-2.yaml" >/dev/null
...
# scripts/auth-secrets.sh's dsn() helper, dev-2-auth namespace).
dsn() {
...
echo "==> out-of-band kratos/hydra secrets (freshly generated, dev-2-auth only)"
...
echo
echo "dev-2 (workstream w2) is up:"
