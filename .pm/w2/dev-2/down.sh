#!/usr/bin/env bash
# Tear down the dev-2 isolated environment (workstream w2): kills the local
# processes up.sh forked (port-forwards, bex-api) and deletes the
# dev-2-auth / dev-2 namespaces from the shared cluster. Leaves the shared
# cluster itself (and any other workstream's resources) untouched.
#
# Usage: bash .pm/w2/dev-2/down.sh
set -euo pipefail
...
echo "dev-2 down. Namespace deletion continues in the background (kubectl get ns to watch)."
