...
**Worker:** worker2 **Goal:** `deploys.Service.List(ctx, service string)` takes no arguments — every surface (`GET .../deploys`, GraphQL `deploys(serviceId)`, MCP `list_deploys`) always returns the full, unbounded deploy history. Render's real `ListDeploysParams` supports `status[]`/`createdBefore`/`createdAfter`/`cursor`/`limit`; add the subset bex can honestly back today. **Status:** done (2026-07-14)

## Tasks (in order)

| id   | title                                                                                                     | est | depends_on |
| ---- | ------------------------------------------------------------------------------------------------------------ | --- | ---------- |
| t001 | `store.ListDeploys`: add `status[]`/`createdBefore`/`createdAfter`/`cursor`/`limit` query support — **DONE**  | 40m | —          |
| t002 | REST: `GET .../deploys` reads the matching query params — **DONE**                                            | 20m | t001       |
| t003 | GraphQL: `deploys(serviceId, status, createdBefore, createdAfter, cursor, limit)` — **DONE**                   | 20m | t001       |
| t004 | MCP: `list_deploys` gains `limit`/`cursor` — matches Render's own real MCP tool — **DONE**                     | 20m | t001       |
| t005 | Render parity — cross-surface consistency; refresh `docs/ADR018-render-parity.md`'s row — **DONE**             | 15m | t004       |
| t006 | Simplify — `/simplify` over the code this milestone changed — **DONE**                                        | 15m | t005       |
| t007 | Test coverage — filter combinations, cursor pagination correctness, limit bounds — **DONE**                   | 30m | t005       |
| t008 | Closeout — DoD met → move milestone to `done/` — **DONE**                                                      | 10m | t007       |

## Outcome (2026-07-14)

`store.DeployFilter` (statuses / exclusive `createdBefore`+`createdAfter` / keyset cursor on `(created_at, id)` — the `AuditFilter` idiom, now factored into a shared `pageNewestFirst` tail both use / limit, `<=0` = the pre-m31 unbounded contract, clamped at `core.MaxPageLimit` by the store's `clampPageLimit`). One `deploys.FilterOf` translator feeds all three surfaces (malformed timestamp or negative limit ⇒ 400, never a silently-dropped filter); REST + GraphQL take all five params, MCP `list_deploys` takes `limit` (default 10, Render's own MCP tool's default — an intentional change from full-history) + `cursor`, with the result's `cursor` field as the bex-shaped page marker. `TestDeployPaginationSurfaceParity` walks the same paged query through REST/GraphQL/MCP and asserts identical sequences; the cursor walk is proven gap/dup-free (memStore + real-Postgres runs). `finishedBefore/After` judged non-trivial (deferred alongside `w2/m32`'s `updatedAt`, per t001's guidance).
