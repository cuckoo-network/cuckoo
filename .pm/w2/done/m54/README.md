...
**Worker:** worker2 **Goal:** Give every bex dashboard page a deterministic, SSR-correct, localized document title following Render's current page/resource hierarchy, while adding a bex-branded global description/Open Graph/Twitter shell that never exposes private resource names. **Status:** DONE 2026-07-17

## Tasks (in order)

| id | title | est | depends_on |
| --- | --- | --- | --- |
| t001 | Shared head contract + global bex metadata shell — **DONE** | 45m | — |
| t002 | Static, auth, collection, create, and settings route titles — **DONE** | 45m | t001 |
| t003 | Project/workspace titles from loaded names — **DONE** | 40m | t001 |
| t004 | Service/deploy titles: SSR name + type, identical across tabs — **DONE** | 50m | t001 |
| t005 | Datastore, environment-group, and Blueprint dynamic titles — **DONE** | 50m | t001 |
| t006 | Redirect, API, error, loading, and not-found head policy + guard — **DONE** | 35m | t002–t005 |
| t007 | Render parity: live matrix + evidence close — **DONE** | 30m | t006 |
| t008 | Simplify: one title/data/metadata path — **DONE** | 30m | t007 |
| t009 | Test coverage: SSR, navigation, privacy, i18n, and route inventory — **DONE** | 45m | t007 |
...
- **Why now:** at kickoff the dashboard had grown to 58 route definitions, but only 30 declared a head; the remainder mixed legitimate inheritance/redirect/API cases with implicit behavior. Existing titles used three competing suffix/separator styles, project/service names were patched through client-only `document.title` effects, and Database/Key Value/env-group/Blueprint pages settled on opaque ids. w5/m42 solved only the post-load service-tab title and explicitly left the SSR id fallback, making the missing shared contract visible rather than complete.
