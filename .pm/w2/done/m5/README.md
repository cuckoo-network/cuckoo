...
**Worker:** worker2 **Goal:** Give agents Render's deploy poll-loop: every rollout is recorded as a deploy object (`dep-…`, status `build_in_progress → update_in_progress → live/failed`, per docs/deployment.md's health gating), listable over REST/GraphQL/MCP and triggerable via `POST /v1/services/{id}/deploys`. **Status:** DONE (t001–t004,t006,t007 DONE 2026-07-09; t005 live acceptance PASSED 2026-07-09 — real Postgres + real operator + real mock cluster: create → deploy #1 live; trigger → deploy #2 live, newest-first; broken image → `update_failed`, `lego/backend/internal/deploys/live_acceptance_test.go` `TestLiveAcceptance`)
