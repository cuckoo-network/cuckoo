...
- **Id decision:** name-as-id, matching the managed-Postgres sibling (no `red-` mint). `docs/identifiers.md` § Known deviations extended to name key-value stores alongside databases — keeps the two datastore surfaces uniform. See t002.
...
- **Docs:** `docs/render-parity.md` Key Value row flipped REST/GraphQL/MCP ✖→✅ (UI stays ✖ → w5/m12); `docs/bex-api.md` § Managed Key Value + MCP table; `docs/keyvalue-management.md` status; `docs/identifiers.md` deviation.
