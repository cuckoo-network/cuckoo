...
**Worker:** worker2 **Goal:** Render's Blueprint `initialDeployHook` — a command that runs exactly once, on a service's first successful deploy — works in `bex.yml`, riding the existing pre-deploy Job mechanism with a ran-once marker, retiring another "accepted but ignored" blueprint field. **Status:** todo

## Tasks (in order)

| id   | title                                                                       | est | depends_on |
| ---- | --------------------------------------------------------------------------- | --- | ---------- |
| t001 | Pin Render semantics; parse + persist the field with a ran-once marker      | 30m | —          |
| t002 | Execute via the pre-deploy Job path, gated on first successful deploy only  | 45m | t001       |
| t003 | Surface echo + no-rerun verification + ledger updates                       | 30m | t002       |
