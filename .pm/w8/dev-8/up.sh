#!/usr/bin/env bash
# Bring up the dev-8 isolated environment (workstream w8): its own Kratos +
# Hydra + Mailpit (namespace dev-8-auth) on the shared local kind/CAPD cluster
...
# namespace (dev-8) are duplicated, so this never collides with the shared
...
echo "==> operator RBAC for the dev-8 apps namespace (secrets CRUD — see rbac-dev-8.yaml)"
kubectl apply -f "$ENVDIR/rbac-dev-8.yaml" >/dev/null
...
# scripts/auth-secrets.sh's dsn() helper, dev-8-auth namespace).
dsn() {
...
echo "==> out-of-band kratos/hydra secrets (freshly generated, dev-8-auth only)"
...
echo
echo "dev-8 (workstream w8) is up:"
