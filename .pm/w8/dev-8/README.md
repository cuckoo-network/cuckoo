...
| hydra-public / issuer (port-forward, BEX_OAUTH_ISSUER) | `58080` |
| loki (port-forward, BEX_LOKI_URL) | `59080` |
| mailpit SMTP (port-forward, BEX_SMTP_ADDR) | `60080` |
| OpenSandbox lifecycle server (`agent-up`) | `61080` |
| ssh-gateway agent attach SSE (`agent-up`) | `62080` |
| OpenFGA (`agent-up`) | `63080` |
| OpenBao (`agent-up`) | `64080` |
| ssh-gateway sandbox-exec (`agent-up`) | `65080` |
...
cd dashboard && VITE_API_URL=http://localhost:54080/graphql \
  VITE_SSR_API_URL=http://localhost:54080/graphql \
...
none of that machinery. `bash scripts/dev-env.sh 8 agent-up` layers it on:
...
| **OpenFGA** (`dev-8-auth`) | `cmd/ssh-gateway` calls `requiredEnv("BEX_OPENFGA_URL")` and has no allow-all hatch; bex-api must share the store or every attach is denied |
| **OpenBao** (`dev-8-auth`, dev mode) | the BYO model key (ADR062) the gateway mints per exchange |
...
stamped `app.bex.co/dev-env: dev-8` and refuses to take it from another
...
bash scripts/dev-env.sh 8 agent-netpol <workspaceId>
...
`bash scripts/dev-env.sh 8 agent-stub` stands up a local Anthropic Messages API
...
The dashboard's `/agents` conversation view will not load in dev-8 on the
...
```sh
cd dashboard && HYDRA_ADMIN_URL=http://localhost:52080 \
  HYDRA_PUBLIC_URL=http://localhost:58080 \
  VITE_API_URL=http://localhost:54080/graphql \
  VITE_SSR_API_URL=http://localhost:54080/graphql \
  VITE_KRATOS_PUBLIC_URL=http://localhost:51080 \
  VITE_KRATOS_SSR_URL=http://localhost:51080 \
  VITE_AGENT_STREAM_URL=http://localhost:62080 \
  yarn dev --port 50080
```
...
```
Access to fetch at 'http://localhost:54080/v1/agent-sessions/<id>/stream'
...
Pointed at `62080` the request reaches the gateway's attach listener directly,
which does allow the ticket header and echoes CORS for the dev-8 dashboard
