...
```sh
bash .pm/w8/dev-8/up.sh      # idempotent: creates namespaces, CNPG Clusters,
                               # mailpit, kratos+hydra (helm), port-forwards,
                               # and a local bex-api
bash .pm/w8/dev-8/status.sh  # pod status + http health checks
bash .pm/w8/dev-8/down.sh    # kills local processes, deletes both namespaces
```
...
`up.sh` sets **`BEX_CP_IDENTITY=dev-8`**, so every tenant namespace this
