#!/usr/bin/env bash
# Tear down the dev-8 isolated environment (workstream w8): kills the local
# processes up.sh forked (port-forwards, bex-api) and deletes the
# dev-8-auth / dev-8 namespaces from the shared cluster. Leaves the shared
# cluster itself (and any other workstream's resources) untouched.
#
# Usage: bash .pm/w8/dev-8/down.sh
set -euo pipefail
...
echo "dev-8 down. Namespace deletion continues in the background (kubectl get ns to watch)."
