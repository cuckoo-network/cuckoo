...
**Worker:** worker8 **Goal:** A successful Git-backed Blueprint sync applies complete manifest bytes from the exact commit it reports, and source failures leave an actionable result without applying stale configuration. **Status:** done
...
| --- | --- | --- | --- |
| t001 | Reject incomplete and oversized Git file responses — **DONE** | 45m | — |
| t002 | Fetch Blueprint content at its resolved immutable commit — **DONE** | 45m | w8/m36/t001 |
| t003 | Fail Git-backed sync when its source is unavailable — **DONE** | 45m | w8/m36/t002 |
| t004 | Record source failures and require a persisted sync run before apply — **DONE** | 60m | w8/m36/t003 |
| t005 | Render parity — **DONE** | 30m | w8/m36/t004 |
| t006 | Simplify — **DONE** | 20m | w8/m36/t005 |
| t007 | Test coverage — **DONE** | 45m | w8/m36/t005, w8/m36/t006 |
| t008 | Closeout — **DONE** | 10m | w8/m36/t007 |
...
Implementation: `lego/backend/internal/github/client.go` (bounded complete-response reads, status classification, commit-SHA validation), `lego/backend/internal/github/service.go` (resolve-then-read seam: `ResolveBlueprintCommit` + `FetchBlueprintFileAtCommit`, shared token preamble; blob-SHA fallback removed), `lego/backend/internal/apps/blueprint.go` (pinned discovery, no stored-manifest fallback on Git-backed sync, `failSync` error runs, fail-closed run insert/completion), `docs/ADR018-render-parity.md` (Blueprint row repair note, row stays ◐).

- `cd lego/backend && go test ./...` — 62 packages ok, zero FAIL (includes the new fault-injection tests below and all pre-existing Blueprint/adapter cross-surface suites).
- `go vet` on the changed packages — clean.
- `make lint-backend` — the pinned golangci-lint binary panics (`file requires newer Go version go1.27`) identically on the untouched tree: pre-existing tool/environment breakage, not from this diff.
- New regressions: `TestGetFileContentsEnforcesSizeBound`, `TestGetFileContentsReadErrorIsError`, `TestGetFileContentsClassifiesFailures`, `TestGetRepoCommitSHAValidatesSHA` (httptest fault injection); `TestBlueprintFetchPinsImmutableCommit` (moving branch), `TestBlueprintResolveFailureIsActionable`, `TestBlueprintFetchHasNoBlobSHAFallback`; `TestSyncBlueprintFetchFailureRecordsErrorRun`, `TestSyncBlueprintWithoutFetcherFailsGitBackedSync`, `TestSyncBlueprintFailedRunInsertBlocksApply`, `TestSyncBlueprintFailedCompletionWriteIsError`, `TestSyncBlueprintSuppliedManifestClaimsNoCommit`, `TestBlueprintDiscoveryPinsOneRevision` (recorded side-effect assertions: zero App CRs, preserved manifest, error-run history).
- Adapters: no REST/GraphQL/MCP/dashboard shape change — the same views carry honest errors; dashboard Sync History (w6/m50) unchanged, no pending-state change so no skeleton side-by-side applies.
- No dev-8 run: no cluster-observable behavior changed and live GitHub cannot be fault-injected; the DoD's fault-injection evidence is the httptest/seam suite above. Pre-existing suites that pinned the removed fallback (`TestSyncBlueprintReappliesManifest`, auto-sync tenant tests, GraphQL sync legs) were moved onto the fetch seam with path-complete fixtures rather than deleted.
