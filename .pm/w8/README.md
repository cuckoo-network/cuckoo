...
- [ ] **m16** — Managed-datastore polish chores: create ipAllowList parity + pg_stat_statements backfill + KV version assessment (8 tasks; t008 added by round 19 — Render's inline PATCH `parameterOverrides` silently ignored) ← groups round-14 consistency find (postgres create `ipAllowList` is REST-only while keyvalue has all three surfaces) with notes `003` + `006`, 2026-07-15 (the w7/m37 chores pattern); coordinates with w4/m24 (descriptions) and w9/m38 (error bodies)
