...
- `007.md` — Live re-grade of m19's promoted blueprint fields + CLI validate on prod (filed by m19; blocked on shared dev-cluster health at close)
- `008.md` — Blueprint `resources[]` omits `environment_group` entries (+ env-group-only stacks suppress the plan) — sub-hour code chore, from the 2026-08-16 lifecycle verification
- `009.md` — Blueprint wire-shape divergence notes for ADR018 (`created` status unreachable, `redis` alias never emitted) — doc chore, from the 2026-08-16 lifecycle verification
