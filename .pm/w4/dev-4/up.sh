#!/usr/bin/env bash
# Bring up the dev-4 isolated environment (workstream w4): its own Kratos +
# Hydra + Mailpit (namespace dev-4-auth) on the shared local kind/CAPD cluster
...
# namespace (dev-4) are duplicated, so this never collides with the shared
...
echo "==> operator RBAC for the dev-4 apps namespace (secrets CRUD — see rbac-dev-4.yaml)"
kubectl apply -f "$ENVDIR/rbac-dev-4.yaml" >/dev/null
...
# scripts/auth-secrets.sh's dsn() helper, dev-4-auth namespace).
dsn() {
...
echo "==> out-of-band kratos/hydra secrets (freshly generated, dev-4-auth only)"
...
echo
echo "dev-4 (workstream w4) is up:"
