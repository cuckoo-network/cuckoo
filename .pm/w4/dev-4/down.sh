#!/usr/bin/env bash
# Tear down the dev-4 isolated environment (workstream w4): kills the local
# processes up.sh forked (port-forwards, bex-api) and deletes the
# dev-4-auth / dev-4 namespaces from the shared cluster. Leaves the shared
# cluster itself (and any other workstream's resources) untouched.
#
# Usage: bash .pm/w4/dev-4/down.sh
set -euo pipefail
...
echo "dev-4 down. Namespace deletion continues in the background (kubectl get ns to watch)."
