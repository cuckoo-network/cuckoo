# dev-4 — workstream w4's isolated local dev environment

A dedicated Kratos + Hydra + Mailpit stack (namespace `dev-4-auth`) plus a
dedicated app namespace (`dev-4`), layered onto the **shared** local
...
`dev-4` sidesteps this by giving workstream w4 its own Kratos/Hydra with
...
| --- | --- |
| dashboard | `50040` |
| kratos-public (port-forward) | `51040` |
| hydra-admin (port-forward) | `52040` |
| mailpit UI (port-forward) | `53040` |
| bex-api (local process) | `54040` |
| bex-db (port-forward) | `55040` |
| bex-api control-plane API (BEX_CP_ADDR, host-only) | `56040` |
| kratos-admin (port-forward, BEX_KRATOS_ADMIN_URL) | `57040` |
| hydra-public / issuer (port-forward, BEX_OAUTH_ISSUER) | `58040` |
| loki (port-forward, BEX_LOKI_URL) | `59040` |
| mailpit SMTP (port-forward, BEX_SMTP_ADDR) | `60040` |
| OpenSandbox lifecycle server (`agent-up`) | `61040` |
| ssh-gateway agent attach SSE (`agent-up`) | `62040` |
| OpenFGA (`agent-up`) | `63040` |
| OpenBao (`agent-up`) | `64040` |
| ssh-gateway sandbox-exec (`agent-up`) | `65040` |

## Start / status / stop

```sh
bash scripts/dev-env.sh 4 up      # idempotent: creates namespaces, CNPG Clusters,
                               # mailpit, kratos+hydra (helm), port-forwards,
                               # and a local bex-api
bash scripts/dev-env.sh 4 status  # pod status + http health checks
bash scripts/dev-env.sh 4 down    # kills local processes, deletes both namespaces
```

Then start the dashboard separately (kept out of `dev-env.sh 4 up` so it stays in the
foreground / under your own process control — `dev-env.sh 4 up` prints the exact
command):

```sh
cd dashboard && VITE_API_URL=http://localhost:54040/graphql \
  VITE_SSR_API_URL=http://localhost:54040/graphql \
  VITE_KRATOS_PUBLIC_URL=http://localhost:51040 yarn dev --port 50040
```

## Recovering after a restart

`dev-env.sh 4 up` is idempotent — re-running it after a reboot (or after the shared
...
`dev-env.sh 4 up` sets **`BEX_CP_IDENTITY=dev-4`**, so every tenant namespace this
...
`dev-env.sh 4 status` prints every managed tenant namespace with its owner — if a row
...
none of that machinery. `bash scripts/dev-env.sh 4 agent-up` layers it on:
...
| **OpenFGA** (`dev-4-auth`) | `cmd/ssh-gateway` calls `requiredEnv("BEX_OPENFGA_URL")` and has no allow-all hatch; bex-api must share the store or every attach is denied |
| **OpenBao** (`dev-4-auth`, dev mode) | the BYO model key (ADR062) the gateway mints per exchange |
...
stamped `app.bex.co/dev-env: dev-4` and refuses to take it from another
...
bash scripts/dev-env.sh 4 agent-netpol <workspaceId>
...
`bash scripts/dev-env.sh 4 agent-stub` stands up a local Anthropic Messages API
...
The dashboard's `/agents` conversation view will not load in dev-4 on the
...
```sh
cd dashboard && HYDRA_ADMIN_URL=http://localhost:52040 \
  HYDRA_PUBLIC_URL=http://localhost:58040 \
  VITE_API_URL=http://localhost:54040/graphql \
  VITE_SSR_API_URL=http://localhost:54040/graphql \
  VITE_KRATOS_PUBLIC_URL=http://localhost:51040 \
  VITE_KRATOS_SSR_URL=http://localhost:51040 \
  VITE_AGENT_STREAM_URL=http://localhost:62040 \
  yarn dev --port 50040
```
...
```
Access to fetch at 'http://localhost:54040/v1/agent-sessions/<id>/stream'
...
Pointed at `62040` the request reaches the gateway's attach listener directly,
which does allow the ticket header and echoes CORS for the dev-4 dashboard
...
- **The bex operator** — already cluster-scoped in the shared cluster,
  reconciles CRs in every namespace including `dev-4`. No need for a second
...
`values/`, `rbac-dev-4.yaml`) used to live in this directory, copied ten times.
They are now one implementation — **`scripts/dev-env.sh 4 {up|down|status|clean|env}`**
...
to keep before re-running. `scripts/dev-env.sh 4 clean` reclaims `logs/` and
