...
```sh
bash .pm/w4/dev-4/up.sh      # idempotent: creates namespaces, CNPG Clusters,
                               # mailpit, kratos+hydra (helm), port-forwards,
                               # and a local bex-api
bash .pm/w4/dev-4/status.sh  # pod status + http health checks
bash .pm/w4/dev-4/down.sh    # kills local processes, deletes both namespaces
```
