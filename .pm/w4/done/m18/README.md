...
**Worker:** worker4 **Goal:** A signed-in user can see and revoke everything that can currently act as them — the OAuth clients they've authorized (Hydra consent sessions; revoking kills the client's tokens) and their active browser sessions (Kratos; sign-out-everywhere) — as two sibling cards under Settings → Security & Compliance. **Status:** done — all 8 tasks complete; dashboard unit/component suite green (838 tests); `auth-oauth21-e2e.sh` leg 14 written + syntax-checked but not executed this session (the ports it needs were bound by a live local dev/cluster session — run it before treating the live e2e proof as verified in a clean environment)

## Tasks (in order)

| id   | title                                                                                                                                        | est | depends_on |
| ---- | --------------------------------------------------------------------------------------------------------------------------------------------- | --- | ---------- |
| t001 | Server-fns beside `hydra-consent.ts`: list + revoke a subject's Hydra consent sessions (admin API, server-only env) — **DONE**                 | 30m | —          |
| t002 | Settings → Security & Compliance: **Connected agents** card — client name, scopes, grant date, revoke with confirm — **DONE**                  | 35m | t001       |
| t003 | **Active sessions** card (`006`): Kratos `/sessions` list + "sign out other sessions" — **DONE**                                               | 40m | —          |
| t004 | e2e: extend `auth-oauth21-e2e.sh` — authorize → list shows the client → revoke → the agent's token stops working — **DONE** (leg 14; not run)  | 30m | t002, t003 |
| t005 | Render parity — superset check: no Render shape (no OAuth provider; session mgmt Kratos-owned); ADR018 "bex ahead" revocation evidence — **DONE** | 15m | t004       |
...
**Verification status:** the server-fn unit tests (`hydra-connected-agents.test.ts`, `kratos-sessions.test.ts`) exercise the exact list → revoke → invalidated-consent-session contract the e2e leg drives live, including the cross-site refusal and the merge-by-client view; the component/hook tests cover the UI wiring. Leg 14 of `scripts/auth-oauth21-e2e.sh` was written and `bash -n` syntax-checked, but not executed in this session — the local ports it needs (5173/5199, 8090, 4433/4434) were bound by an already-running dev/cluster session, and the user chose to skip a live run rather than risk interfering with it. Run `scripts/auth-oauth21-e2e.sh` in a clean environment to close that last gap before treating the live end-to-end proof as verified.
