...
**Worker:** worker4 **Goal:** An interrupted environment-group content operation can recover safely without permanently blocking the group or the workspace's group list. **Status:** done
...
| --- | --- | --- | --- |
| t001 | Persist operation ownership, phase, and recovery state within existing secret storage | 60m | w4/m97/t008 — **DONE** |
| t002 | Recover interrupted mixed saves while preventing stale writers from committing | 75m | t001 — **DONE** |
| t003 | Make interrupted clone source locks recoverable without changing source contents | 45m | t002 — **DONE** |
| t004 | Keep group lists usable and expose actionable busy/recovery states across existing surfaces | 60m | t002, t003 — **DONE** |
...
- **Source:** Depends on [w4/m97](../done/m97/README.md), which establishes metadata concurrency and lifecycle coordination. Completed [w2/m73](../../w2/done/m73/README.md) established staged saves and Clone; this milestone closes their process-interruption recovery gap.
...
- Durable op records + per-opID prior/proposed maps; lease-bounded recovery; clone source unlock; list `availability` field (REST/GraphQL).
- `go test ./internal/envgroups/ -race` green including `recovery_test.go`.
- Docs: ADR006/ADR018 + `docs/drills/2026-09-09-env-group-save-recovery.md`.
- Limitations: live `dev-4` kill/restart drill not run this session (recorded in the drill note). Dashboard GraphQL list/detail request `availability` and show a status row when set.
