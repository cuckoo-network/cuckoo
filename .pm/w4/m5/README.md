...
**Worker:** worker4 **Goal:** a versioned, policy-scoped secret store for _tenant_ credentials runs on the cluster as platform infrastructure (mirroring the w4/m1 Ory playbook: ADR → pinned Argo chart → out-of-band bootstrap → verify script), with the bex-api ServiceAccount able to read/write only `tenants/*` — ready for the product wiring in m6. Distinct from w1/m7 t003 (platform deploy secrets at rest in git). **Status:** todo

## Tasks (in order)

| id   | title                                                                                                            | est | depends_on |
| ---- | ---------------------------------------------------------------------------------------------------------------- | --- | ---------- |
| t001 | ADR `docs/secrets.md`: OpenBao for tenant credentials — alternatives, storage backend, unseal strategy            | 35m | —          |
| t002 | Argo Application for OpenBao (pinned chart, base values + local overlay)                                          | 35m | t001       |
| t003 | `scripts/bao-init.sh` — out-of-band init/unseal via `.env` + KV v2 mount `tenants/`                               | 40m | t002       |
| t004 | Kubernetes auth method: role + policy for the bex-api ServiceAccount scoped to `tenants/*`                        | 30m | t003       |
| t005 | `scripts/bao-verify.sh` — KV write/read, scoped SA login, restart durability, sealed-state behavior               | 35m | t004       |
| t006 | Docs: index entry, env tables, prod deploy path                                                                   | 25m | t005       |
| t007 | Simplify — run `/simplify` over the code this milestone changed                                                   | 20m | t006       |
