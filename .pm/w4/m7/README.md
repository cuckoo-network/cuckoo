...
| ---- | ------------------------------------------------------------------------------------------------------------ | --- | ---------- |
| t001 | SendGrid SMTP relay + secret plumbing (`.env` → `auth-secrets.sh` → gh-secrets) for both consumers (Kratos courier + bex-api `BEX_SMTP_*`), recorded in `docs/auth.md` | 30m | —          |
...
| t005 | Prod wiring + docs: `deploy.yml` step, `BEX_SMTP_*` into the bex-api deployment, `docs/auth.md` consequences updated, reset-password page verified | 30m | t004       |
