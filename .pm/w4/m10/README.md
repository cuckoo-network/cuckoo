...
**Worker:** worker4 **Goal:** Every authenticated write verb (and every authz denial) leaves one record — caller (`core.IdentityFrom`), verb, resource, outcome, timestamp — captured at one interception point in the feature kernel, stored in the control-plane store, readable by workspace admins. **Status:** todo
...
| t003 | Read surface: REST in Render's audit-logs shape (owner/workspace-scoped path, cursor paging) + GraphQL query, admin-scoped (`can_manage`) | 30m | t002       |
