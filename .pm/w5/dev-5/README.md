...
| kratos-admin (port-forward, BEX_KRATOS_ADMIN_URL) | `57050` |
| hydra-public / issuer (port-forward, BEX_OAUTH_ISSUER) | `58050` |
| loki (port-forward, BEX_LOKI_URL) | `59050` |
| mailpit SMTP (port-forward, BEX_SMTP_ADDR) | `60050` |
| OpenSandbox lifecycle server (`agent-up`) | `61050` |
| ssh-gateway agent attach SSE (`agent-up`) | `62050` |
| OpenFGA (`agent-up`) | `63050` |
| OpenBao (`agent-up`) | `64050` |
| ssh-gateway sandbox-exec (`agent-up`) | `65050` |

## Start / status / stop

```sh
bash scripts/dev-env.sh 5 up      # idempotent: creates namespaces, CNPG Clusters,
                               # mailpit, kratos+hydra (helm), port-forwards,
                               # and a local bex-api
bash scripts/dev-env.sh 5 status  # pod status + http health checks
bash scripts/dev-env.sh 5 down    # kills local processes, deletes both namespaces
```

Then start the dashboard separately (kept out of `dev-env.sh 5 up` so it stays in the
foreground / under your own process control — `dev-env.sh 5 up` prints the exact
command):
...
`dev-env.sh 5 up` is idempotent — re-running it after a reboot (or after the shared
...
`dev-env.sh 5 up` sets **`BEX_CP_IDENTITY=dev-5`**, so every tenant namespace this
control plane provisions is stamped `app.bex.co/control-plane: dev-5` and its
...
`dev-env.sh 5 status` prints every managed tenant namespace with its owner — if a row
shows an owner other than `dev-5` it belongs to another harness and this one
will leave it alone; if *this* harness's rows show anything but `dev-5`, its
...
none of that machinery. `bash scripts/dev-env.sh 5 agent-up` layers it on:
...
| **OpenFGA** (`dev-5-auth`) | `cmd/ssh-gateway` calls `requiredEnv("BEX_OPENFGA_URL")` and has no allow-all hatch; bex-api must share the store or every attach is denied |
| **OpenBao** (`dev-5-auth`, dev mode) | the BYO model key (ADR062) the gateway mints per exchange |
...
stamped `app.bex.co/dev-env: dev-5` and refuses to take it from another
...
bash scripts/dev-env.sh 5 agent-netpol <workspaceId>
...
`bash scripts/dev-env.sh 5 agent-stub` stands up a local Anthropic Messages API
...
`values/`, `rbac-dev-5.yaml`) used to live in this directory, copied ten times.
They are now one implementation — **`scripts/dev-env.sh 5 {up|down|status|clean|env}`**
...
to keep before re-running. `scripts/dev-env.sh 5 clean` reclaims `logs/` and
