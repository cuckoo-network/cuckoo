#!/usr/bin/env bash
# Tear down the dev-5 isolated environment (workstream w5): kills the local
# processes up.sh forked (port-forwards, bex-api) and deletes the
# dev-5-auth / dev-5 namespaces from the shared cluster. Leaves the shared
# cluster itself (and any other workstream's resources) untouched.
#
# Usage: bash .pm/w5/dev-5/down.sh
set -euo pipefail
...
echo "dev-5 down. Namespace deletion continues in the background (kubectl get ns to watch)."
