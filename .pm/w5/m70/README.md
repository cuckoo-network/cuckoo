...
- **Source:** 2026-08-17 discussion following the Stripe live cutover (runbook §6 executed 2026-08-16, first live tenant card-bound 2026-08-17): user request to learn from Render's workspace billing page (credit balance display) and to rename Usage→Billing for consistency. Credits are currently granted manually via Stripe Dashboard credit grants (the ADR049 product mechanism remains Proposed/unbuilt; this milestone is read-only display, not grant management).
...
- **Out of scope (recorded):** ADR049's grant/approval/idempotent-operations machinery, credit-first payment-gate semantics, and any collection-management UI. Wire renames of the usage API (ADR023 extension) are explicitly excluded.
