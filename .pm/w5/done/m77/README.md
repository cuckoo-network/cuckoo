...
| ---- | ------------------------------------------------------------------------ | --- | ---------- |
| t001 | Baseline the current ACP stream path and freeze the adoption boundary — **DONE** | 45m | —          |
| t002 | Consolidate reviewed ACP runtime profiles into one release-locked manifest — **DONE** | 60m | t001       |
| t003 | Batch transcript persistence without blocking browser forwarding — **DONE** | 75m | t001       |
| t004 | Buffer driver logging and reuse encoded stream frames — **DONE** | 60m | t001       |
| t005 | Simplify — `/simplify` over profile, driver, gateway, and store changes — **DONE** | 30m | t002, t003, t004 |
| t006 | Test coverage — profile validation, batching, ordering, and failure recovery — **DONE** | 60m | t005       |
...
Completed the partial implementation previously shipped in `1f1647b44`. One canonical manifest now drives backend, driver, and exact-package image installation. One bounded serial gateway writer preserves append order and terminal flush; the driver buffers writes and reuses encoded frames across GET/POST streaming. Reconnect and Completer recovery account for existing bytes once, including historical holes. See [measured evidence](evidence/README.md) for before/after operation counts, test commands, race/failure coverage, image checks, and the explicit production-model verification deferral.
