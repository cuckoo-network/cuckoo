...
- **Source:** user report 2026-07-09 (`/services/nightly-report` showed `www.eden-cms.com` data — traced to `dashboard/scripts/local-bex.mjs:194` echoing the single hardcoded `SERVICE` for any id) + inbox note `w5/006` (post-add DNS/CNAME instructions, split out of `w1/m11.5`), surfaced as the ◐ "Verify / DNS instructions" row in `docs/render-parity.md`.
