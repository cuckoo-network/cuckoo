# w5 · m9 — MCP `query_render_postgres` (read-only SQL for agents)

**Worker:** worker5 **Goal:** Un-defer the one Render MCP Postgres tool bex omitted — run a read-only SQL query against a managed database from the MCP surface, with hard safety rails (read-only transaction, statement timeout, row cap). **Status:** DONE (2026-07-08)

## Tasks (in order)

| id   | title                                                                                                            | est | depends_on | status |
| ---- | ------------------------------------------------------------------------------------------------------------------ | --- | ---------- | ------ |
| t001 | Query verb in the postgres feature: internal conn string, read-only enforcement, statement timeout, row cap        | 30m | —          | **DONE** |
| t002 | MCP tool under Render's exact name `query_render_postgres` (MCP-only, matching Render — no REST/GraphQL)            | 20m | t001       | **DONE** |
| t003 | Safety acceptance: writes rejected, timeout enforced, unknown db → not-found, values never logged                   | 20m | t001       | **DONE** |
...
> **t003 verification note:** the adversarial safety matrix (write-shape matrix incl. the multi-statement / `SET transaction_read_only=off` escapes, statement timeout, row-cap truncation, unknown-db not-found, value-free errors) was verified against a real `postgres:17` server via the gated `TestQueryReadOnlyIntegration` (`BEX_TEST_DB_URI`). Read-only-transaction / `statement_timeout` / extended-protocol semantics are core Postgres behaviors identical on CNPG, so the full mock-cluster CNPG deploy was not separately run.
