...
**Worker:** worker5 **Goal:** Normalize each replica with its own trustworthy limit before aggregation so usage charts remain correct through rollouts. **Status:** in-progress (t001–t007 done 2026-09-08; t008 open — service-level live walkthrough green 2026-09-09, authed-surface checks + pending/ready captures remaining)
...
REMAINING for t008 (strictly per this milestone's no-close-without-live-evidence rule): authed REST (`?percentage=true&aggregateAllMethod=AVG`) / GraphQL (`percentage:true`) / MCP / dashboard Percentage/Total checks against a real bex App (needs Kratos session + workspace + service create — not attempted), and the desktop + narrow-mobile pending/ready geometry captures. Then check the two DoD boxes, set t008 `status: done`, move it to `done/`, `mv .pm/w5/m90 .pm/w5/done/m90` (then `rmdir` the original — no tombstone), and flip the m90 workstream checkbox to `[x]`.
