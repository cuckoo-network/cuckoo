...
**Worker:** worker6 **Goal:** Env groups stop being the last workspace resource with a non-Render list shape and a delete leak: `GET /v1/env-groups` returns Render's `{envGroup, cursor}` envelope and honors `cursor`/`limit`; deleting a workspace purges its env groups; the lazy on-read ownership migration leaves an audit trail. **Status:** todo

## Tasks (in order)

| id   | title                                                                             | est | depends_on |
| ---- | ---------------------------------------------------------------------------------- | --- | ---------- |
| t001 | Verify Render's `GET /env-groups` contract in the pinned OpenAPI                  | 15m | —          |
| t002 | Implement the `{envGroup, cursor}` envelope + `cursor`/`limit`; check GraphQL/MCP | 45m | t001       |
| t003 | Extend the workspace-delete purger to env groups                                  | 60m | —          |
| t004 | Emit an audit event when the lazy on-read ownership migration fires               | 25m | —          |
