...
| t004 | Network policy conformance test: live-verify the `docs/tenant-isolation.md` reachability matrix actually holds on a running cluster (not just declared) — tenant↔tenant, tenant↔platform, tenant↔control-plane denies | 45m | —                                     |
...
| t006 | OAuth/OIDC audit: verify PKCE enforcement, token-audience checks (`BEX_OAUTH_RESOURCE`), and scope enforcement match `docs/auth.md` §7 as actually implemented, not just documented | 30m | —                                     |
...
- **Why now:** #7 is the last unowned V0 goal; RBAC/image-signing/injection surface have never been audited, and `docs/tenant-isolation.md`'s reachability matrix has no live test backing its threat-model claims.
