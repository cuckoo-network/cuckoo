...
**Worker:** worker6 **Goal:** Agents and API users see workspaces exactly as Render exposes them: `GET /v1/owners` (list, name/email filters), `GET /v1/owners/{ownerId}` (with user-ID→default-workspace behavior), `GET /v1/owners/{ownerId}/members` — read-only, `tea-`/`own-` ID semantics, `ownerId` scoping on every resource — plus the official MCP trio `list_workspaces` / `select_workspace` / `get_selected_workspace` with stateful per-session selection. **Status:** todo (gated on w6/m1)

## Tasks (in order)

| id   | title                                                                                          | est | depends_on   |
| ---- | ---------------------------------------------------------------------------------------------- | --- | ------------ |
| t001 | Pin the owner object schema against Render's OpenAPI spec (fields, `own-` prefix)              | 20m | —            |
| t002 | REST `GET /v1/owners` + `GET /v1/owners/{ownerId}` (filters, user-ID→default-workspace)        | 30m | t001, w6/m1  |
| t003 | REST `GET /v1/owners/{ownerId}/members` (Render-shaped, roles mapped from FGA)                 | 25m | t002         |
| t004 | `ownerId` scoping parity: field on every resource object + `ownerId` filter on list endpoints  | 25m | t002         |
| t005 | MCP tools: `list_workspaces` / `select_workspace` / `get_selected_workspace` (session-scoped)  | 30m | t002         |
| t006 | Render parity — field-for-field diff of owners/members/MCP responses vs Render's docs          | 20m | t003, t004, t005 |
