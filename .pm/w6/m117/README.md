...
| t002 | Stop reporting an unreadable recovery source as empty — propagate both read failures | 45m | t001 | — **DONE** |
| t003 | Make the Recovery card honest, and stop offering a restore it cannot substantiate | 45m | t002 | — **DONE** (2026-09-01 worktree: dashboard exposes read errors, disables Restore without both bounds; API refuses before billing/create) |
...
- **A failed read is visibly a failed read.** With the Barman `ObjectStore` Get or the CNPG `Backup` List failing (RBAC removed, CRD absent, wrong namespace — pick one and simulate it), `recovery-info` reports an explicit unknown/error state, and the card says so. It must not be byte-identical to a database that genuinely has no backups, which is the current behavior and the reason this shipped unnoticed.
...
- **Ground-truth resolution (2026-09-01, `hetzner-prod`):** all three Database CRs, CNPG Clusters, ScheduledBackups, and Backup objects are in the same tenant namespace (`tea-d98210cbbpdc73dcrkvg`). Each database has completed plugin-method backups through 2026-09-01; all three Clusters report `ContinuousArchiving=True` and `LastBackupSucceeded=True`, so there is no separate backup-operations incident. The native CNPG Cluster fields this API tried to read (`status.firstRecoverabilityPoint` / `status.lastSuccessfulBackup`) are absent on all three because these installations use Barman Cloud plugin backups. The authoritative data is instead present on `ObjectStore/bex-tenant-postgres.status.serverRecoveryWindow`, keyed by each Database CR's workspace-prefixed `status.backupServerName`:

  ```text
  beancount-forum-db  first=2026-08-21T22:42:06Z  last=2026-09-01T03:01:06Z
  tianpan-forum-db    first=2026-08-21T22:42:56Z  last=2026-09-01T03:01:37Z
  blockeden-forum-db  first=2026-08-21T22:42:21Z  last=2026-09-01T03:01:04Z
  ```

  Production binds `ClusterRole/bex-tenant-api` into this namespace. That role granted neither the CNPG Backup list nor the Barman ObjectStore get to `system:serviceaccount:bex-system:bex-api`; the earlier m117 change updated only the obsolete/default-namespace `Role/bex-api-apps`. The corrective worktree therefore reads the ObjectStore entry rather than an empty Cluster field and grants the exact read-only permissions in both namespace regimes.
