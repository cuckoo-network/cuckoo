...
**Worker:** worker6 **Goal:** `type=request` returns real rows for a service receiving HTTP traffic, and the Method / Status code / Request path filters that four surfaces advertise stop being guaranteed-empty — with the production check `w3/m8` left owed finally run. **Status:** root cause found, fixed (`4c66473b`), and **verified working in production 2026-08-28** by direct read-only probes of live Loki — `type=request` returns rows, `label=method`/`status` are non-empty, and the `host` and metrics filters narrow instead of emptying. **8 of 9 tasks done.** Only `t005`'s authenticated REST/GraphQL/MCP/dashboard sweep remains (no QA credentials in `.env`); `scripts/logs-verify.sh` itself was deliberately not run against prod, since it mutates the live cluster and covers only the `default` namespace the bug never broke.
