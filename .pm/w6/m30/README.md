...
**Worker:** worker6 **Goal:** Close the divergence `w6/m23` consciously shipped and `docs/ADR005-custom-domain.md:60` records: the auto-paired www↔apex sibling 3xx-redirects to the canonical host (the one the tenant explicitly added), matching Render — instead of being served identically. **Status:** todo

## Tasks (in order)

| id   | title                                                                                                       | est | depends_on |
| ---- | ------------------------------------------------------------------------------------------------------------- | --- | ---------- |
| t001 | Capture Render's exact sibling-redirect behavior live (status code, path/query preservation, cert on the redirecting host) → `docs/render-artifacts/` | 30m | —          |
| t002 | Operator: per-sibling-host redirect — Traefik redirect middleware + router on the sibling host (cert still issued), canonical = the tenant-added host | 60m | t001       |
| t003 | Surface it: domain views mark the sibling `redirect → <canonical>` on REST/GraphQL/MCP; dashboard add-dialog copy; replace ADR005's divergence paragraph | 45m | t002       |
| t004 | Render parity — behavior vs t001's capture, three-surface field consistency; refresh the ADR018 domains row     | 20m | t003       |
| t005 | Simplify — `/simplify` over the code this milestone changed                                                    | 20m | t004       |
| t006 | Test coverage — envtest: rendered middleware/router per pairing; redirect-vs-serve regression                   | 30m | t004       |
