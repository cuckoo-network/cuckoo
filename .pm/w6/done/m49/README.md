...
- **Source:** live `/qa-find-bugs` hunt of `https://dashboard.bex.co`, 2026-08-23, signed in as the QA user, workspace `tea-d98210cbbpdc73dcrkvg` — an adversarial-input testing angle (duplicate-name creation) that prior iterations of this hunt had not covered. Evidence is the raw GraphQL request/response pairs below, run from inside the authenticated page (`page.evaluate`/`fetch(..., {credentials:'include'})`), each reproducible against production; every `qa-20260823-*` resource created to obtain them was deleted immediately after and the Overview page verified back to its pre-hunt baseline.
  ...
  5. **Confirmed live 2026-08-23, not applicable** (t006): `dashboard/src/features/api-keys/hooks/use-create-api-key.ts:53-54` and `dashboard/src/features/registry-credentials/hooks/use-create-registry-credential.ts:52-53` share the identical bare `catch { toast.error(...) }` pattern, but neither resource type enforces name uniqueness — a duplicate-named API key and a duplicate-display-name registry credential (distinct hosts) both succeeded twice with no conflict. There is no conflict message for either hook to surface, so no fix applies. Full detail in `done/t006.md`'s Live probe result section.
...
- **Blast radius:** 10 `use-create-*.ts` hooks total. 2 already correct (services — full `w4/m19` treatment; workspaces — passthrough), both regression-test targets. 4 confirmed broken live (keyvalue, databases, projects — t002/t003/t004; environments confirmed 2026-08-23 — t005). 2 more share the identical bare-catch code shape but are confirmed live 2026-08-23 to be not-applicable (api-keys, registry-credentials — t006: neither type enforces name uniqueness, so there is no conflict message to swallow). 2 use a different, seemingly-adequate mechanism not part of this bug class and out of scope: blueprints' plain generic toast has no name-conflict concept at all (may or may not need one — not investigated); webhooks' `WebhookMutationError`/named-refusal system already renders inline for named refusals.
