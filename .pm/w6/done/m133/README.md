...
**Worker:** worker6 **Goal:** the parameter editor shows and replaces only what the tenant declared, and operator-owned settings — the WAL archive/restore commands, the TLS paths, the replication group — cannot be captured into tenant config. **Status:** done — closed 2026-08-29: every t008 live probe passed on a disposable production database (empty declared set, single-parameter round-trip, operator-key refusals, intact pg_settings insight, editor bound to the declared spec) and all four carried items are settled or explicitly re-carried; see done/t008.md
...
| t008 | Closeout — **DONE** | 10m | t004, t007 |
