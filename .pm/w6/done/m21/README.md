...
**Worker:** worker6 **Goal:** close a documented Render service-create field gap — explicit Dockerfile path + start-command override **Status:** done

## Tasks (in order)

| id   | title                                                                                          | est  | depends_on | status     |
| ---- | ----------------------------------------------------------------------------------------------- | ---- | ---------- | ---------- |
| t001 | `App.Spec.DockerfilePath`, `StartCommand` fields (types + deepcopy + CRD yaml regen)             | 45m  | —          | — **DONE** |
| t002 | operator/build.go: thread `DockerfilePath` into the BuildKit `--opt filename=` arg               | 1h   | t001       | — **DONE** |
| t003 | operator app_controller.go: thread `StartCommand` into the Deployment container `Command`/`Args` | 1h   | t001       | — **DONE** |
| t004 | backend/internal/apps: thread both fields through bex.yml ingestion + REST/GraphQL/MCP App create/update | 1.5h | t002, t003 | — **DONE** |
| t005 | Render parity: verify field shape/semantics consistent across REST/GraphQL/MCP + dashboard UI    | 45m  | t004       | — **DONE** |
