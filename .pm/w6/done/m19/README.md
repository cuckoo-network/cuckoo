...
**Worker:** worker6 **Goal:** bring Environments (`w1/m32`) to Render parity on `protectedStatus`/`networkIsolationEnabled`/`ipAllowList` **Status:** done

## Tasks (in order)

| id   | title                                                                                                | est | depends_on | |
| ---- | ----------------------------------------------------------------------------------------------------- | --- | ---------- |---|
| t001 | Store: add `protectedStatus`/`networkIsolationEnabled`/`ipAllowList` columns + migration + `Environment` struct fields | 45m | —          | **DONE** |
| t002 | REST/GraphQL/MCP verbs to get/set the three fields on an Environment                                   | 1h  | t001       | **DONE** |
| t003 | New `core.LabelEnvironment` CR label + writer path projecting environment membership onto App CRs (`environments/service.go`) | 1h  | t001       | **DONE** |
| t004 | Operator: environment-scoped `NetworkPolicy` variant in `reconcileNetworkPolicy` (`app_controller.go`), active when `networkIsolationEnabled` | 1.5h | t003      | **DONE** |
| t005 | Enforce `protectedStatus: protected` — block delete/suspend/direct-deploy-override verbs on member Apps without explicit confirmation | 1h  | t002       | **DONE** |
| t006 | Propagate environment `ipAllowList` to member Database/KeyValue resources' existing per-resource allowlist mechanism | 1h  | t002       | **DONE** |
| t007 | Render parity: verify field shape/semantics/errors consistent across REST/GraphQL/MCP + dashboard UI  | 45m | t006       | **DONE** |
| t008 | Simplify: run `/simplify` over the code this milestone changed                                         | 30m | t007       | **DONE** |
| t009 | Test coverage: enforcement tests for protection/isolation/allowlist, not just storage round-trip       | 1h  | t007       | **DONE** |
