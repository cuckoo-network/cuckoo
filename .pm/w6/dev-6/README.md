# dev-6 — workstream w6's isolated local dev environment

A dedicated Kratos + Hydra + Mailpit stack (namespace `dev-6-auth`) plus a
dedicated app namespace (`dev-6`), layered onto the **shared** local
...
`dev-6` sidesteps this by giving workstream w6 its own Kratos/Hydra with
...
| bex-db (port-forward) | `55060` |
| bex-api control-plane API (BEX_CP_ADDR, host-only) | `56060` |
| kratos-admin (port-forward, BEX_KRATOS_ADMIN_URL) | `57060` |
| hydra-public / issuer (port-forward, BEX_OAUTH_ISSUER) | `58060` |
| loki (port-forward, BEX_LOKI_URL) | `59060` |
| mailpit SMTP (port-forward, BEX_SMTP_ADDR) | `60060` |
| OpenSandbox lifecycle server (`agent-up`) | `61060` |
| ssh-gateway agent attach SSE (`agent-up`) | `62060` |
| OpenFGA (`agent-up`) | `63060` |
| OpenBao (`agent-up`) | `64060` |
| ssh-gateway sandbox-exec (`agent-up`) | `65060` |

## Start / status / stop

```sh
bash scripts/dev-env.sh 6 up      # idempotent: creates namespaces, CNPG Clusters,
                               # mailpit, kratos+hydra (helm), port-forwards,
                               # and a local bex-api
bash scripts/dev-env.sh 6 status  # pod status + http health checks
bash scripts/dev-env.sh 6 down    # kills local processes, deletes both namespaces
```

Then start the dashboard separately (kept out of `dev-env.sh 6 up` so it stays in the
foreground / under your own process control — `dev-env.sh 6 up` prints the exact
command):

```sh
cd dashboard && VITE_API_URL=http://localhost:54060/graphql \
  VITE_SSR_API_URL=http://localhost:54060/graphql \
  VITE_KRATOS_PUBLIC_URL=http://localhost:51060 yarn dev --port 50060
```

## Recovering after a restart

`dev-env.sh 6 up` is idempotent — re-running it after a reboot (or after the shared
...
KeyValue/App/Database CRs in the `dev-6` namespace survive as long as the
underlying `bex` cluster (and its PVs) survive.

## Tenant-namespace isolation between concurrent dev-N stacks

`dev-env.sh 6 up` sets **`BEX_CP_IDENTITY=dev-6`**, so every tenant namespace this
control plane provisions is stamped `app.bex.co/control-plane: dev-6` and its
...
`dev-env.sh 6 status` prints every managed tenant namespace with its owner — if a row
shows an owner other than `dev-6` it belongs to another harness and this one
will leave it alone; if *this* harness's rows show anything but `dev-6`, its
...
none of that machinery. `bash scripts/dev-env.sh 6 agent-up` layers it on:
...
| **OpenFGA** (`dev-6-auth`) | `cmd/ssh-gateway` calls `requiredEnv("BEX_OPENFGA_URL")` and has no allow-all hatch; bex-api must share the store or every attach is denied |
| **OpenBao** (`dev-6-auth`, dev mode) | the BYO model key (ADR062) the gateway mints per exchange |
...
stamped `app.bex.co/dev-env: dev-6` and refuses to take it from another
...
bash scripts/dev-env.sh 6 agent-netpol <workspaceId>
...
`bash scripts/dev-env.sh 6 agent-stub` stands up a local Anthropic Messages API
...
The dashboard's `/agents` conversation view will not load in dev-6 on the
...
```sh
cd dashboard && HYDRA_ADMIN_URL=http://localhost:52060 \
  HYDRA_PUBLIC_URL=http://localhost:58060 \
  VITE_API_URL=http://localhost:54060/graphql \
  VITE_SSR_API_URL=http://localhost:54060/graphql \
  VITE_KRATOS_PUBLIC_URL=http://localhost:51060 \
  VITE_KRATOS_SSR_URL=http://localhost:51060 \
  VITE_AGENT_STREAM_URL=http://localhost:62060 \
  yarn dev --port 50060
```
...
```
Access to fetch at 'http://localhost:54060/v1/agent-sessions/<id>/stream'
...
Pointed at `62060` the request reaches the gateway's attach listener directly,
which does allow the ticket header and echoes CORS for the dev-6 dashboard
...
- **The bex operator** — already cluster-scoped in the shared cluster,
  reconciles CRs in every namespace including `dev-6`. No need for a second
...
`values/`, `rbac-dev-6.yaml`) used to live in this directory, copied ten times.
They are now one implementation — **`scripts/dev-env.sh 6 {up|down|status|clean|env}`**
...
to keep before re-running. `scripts/dev-env.sh 6 clean` reclaims `logs/` and
