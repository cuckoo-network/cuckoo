...
```sh
bash .pm/w6/dev-6/up.sh      # idempotent: creates namespaces, CNPG Clusters,
                               # mailpit, kratos+hydra (helm), port-forwards,
                               # and a local bex-api
bash .pm/w6/dev-6/status.sh  # pod status + http health checks
bash .pm/w6/dev-6/down.sh    # kills local processes, deletes both namespaces
```
...
`up.sh` sets **`BEX_CP_IDENTITY=dev-6`**, so every tenant namespace this
