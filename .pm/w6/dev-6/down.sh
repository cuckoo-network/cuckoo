#!/usr/bin/env bash
# Tear down the dev-6 isolated environment (workstream w6): kills the local
# processes up.sh forked (port-forwards, bex-api) and deletes the
# dev-6-auth / dev-6 namespaces from the shared cluster. Leaves the shared
# cluster itself (and any other workstream's resources) untouched.
#
# Usage: bash .pm/w6/dev-6/down.sh
set -euo pipefail
...
echo "dev-6 down. Namespace deletion continues in the background (kubectl get ns to watch)."
