...
**Worker:** worker6 **Goal:** only services with an observable activity signal and wake path auto-sleep; the dashboard promises wake-on-request only for those services **Status:** todo (implementation complete locally; deployed closeout pending)
