...
- **Narrowed by the 51st run (2026-08-27), recorded in `t001`:** a simultaneous paired probe measured `service_hibernated`/`service_woken` for both auto-sleep-eligible types in one window with neither URL requested — **private: 1 hibernation, 0 wakes; web: 4 and 4**. Only the web service flaps, and exactly one path differs between them on the park route: `holdHibernateForRouting` (`app_controller.go:1922-1934`), the only code that raises a parked App's replica count back to 1, which `ingressRoutesToActivator` (`:1944-1956`) documents as skipped for an App with no Ingress. Also ruled out: `last-active` is not being cleared (`runningRequeue:2094-2110` re-stamps only when zero; `hibernated():2120-2135` writes `Status` only). `t001` now starts at the hold and asks why it appears to re-enter.
- **Unverified this run:** (1) the mechanism is narrowed but not confirmed — the Ingress object was never observed across a cycle, so "the hold re-enters" remains a prediction. (2) `w6/m119/t009` answered the private-service question: it does **not** flap, which is what makes the contrast diagnostic. (3) Whether the cycle depends on the short 60s TTL used here or also occurs at the dashboard's shortest preset (300s) — the fixture used an API-only value, and a longer TTL may simply lengthen the period or may not reproduce at all. (4) Whether the false `service_woken` events actually reach webhook subscribers — no webhook was subscribed during this run.
