...
**Worker:** worker6 **Goal:** a tenant whose build fails reads a sentence that names the cause, over a duration that matches reality, with the failing phase's own error in the log **Status:** todo

## Tasks (in order)

| id   | title                                                                                     | est | depends_on |
| ---- | ----------------------------------------------------------------------------------------- | --- | ---------- |
| t001 | Stop shipping the raw Job message; use the failure tail the build already captures           | 60m | —          |
| t002 | Stop stamping `started_at` from a terminal-skip transition, and carry the real duration      | 60m | —          |
