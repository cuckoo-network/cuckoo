# dev-beta — an isolated local dev environment

A dedicated Kratos + Hydra + Mailpit stack (namespace `dev-beta-auth`) plus a
dedicated app namespace (`dev-beta`), layered onto the **shared** local
...
dev-beta sidesteps this by giving itself its own Kratos/Hydra with its own
...
| --- | --- |
| dashboard | `5373` |
| kratos-public (port-forward) | `25433` |
| hydra-admin (port-forward) | `25445` |
| mailpit UI (port-forward) | `25825` |
| bex-api (local process) | `28095` |

Chosen to avoid both the standard `5173`/`4433`/`4445`/`8090` local-dev ports
and `env/dev-alpha`'s own `5273`/`15433`/`15445`/`15825`/`18095`/`25432` — a
distinct namespace pair (`dev-beta`/`dev-beta-auth`) and port range, so
dev-beta can run alongside the shared cluster's dashboard *and* a concurrent
dev-alpha, all at once.

## Start / status / stop

```sh
bash env/dev-beta/up.sh      # idempotent: creates namespaces, CNPG Clusters,
                               # mailpit, kratos+hydra (helm), port-forwards,
                               # and a local bex-api
bash env/dev-beta/status.sh  # pod status + http health checks
bash env/dev-beta/down.sh    # kills local processes, deletes both namespaces
```
...
```sh
cd dashboard && VITE_API_URL=http://localhost:28095/graphql \
  VITE_KRATOS_PUBLIC_URL=http://localhost:25433 yarn dev --port 5373
```
...
KeyValue/App/Database CRs in the `dev-beta` namespace survive as long as the
...
- **The bex operator** — already cluster-scoped in the shared cluster,
  reconciles CRs in every namespace including `dev-beta`. No need for a
  second instance.
- **OpenFGA** — left unset for dev-beta's local bex-api, same as the shared
