#!/usr/bin/env bash
# Quick health check for the dev-beta isolated environment.
# Usage: bash env/dev-beta/status.sh
set -euo pipefail
...
if [ -f "$ENVDIR/.kubeconfig" ]; then
  ...
  echo
  echo "== $DEV_BETA_AUTH_NS pods =="
  kubectl -n "$DEV_BETA_AUTH_NS" get pods 2>&1 || true
  echo
  echo "== $DEV_BETA_NS resources =="
  kubectl -n "$DEV_BETA_NS" get apps.app.bex.co,keyvalues.app.bex.co,databases.app.bex.co 2>&1 || true
fi
