...
- **Publish** (`internal/publish`) is the "different output sink": an in-cluster Job whose init container stages the site into a shared volume — from the built image (`extract`: copies `publishPath` out of it) or, on the no-build path, from a shallow git checkout (`clone`: `alpine/git`, same `CloneSecret` token contract as the build plane for private repos) — and whose `aws-cli` container `s3 sync`s it to `s3://<bucket>/<app-id>/<revision>/` for unlabeled Apps, or `s3://<bucket>/<tea-id>/<app-id>/<revision>/` when the App carries `app.bex.co/workspace` ([ADR074](ADR074-workspace-scoped-artifact-identity.md)). `App.status.staticPrefix` records which prefix the current revision actually occupies so the static-server does not probe S3. Revision-prefixed keys give **atomic cutover and rollback** — the App's active revision just repoints the prefix.
- **Serve** (`internal/staticserver`, the `staticserver` binary) is one always-on proxy for **all** static sites. It watches `static_site` Apps for the host→revision mapping and their edge rules, resolves each request's `Host` to a site, does **signed** S3 GETs (the bucket stays private), maps `/` → `index.html` (+ SPA fallback), applies the routes/headers, and caches objects — and confirmed misses (w4/m94) — in memory (a revision prefix is immutable, so neither a hit nor a 404 is ever stale within it). The operator points each static-site host's Ingress at the static-server's Service — the same by-name Ingress backend the wake activator uses.

**Suspend/resume** (w3/m46): a suspended static site keeps its host Ingress and TLS certificate pointed at the static-server, exactly as when running — the resolver drops a suspended App from its host→site map, so the static-server answers its ordinary `no static site for host` 404, now served over the App's own **managed** certificate. This mirrors a suspended compute service, whose Ingress and cert are likewise retained (`docs/ADR007`), so the "URL and certificates are kept" contract holds for both. (Before w3/m46 the operator removed the Ingress on suspend, leaving Traefik with no route or cert for the host — visitors hit a default self-signed cert and a bare 404.) The published content stays in the object store; resume just re-adds the App to the resolver.
...
Static hosting has three independent security domains:
...
## Deletion and finalizer bound (w3/m81)

Deleting a static site removes its host Ingress/TLS and drops it from the static-server's resolver at once (the URL stops serving within seconds), but the published objects must still be purged from the shared `bex-static` bucket. The operator's `app.bex.co/finalizer` runs that purge as a persisted, restart-adoptable cleanup Job (`publish.PurgeJob`) that `aws s3 rm --recursive`s each of the App's revision prefixes; the Job carries a **15-minute `activeDeadlineSeconds` and `BackoffLimit` 3**, and its terminal completion is recorded on the App (`app.bex.co/static-purge-complete`) before the Job and its Pods are removed, so a manager restart re-adopts rather than re-runs it. The finalizer is removed — and the CR finally deleted — only after the purge, the cross-namespace execution/artifact reclaim, and registry-credential revocation all report done.

**Overrun is bounded and observable, never hidden.** Before w3/m81 the finalizer requeued every 2 seconds indefinitely with no status, so a purge that could not converge (a missing publish Secret, a Job wedged on the `bex-build-job-shape` admission policy, an S3 outage) left the App `Terminating` forever with nothing on any surface — the exact 2+-hour hang (`srv-da7tf87krsvc73c3mcng`) this milestone was promoted to close. Now, if finalization has not completed within 15 minutes of the deletion request, the operator stamps a `DeletionStalled=True` condition on the App (`Reason: CleanupExceededDeadline`, message naming the blocked step) and backs its requeue off to 30 seconds. That condition is the actionable **operator** signal (`kubectl get app -o yaml` / `kubectl describe app`); the **tenant** signal is the aggregate `terminating` count in workspace-limits (w6/m129), because by-id reads are already `not found` ([ADR006 § Reads while a deletion finalizes](ADR006-bex-api.md#reads-while-a-deletion-finalizes-w3m81)). The finalizer is deliberately **retained** on overrun so no object prefix is silently orphaned behind `scripts/delete-audit.sh` (the residue gate that proves prefixes absent); a genuinely unrecoverable purge is an operator action, not a silent finalizer drop. The 15-minute threshold is injectable (`AppReconciler.FinalizerOverrunAfter`) so the failure path is regression-tested without multi-minute sleeps.
...
- **Edge rules** — `GET`/`PUT /v1/services/{id}/routes` and `.../headers` (bulk replace, Render-compatible). GraphQL `setStaticRoutes`/`setStaticHeaders`/ `setPublishPath`; MCP `list_static_routes`/`update_static_routes`/ `list_static_headers`/`update_static_headers`/`update_service(publishPath:)` (bex makes functional what Render's official MCP ships only as a stub).

A route is `{type: redirect|rewrite, source, destination}`; a header is `{path, name, value}` — Render-identical. A redirect answers 301; a rewrite serves another path with 200. Source patterns support a trailing `/*` wildcard; the capture substitutes a `:splat` token or trailing `/*` in the destination. An explicit SPA fallback is a rewrite of `/*` → `/index.html`.

**An existing published object wins over every rule** (w4/m94, matching [Render's documented order](https://render.com/docs/redirects-rewrites)): the server looks the normalized request path up first and serves a hit with its own bytes, content type, and cache policy — so the canonical `/* → /index.html` SPA rewrite never masks a real file like `/render.yaml` or a built JS/CSS asset. Only a genuine object miss advances to the ordered rules (first match wins) and then the implicit fallback below; an origin permission failure, timeout, oversize object, or capacity shed keeps its own error class (502/413/503) rather than degrading into a rule. **Custom headers match the visitor's request path**, not the rewrite destination ([Render's header-path contract](https://render.com/docs/static-site-headers)): a `/qa-route`-scoped header still applies when a wildcard rewrite serves `/index.html` for it, and an `/index.html`-scoped header does not leak onto rewritten requests; content type and cache defaults stay tied to the served object.

### Default SPA fallback for extension-less misses (deliberate Render divergence)

Even with **no** `routes` configured, the static-server serves the site's root `index.html` (200) for a miss on an **extension-less** path (e.g. `/dashboard`, `/app/settings`) when a root `index.html` exists — a miss on an **asset-like** path (one with a file extension, e.g. `/main.js`) stays a **404** (`staticserver.go` `spaFallback`). This is an intentional, SPA-friendly default: single-page apps resolve their client-side routes out of the box without the owner having to add the `/* → /index.html` rewrite first.

It is a **documented, deliberate divergence from Render**, which returns 404 for any unmatched path unless the site ships a `404.html` or configures a rewrite. The decision (w3/m46) is to **keep the friendlier default**: it never hides a real asset (extension-bearing misses still 404), and a Render user who adds the canonical `/* → /index.html` rewrite lands on identical behavior, so the two platforms converge for any SPA that configured it on Render. Content-oriented (non-SPA) sites that want hard 404s for unknown paths are the group this default trades against; a per-site opt-out (honor a `404.html`, or a "no implicit fallback" flag) is a recorded future option, not built. The parity ledger ([ADR018](ADR018-render-parity.md)) records this under the static-site rows as an accepted divergence.
