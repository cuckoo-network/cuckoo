...
- **`highest_available` makes enrollment bite, and only then.** `session.whoami.required_aal` and the settings flow's `required_aal` are both `highest_available`: a session counts as fully authenticated only at the highest AAL the identity _can_ reach. A password-only identity is satisfied at aal1 (its login is completely unchanged); the moment an identity enrolls any second factor, `highest_available` becomes aal2, so `/sessions/whoami` 403s (`session_aal2_required`) until the second factor is presented, and changing security settings itself requires aal2.
...
- **The dashboard renders it with zero bespoke form code.** Settings' `<Settings>` and login's `<Login>` (Ory Elements) already track whatever methods Kratos enables, so the enroll/unenroll TOTP · security-key · recovery-code sections and the second-factor challenge appear automatically. The one wiring addition is the **aal2 step-up** in [use-ory-flow.ts](../dashboard/src/common/hooks/use-ory-flow.ts): when a user with a second factor lands on the login page holding an aal1 session (the auth guard bounced them there because `whoami` 403'd), Kratos rejects a first-factor flow with `session_already_available`; the hook then mints an `aal=aal2` login flow, and — only if that flow actually carries a second-factor node — renders the challenge, otherwise navigates on (a fully-aal2 or no-second-factor session has nothing to prove).

**Verified** by [scripts/auth-mfa-e2e.sh](../scripts/auth-mfa-e2e.sh) (exit 0 = pass): register → enroll TOTP from the settings flow (secret read from the flow, code computed from it) → step up to aal2 → enroll recovery codes → assert password-only login is aal1/`whoami` 403 → a wrong TOTP code is rejected while the right one upgrades to aal2 → a `lookup_secret` recovery code logs in "without the device" and is rejected on reuse (single-use). It runs against the mock cluster's `kratos-public` by default, or set `KRATOS_PUBLIC_URL=https://auth.bex.co` for the prod smoke. The WebAuthn ceremony is the one remaining manual browser check.
...
- **Since w4/m10, the same seam doubles as the audit interception point**: `Base.Authorize`/`AuthorizeOn` records one event (caller/verb/resource/outcome) for every `can_operate`/`can_create`/`can_manage_keys`/`can_manage` call — success and denial alike — while `can_view`/`can_view_logs`/`can_view_sensitive` stay unrecorded (volume). A cross-tenant `AuthorizeOn` denial (e.g. a non-admin naming another workspace to `workspaces.Delete`) is exactly the "who tried what" case the audit log exists for. See [bex-api.md § Audit log](bex-api.md#audit-log-w4m10).
