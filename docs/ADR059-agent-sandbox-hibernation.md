...
Converge the workspace's **mutable state** onto a known mount (`/workspace` + the HOME dirs that matter — working tree, `~/.zed_server`, tool/dep caches) and snapshot **that**, not the whole rootfs. **Continuity amendment (w5/m84, 2026-08-30):** "the HOME dirs that matter" explicitly includes each reviewed agent profile's **session-state directories** (e.g. `claude-code-acp`'s on-disk conversation state under HOME) — they are the substrate ADR047 D3's `session/load` resume rung depends on. A snapshot that omits them silently downgrades every resume to transcript re-priming (ladder rung 2), which is a fidelity loss, not an equivalent. Store it in **object storage** (S3-compatible, per-workspace prefix, **ADR050-encrypted**), the way Gitpod and Cloudflare do — object storage scales to an unbounded number of small per-instance blobs and imposes no manifest/tag/ACL cardinality. **Not** a general OCI registry (per-instance commits explode Zot's manifest/tag/ACL model; Modal-style image-diffs need a bespoke store Zot isn't).
...
**Agent-context amendment (w5/m84, 2026-08-30).** The bug that motivates this: a user resumed hibernated session `ags-da9mh5vj596c73en5eq0` and sent "try again". The dashboard showed the whole earlier conversation (replayed from the database transcript), but the agent answered _"I need more context — what should I try again?"_ — because restoring a snapshot only restores **files**. The agent's memory of the conversation lives in its process and its own state files, and nothing brought either back. The UI looked continuous; the agent was blank.

So the rule is now explicit: **whenever a session continues in a new pod, the driver must give the new agent its history back before delivering the user's new message.** It tries three methods, best first:

1. **Let the agent reload its own memory.** If the snapshot contains the agent's saved conversation state (see the D3 amendment above — those files are now part of the snapshot) and the agent supports ACP `session/load`, restore the files and ask the agent to load the old session. Full fidelity.
2. **Rebuild context from the transcript.** If that state is missing or the agent can't load it, the driver prepends a bounded summary of the prior conversation (user prompts + assistant replies, pulled from the durable transcript in the control-plane DB) to the new prompt. The agent doesn't literally remember, but it knows what happened.
3. **Re-send the original task.** If the agent _never actually ran_ — every earlier turn died during setup (this session's case: both turns failed at `git clone`, so the snapshot was 109 bytes of empty workspace and there is no conversation to restore) — then prepend `agent_config.task` to the new message. "Try again" must mean "retry the task", not a question to a blank agent.

Two clarifications this incident forced: an **empty snapshot is normal** for a session whose turns all failed during setup — resume must expect it and use method 3, not treat it as restorable state. And this applies to **more than hibernation**: a Steer that dispatches a fresh sandbox has exactly the same problem (new pod, blank agent), so it follows the same three steps. Full details of the three methods: ADR047 D3's continuity amendment.
