...
**Agent-context amendment (w5/m84, 2026-08-30 — `ags-da9mh5vj596c73en5eq0`).** "Files intact, process cold-restarts" governs the **workspace** only; it must not mean the agent forgets the conversation. A fresh-pod resume/steer primes the new agent process per ADR047 D3's continuity ladder (`session/load` from snapshot-carried agent state → bounded transcript re-priming → `agent_config.task` re-delivery). Two legal-but-previously-unhandled shapes are named: a hibernated session whose every turn failed during **setup** has an empty snapshot by construction and its resume MUST take the task-re-delivery rung; and non-hibernated **redispatch** (steer onto a fresh sandbox) crosses the same generation boundary and takes the same ladder — hibernation is one trigger of the amnesia, not its definition.
