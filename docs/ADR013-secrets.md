...
**Status:** accepted; substrate deployed on the local mock cluster (w4/m5), wired into the product in w4/m6 (bex-api's env-vars API reads/writes tenant credentials through it — see [Product usage](#product-usage-w4m6-the-env-vars-api)), wired into prod in w1/m10 (init/unseal + Kubernetes auth run from `deploy.yml`, `replicas: 3` overlay — see [Prod deploy path](#prod-deploy-path)), and extended in w11/m8 with an optional KV-v2 compare-and-set capability for one-variable mobile edits.
...
The product's base `SecretKV` seam keeps its original unversioned read/write/delete/list contract. A separate optional `VersionedSecretKV` capability returns one atomic data+version snapshot and supports a full-map KV-v2 write with `options.cas=<observed-version>`. This is opt-in per request: the mount does **not** enable `cas_required`, which would break legacy env-var, secret-file, and env-group writers that deliberately retain their existing semantics. CAS 0 is create-only; a stale version is a conflict, never an unconditional overwrite.
...
flowchart LR
  ...
  api -->|"ordinary KV v2 write or optional revision CAS<br/>(source of truth)"| bao["OpenBao<br/>tenants/&lt;tenant&gt;/services/&lt;svc&gt;/env"]
...
- **Revision-aware one-key update (w11/m8).** The sensitive GraphQL `service.envVarKeys` list remains values-free and now attaches the same opaque whole-environment revision to every key; the explicit `service.envVar(key:)` reveal returns the revision of the coherent map it read. Sensitive REST/MCP list/get responses expose the same additive `revision`, and their coherent environment patch accepts the same optional `expectedEnvRevision`, so adapter semantics and coded errors remain aligned. The revision is a canonical `evr1_…` encoding of OpenBao metadata, not a credential or authorization bypass, and clients must treat it as an opaque echo-only token. Revision mode updates exactly one existing ordinary key/value assignment: it cannot create a missing key and accepts no secret files, rename, delete, or generated value. The server reads one versioned snapshot and submits the completed map with KV-v2 CAS. A missing key returns `ENVIRONMENT_VARIABLE_NOT_FOUND`, an invalid key returns `ENVIRONMENT_VARIABLE_INVALID`, and a stale observation returns `ENVIRONMENT_REVISION_CONFLICT`, all before any source or Kubernetes projection write. Even a same-value CAS advances the revision so two submissions from one observation resolve to one success and one conflict; because effective data did not change, that case does not roll pods.
- **CAS compensation.** A revision-aware projection annotates `<service>-env` with the opaque source revision that owns it. If the source CAS succeeds but projection or App patching fails, compensation restores the prior OpenBao map with a second CAS against the failed attempt's version, then conditionally restores or deletes the derived Secret only while that failed revision still owns its exact Kubernetes UID/resourceVersion. A fully restored attempt returns `ENVIRONMENT_UPDATE_RESTORED`; a newer source or projection owner returns `ENVIRONMENT_REVISION_CONFLICT` without being overwritten; an incomplete rollback returns `ENVIRONMENT_RESTORATION_FAILED`. Those stable outcomes remain visible through REST `code`, GraphQL `extensions.code`, and MCP error text. Revisions, keys, values, OpenBao URLs/paths, response bodies, Kubernetes object names, and CAS versions never enter CAS-conflict or version-store error text. The revision capability is additive; a non-versioned `SecretKV` continues serving legacy behavior, while a revision-bearing mutation fails closed when that optional capability is absent.
