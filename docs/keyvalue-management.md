...
**Status:** accepted and implemented at the mechanism layer — the `KeyValue` CRD (`lego/types/v1alpha1/keyvalue_types.go`), the Valkey-projecting controller (`lego/operator/internal/controller/keyvalue_controller.go`), and the `valkey` tier family (`lego/types/tiers/tiers.yaml`) all ship. It is the direct sibling of managed Postgres ([docs/postgresql-management.md](postgresql-management.md)) — same shape, one engine down. The Render-compatible REST/GraphQL/MCP/dashboard surface is deliberately deferred to w2/w5; today a tenant provisions a store via the `KeyValue` CR directly.
...
- The REST/GraphQL/MCP/dashboard surface (`list_key_value` / `get_key_value` / `create_key_value` + a dashboard type) is deferred to w2/w5 — the mechanism ships first so those adapters have something to call.
