...
**Free web tier = sleep-when-idle (decided).** Idle free web services **hibernate** (`sleep = free`) and **wake on the next public request** via the gateway **activator** (Knative-style); a sleeping pod occupies nothing, so the cluster **overcommits well beyond Σ** and Free approaches \$0. So the `Σ(running pods)` above is really `Σ(paid pods + currently-awake free web pods + private/worker pods)` — sleeping free web services don't count. **Private services and background workers stay running even on Free:** neither has a public Ingress wake path, and private in-cluster traffic does not traverse the activator that records activity. Sleeping either would make ordinary traffic invisible and could park it permanently. This deliberately spends the pod until an internal traffic observer and wake path exist. **Paid tiers stay reserved** (always-on, `request=limit`). The price of web auto-sleep is a **cold-start** on wake (hold/queue the first request while the pod resumes) and an **idle-detector + activator in the request path**. This is the same `sleep = free` loop bex already runs for the OpenSandbox runtime (idle → pause, request → resume) — see [`ADR002-architecture.md`](ADR002-architecture.md). **The default makes this real (w6/m116):** `spec.idleTTLSeconds: 0` (the value a free service is created with) means **the platform default idle window — 15 minutes**, not "never"; the operator resolves 0 to that window, so a free web service sleeps by default with no per-service configuration. Before w6/m116 the operator read 0 as "disabled", so no free service created the normal way ever hibernated and this overcommit assumption did not hold in practice. A free user may set a different positive window but cannot switch auto-sleep off entirely — matching Render's always-on free spin-down.
