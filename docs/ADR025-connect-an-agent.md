...
bex-api serves a Render-compatible MCP server at `/mcp` (streamable-HTTP), the same one-core-three-adapters surface as REST/GraphQL ([ADR006-bex-api.md](ADR006-bex-api.md)). Its POST transport is replica-independent: a load balancer may send every request to a different bex-api pod, while selected-workspace state remains shared and identity-scoped in the control-plane store. No session-affinity configuration is required. w4/m9 added the OAuth 2.1 provider side ([ADR012-auth.md §7](ADR012-auth.md#7-oauth-21-provider-for-agents-one-dashboard-login-first-party-sessions--third-party-clients-w4m9)) so a real MCP client — Claude Code, Cursor, or your own agent — can hold its own revocable, user-consented credential instead of a shared secret. This page is the connect recipe: what the discovery/token dance looks like, config snippets for both clients, the API-key path for headless use, and what to check when it doesn't come up clean.
...
- **`404 session not found` after a successful initialize** — this identifies a pre-w2/m58 bex-api replica still serving the old stateful transport. Current releases accept the session id on any replica and share selected-workspace state through Postgres; upgrade every pod in the rollout rather than adding sticky routing.
...
- **400 `invalid_scope: … must also request at least one of bex.read, bex.write, bex.sensitive` at consent (w8/m27)** — your client asked for the API **audience** without a granular **capability**. Identity-only scopes (`openid`, `offline_access`) never confer API authority, and the legacy `bex.api` alias is platform-client compatibility only — a third-party client cannot use it as an umbrella grant. Fix client-side: request at least `scope=openid offline_access bex.read` (add `bex.write` / `bex.sensitive` only if the agent must mutate or read secrets). The three capabilities are advertised in the [RFC 9728 metadata](https://api.bex.co/.well-known/oauth-protected-resource) `scopes_supported`, which the MCP authorization spec tells discovery-driven clients to honor.
- **401 on `/mcp` after a previously working connect (Connected Agents still listed) (w8/m27)** — the token was minted under the round-14 `bex.api` / identity-only regime. Those grants introspect inactive immediately on deploy; listed Connected Agents entries do not self-heal. Re-run the client's connect/login so it re-consents for `bex.read` (and `bex.write` / `bex.sensitive` if needed). Official CLI and bex-mobile (platform-marked) keep working.
