...
- **400 `invalid_scope: … must also request the "bex.api" scope` at consent (round-14 #1)** — your client asked for the API **audience** without the API **scope**. A token that calls this API is a full control-plane credential, so it can't be minted behind a consent screen that only shows identity scopes. Fix client-side: request `scope=openid offline_access bex.api` (the `bex.api` value is advertised in the [RFC 9728 metadata](https://api.bex.co/.well-known/oauth-protected-resource) `scopes_supported`, which the MCP authorization spec tells discovery-driven clients to honor).
