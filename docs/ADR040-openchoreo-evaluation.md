...
| Build idempotence and lifetime safety | Deterministic Job/Image names, current App UID ownership, safe legacy adoption, newest-wins cancellation, and old-build rejection. | WorkflowRun has its own lifecycle/finalizer and immutable releases link outputs to deployments. **Comparable patterns.** | Make cancellation and ownership invariant across a future queue or external workflow engine. |
