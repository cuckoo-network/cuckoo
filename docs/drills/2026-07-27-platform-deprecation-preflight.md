...
- Commits `68fbd13c` and `22af231c` upgraded or replaced all 11 Node 20-era action refs. The final inventory is 36 direct invocations across 14 reviewed external refs; all JavaScript actions use maintained Node 24 majors, while unchanged composite actions were inspected through their nested dependencies.
- The initial 11-workflow matrix passed infra, operator, application-cluster, lint, backend, deploy, dashboard, docs, GitOps, Cluster API, and script checks. Complete log scans found zero Node 20 runtime warning or insecure-runtime override.
- The initial deploy's only annotation exposed a pre-existing functional gap: `gitleaks/gitleaks-action` requires a separate license for organization repositories and had exited before scanning. The corrected deploy downloads official Gitleaks CLI v8.30.1 by a fixed SHA-256, scans the exact pushed commit range, and preserves warn-first findings without permitting setup failure.
- [Corrected deployment run 30357908234](https://github.com/bex-co/bex/actions/runs/30357908234) scanned one commit with no leaks, passed all test, signing/SBOM, CVE, image-publish, and production-rollout steps, and ended with seven successful checks, zero annotations, and zero Node 20 warnings. [Scripts run 30357908396](https://github.com/bex-co/bex/actions/runs/30357908396) and [docs run 30357906806](https://github.com/bex-co/bex/actions/runs/30357906806) also passed.
- Production after rollout had bex-controller-manager 1/1, bex-api 2/2, dashboard 1/1, and both datastore SNI proxy DaemonSets 3/3 Ready. Workflow permissions were unchanged.

### Final regression and absence gates

- `scripts/platform-deprecations-validate.sh` now fails closed on active native Barman fields, Kubernetes 1.31 provisioning pins, the kpack override, old datastore Traefik types/helpers, removed backfill symbols, and the six retired normalization scripts. Scripts CI runs this guard for every operator, backend, GitOps, infra, workflow, or script change; GitOps validation invokes the same source/config check before rendering.
- Cluster API validation requires every control-plane/worker version and the snapshot default to match the semantic Packer pin and rejects Kubernetes 1.31 or older. GitHub Actions validation freezes the reviewed Node 24-compatible action inventory and the checksum-pinned, actually executed Gitleaks CLI scan.
- Full local operator, backend, dashboard, build, lint, GitOps, script, Cluster API, Action, and clean-init Terraform checks passed. The new [scripts](https://github.com/bex-co/bex/actions/runs/30360706432), [Cluster API](https://github.com/bex-co/bex/actions/runs/30360706473), and [GitOps](https://github.com/bex-co/bex/actions/runs/30360706390) runs are green; [deployment 30359262323](https://github.com/bex-co/bex/actions/runs/30359262323) passed the test, build, supply-chain, CVE, publish, and production-rollout path.
- Restore behavior is not inferred from manifests: [the separate plugin PITR record](2026-07-28-barman-plugin-pitr.md) proves known-row recovery and post-target exclusion for both a tenant database and bex-db, while both production sources stayed healthy and throwaway resources were removed.

### Closeout inventory — 2026-07-28 UTC

- The production application cluster had 9/9 Ready Nodes, all on Kubernetes v1.34.9. Barman Cloud plugin v0.13.0 was 1/1 Ready; four CNPG Clusters contained zero native Barman fields, bex-db declared the sole WAL-archiver plugin, and the sole ScheduledBackup used `method: plugin`. bex-db was 2/2 Ready with `ContinuousArchiving=True`.
- Production had five Apps, four with active revisions, and zero active missing artifact or release fingerprints. All 15 retained execution artifacts carried an App UID; missing-UID kpack artifacts and old `bex-system` clone copies were zero. Production and maintained-local datastore legacy-name/allowlist counts were zero.
- Production and maintained local each had exactly one current TCP object, `IngressRouteTCP/bex-system/bex-ssh-gateway`, and zero legacy datastore routes. The production edge had zero explicit server targets, exactly one private canonical worker selector resolving six workers, listeners 22/80/443/5432/6379, and no listener lacking a healthy backend.
- Final workloads were bex-controller-manager 1/1, bex-api 2/2, dashboard 1/1, and both datastore SNI proxy DaemonSets 3/3. The final absence-guard checks were 3/3 successful with zero annotations, and the preceding production deployment passed all tests, image/SBOM/signing/CVE gates, publish, and rollout steps.
- Completed native-era Backup CR history remains only as retention-governed evidence. Current Cluster backup/recovery fields, WAL archiving, and scheduling are plugin-only; no retired production path remains load-bearing.
