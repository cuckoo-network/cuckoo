...
The canonical PSL still lacks `onbex.co`, and this historical probe demonstrates the sibling-cookie risk. **Current posture (corrected 2026-08-18; [`.pm/DO_NOT_DO.md` `#PSL`](../../.pm/DO_NOT_DO.md)):** the risk is accepted before open signup; production keeps `BEX_BASE_DOMAIN=onbex.co`; unsetting it is not authorized. Host-only/`__Host-` guidance remains interim tenant defense in depth. A dedicated private Public Suffix may ship later only after both the startup and real-browser gates pass.
