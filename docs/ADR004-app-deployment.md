...
5. The selected pipeline pushes the release-generation tag to Zot. BuildKit output is pushed by credential-isolated Skopeo; kpack pushes through its Image controller. In local HTTP clusters kpack uses `BEX_KPACK_REGISTRY=zot.local:5000`, an alias for canonical `BEX_REGISTRY=zot.bex-registry.svc:5000`; the operator rewrites the successful digest back to the canonical name before workload rollout.
