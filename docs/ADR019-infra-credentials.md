...
- **CA is revocation-proof.** `bex-ca` compromise can't be contained by revoking a cert; it requires rotating the cluster CA (disruptive). _Follow-up:_ document a CA-rotation runbook alongside [ADR011-etcd-backup-restore.md](ADR011-etcd-backup-restore.md).
