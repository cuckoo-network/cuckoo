...
| `jobs list` | ✅ |  | `GET /v1/services/{id}/jobs` returns `[]jobWithCursor` — verified against bex-api with the dev-7 isolated env; empty list, non-empty list, and status filters all work. |
| `jobs create` | ✅ |  | `POST /v1/services/{id}/jobs` returns the created job with Render's shape (`id`, `serviceId`, `startCommand`, `planId`, `status`, `createdAt`); creates a k8s Job in the App's namespace using the service's image. |
| `jobs cancel` | ✅ |  | `POST /v1/services/{id}/jobs/{jobId}/cancel` returns the canceled job; returns 409 when already terminal — verified live with `render jobs cancel --confirm`. |
