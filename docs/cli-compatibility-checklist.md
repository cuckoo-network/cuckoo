...
| `ssh` | ◐ | **Running-instance path ✅ in production (w2/m39):** the unmodified official CLI v2.21.0 (`c398207`) reached public `ssh.bex.co:22` by service name and complete instance id, received the app runtime environment, and passed the same uninterrupted matrix as raw OpenSSH: pinned host key, PTY/resize, exit status, restart/redeploy closure, and authorization/type/lifecycle denials. The verifier isolates `RENDER_CLI_CONFIG_PATH` because v2.21.0's direct-id workspace check reads persisted config even while list calls honor `RENDER_WORKSPACE`. Both upstream instance-picker callbacks also assign the service id instead of the selected instance id (`cmd/ssh.go`), so exact targeting uses the CLI's documented direct full-instance-id argument without patching the binary or claiming that menu works. The command remains ◐ only because `--ephemeral` is deliberately not implemented; running-instance compatibility itself is complete. [ADR035](ADR035-ssh.md), [sanitized evidence](../.pm/w2/done/m39/evidence/2026-07-17-production-acceptance.md). |
