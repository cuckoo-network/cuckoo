...
- [~] **`psql [id|name]`** — reaches bex, resolves the target by id **and** name, then stops at the Render-parity IP-allow-list gate (no `BEX_DB_DOMAIN`/allow-list in dev-9 — the same block Render imposes). The bex-side wire contract is now proven CLI-compatible (`scripts/psql-compat-verify.sh` + its hermetic regression `scripts/psql-compat-verify.test.sh` drive the unmodified CLI's non-interactive path against bex's exact shapes: id/name resolution, the client-side `hasAccessToPostgres` allow-list gate, the external-string dependency, and a non-zero-exit surface). Flips to ✅ on the first live run in a `BEX_DB_DOMAIN` + pg-sni-proxy env, mirroring the `pgcli` supplement ([the psql CLI artifact](render-artifacts/psql-cli.md))
