...
- [~] **`blueprints`** — manage Blueprints (infrastructure as code)
  - [~] `blueprints validate <render.yaml>` — the unmodified v2.21.0 CLI successfully decoded both a valid `hello-go/render.yaml` response and an invalid `autoDeployTrigger: checksPass` response from freshly deployed `api.bex.co` on 2026-08-03 (the latter carried `services[0].autoDeployTrigger` plus source line/column). This proves the representative success/refusal wire shapes; the documented unsupported subset remains intentionally partial rather than a blanket parity claim. **w8/m19 supplement (2026-08-16):** the acceptance set the CLI validates against grew — static `buildCommand`, `dockerContext` (server+cron), `registryCredential`/`image.creds` by-name references, and custom `.yaml`/`.yml` Blueprint paths now validate instead of failing closed, and every remaining `unsupported` registry entry carries a real disposition (anti-goal citation or concrete mismatch) instead of the m63 audit-baseline placeholder. This is repository-regression evidence (the same multipart validate wire the 2026-08-03 live run graded, exercised by `internal/apps` fixture tests over the promoted fields); the local CLI-vs-dev-stack re-run was blocked by dev-cluster instability and the next live production `blueprints validate` capture after deploy re-grades it end to end.
