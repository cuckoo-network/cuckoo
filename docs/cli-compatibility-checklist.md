...
The full command surface of the official [Render CLI](https://render.com/docs/cli) (`render-oss/cli` **v2.26.0**), enumerated from `render --help` and **graded against a live bex** by driving the unmodified CLI through its REST API. Every command, subcommand, and flag is a dedicated checklist item.
...
`lego/cli/` imports (rather than forks) Render CLI **v2.26.0**, pinned to `6c0f561f8af9d4a6cfb88f4d1845ffd18cee181a`, and starts its exported command runner after mapping Bex defaults. With no `RENDER_*` input, it targets `https://api.bex.co/v1/` and lets the upstream config library persist its normal YAML schema as `~/.bex/cli.yaml`; `BEX_HOST`, `BEX_CLI_CONFIG_{DIR,PATH}`, `BEX_WORKSPACE`, `BEX_OUTPUT`, and a temporary `BEX_ACCESS_TOKEN` are supported Bex inputs. The launcher also defaults the upstream telemetry opt-out on (`RENDER_CLI_DISABLE_ANALYTICS=1` unless the user already set that or `DO_NOT_TRACK`): v2.26.0 made Render's usage analytics opt-out — on by default, phoning a stable install id + active workspace to Render's endpoint — which a bex user never consented to; the launcher neutralizes it in `internal/bridge` (process config only, no fork). Hermetic launcher tests cover that mapping, the analytics default, process-level authenticated REST behavior, and Render-config isolation. `scripts/bex-cli-auth-e2e.sh` is the live device-login/refresh/ logout evidence runner; it must be run against a Bex auth environment before a release tag. The launcher overlays Bex help chrome (`Use`, Short/Long/examples, `bex docs`, version line) via `lego/cli/internal/branding` without forking; residual runtime strings (login TUI, `ErrLogin`, User-Agent prefix, skills state path) remain upstream.

This launcher record does not replace the unmodified server oracle below: that client is the broad command/API compatibility baseline. The command surface is graded at the launcher pin (**v2.26.0** after the w8/m33 round-3 re-baseline; prior: v2.24.0 w2/m79, v2.21.0 live-REST production runs / launcher v2.22.0). Known server-side non-goals are still recorded as `[-]` in this checklist, including workflows, ephemeral SSH, and `ea` objects; importing a newer client does not claim those operations as implemented.
...
- **Upstream usage analytics are disabled by default** (v2.26.0 made them opt-out). The launcher sets `RENDER_CLI_DISABLE_ANALYTICS=1` in `internal/bridge` unless the user already set that or `DO_NOT_TRACK`, so the `bex` CLI does not phone a stable install id + active workspace to Render's telemetry endpoint — a service a bex user has no relationship with. This is launcher process config, not a fork; a user who explicitly re-enables it (or sets `DO_NOT_TRACK`) is left untouched.
- **Render's spec-based compute plan names are not accepted** (v2.25.0). The CLI's `--plan` help/picker advertises Render's new `0.1c-256mb` … `128c-1024g` (Postgres) and `1g` (Key Value) names, but that list is documentation-only, not validation — the CLI passes any value through, and bex validates against its own tier catalog ([ADR030](ADR030-pricing.md)), returning a named 400 for a name it does not offer. bex's own plan names work; whether to add spec-based aliases is the pricing-catalog decision filed as `.pm/w8/011.md`.
...
- [x] **`postgres`** (alias `pg`) — manage Render Postgres databases
  - [x] `postgres create` — id/name/ipAllowList wire shape correct (description persists); Render-route omission creates public intent while explicit `public:false` and shared Core callers remain private
    ...
    - [x] `--name`
    - [~] `--plan <free|basic_*|pro_*|accelerated_*>` — bex's own tier names round-trip; v2.25.0's help/picker also advertises Render's spec-based compute plans (`0.1c-256mb` … `128c-1024g`), which bex's tier catalog rejects with a named 400 (documentation-only CLI list, not validation — divergence `.pm/w8/011.md`)
