...
Established by the CLI and reused by the train: the tag is the **only** version source (build-time injection, no VERSION files); releases are tag-triggered (`cli-release.yml` template: test gates → build matrix → `checksums.txt` → keyless cosign bundle → GitHub release → channels) and immutable; the shared repo's `/releases` is partitioned by tag prefix, so consumers always filter by prefix (never `/releases/latest`) and asset URLs carry the encoded `%2F`. Versions are minted only by the `/release` skill (sync shipped source → validate checks → select version → tag → watch → verify channels; invocation authorizes publication without a separate version confirmation); `/ship` lands code and never tags. Staleness reminder + tag ruleset: `w4/032`.
