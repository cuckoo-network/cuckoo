...
This ADR supplements [ADR003](ADR003-control-plane.md), [ADR018](ADR018-render-parity.md), [ADR022](ADR022-tenant-isolation.md), [ADR028](ADR028-security-review.md), and [ADR034](ADR034-scalable-build-pipeline.md). Where an older document asserts that the current BuildKit push credential is inaccessible to tenant `RUN` processes, the security correction in this ADR takes precedence until O-01 is closed.
...
| ID | Severity | Finding | User or platform impact | Required correction |
| --- | --- | --- | --- | --- |
| O-01 | **Critical** | Untrusted build and pre-deploy execution crosses the platform namespace and credential boundary. | A tenant build can reach destinations not covered by tenant egress policy. The current BuildKit mode also makes registry-push credential confidentiality unproven. | Move untrusted execution to an explicitly isolated boundary; disable SA token mounting; apply default-deny ingress and egress plus narrow allow rules; restore a BuildKit process sandbox or remove the push credential from the process boundary; prove the result with adversarial live tests. |
| O-02 | **High, latent** | The tenant-image signature webhook selects a namespace label that production does not set and fails open. | Enabling signing as currently documented would not select tenant Pods, so unsigned images could still run. | Select actual workload labels or label the namespace deliberately; use fail-closed policy after readiness is proven; add rendered-manifest and live unsigned-image tests. |
| O-03 | **High** | `Database` deletion can be filtered before finalization. | Backup-purge and public-TLS cleanup may never run, leaving a terminating CR or leaked artifacts. | Admit deletion-timestamp/finalizer changes in the primary predicate, as the App controller already does; add an envtest deletion case. |
| O-04 | **High** | Postgres and Valkey do not consistently implement the advertised grow-only storage contract. | A Postgres plan downgrade can ask CNPG to shrink a PVC and be rejected forever; a Valkey storage increase does not resize its PVC. | Persist effective capacity independently of the selected plan, reject shrink, patch PVC requests for growth, check StorageClass expansion support, and surface resize conditions. |
| O-05 | **High** | `App.spec.type` is mutable but reconciliation does not prune resources from the previous type. | `web -> cron_job` can leave an old Deployment/Service/Ingress running; `cron_job -> web/static/worker` can leave the old CronJob firing. | Prefer an immutable type field. If transitions remain supported, compute the complete desired-kind set and delete every formerly-owned kind before reporting Ready. |
| O-06 | **High** | Global generation predicates suppress status-only events from owned Deployments/StatefulSets. | An App can remain `Ready` after its Pods crash, and KeyValue can report readiness from an old revision or miss a failed rollout. | Filter primary and owned resources separately; watch status-bearing child events; require observed generation and rollout revision convergence; retain a bounded safety requeue. |
| O-07 | **Medium-high** | Cross-namespace build credential copies are not fully owned or deleted. | Clone, env/file, registry, kpack Secret, and per-App kpack ServiceAccount copies can outlive the App in the shared build namespace. | Give every copy an App-identity label, inventory all generated identities, and make finalization list/delete them idempotently. Prefer tenant-scoped build namespaces to reduce shared residue. |
| O-08 | **Medium-high** | Finalizers acknowledge cleanup before cleanup succeeds. | Registry, static-object, backup, and TLS material can survive a successful API delete with no controller left to retry. Removed custom hosts can also leave TLS private keys. | Make cleanup synchronous or persist a cleanup Job and wait for terminal success; retry errors; remove finalizers only after every required artifact is absent; inventory historical host artifacts. |
| O-09 | **Medium** | Mutating a KeyValue password Secret does not roll the StatefulSet. | Clients receive the new password while the running Valkey process still accepts the old password, causing an outage until restart. | Treat the generated credential as controller-owned/immutable, or add a deterministic Secret-content hash to the Pod template and define a safe rotation procedure. |
| O-10 | **Medium** | Registry and Prometheus calls use unbounded default HTTP clients. | A stalled endpoint can hold scarce reconcile workers indefinitely and amplify the synchronous-build bottleneck. | Use shared clients with connect, TLS, response-header, and total request deadlines; propagate reconcile contexts and bound response bodies. |

### O-01: the build boundary is not currently defensible

Production sets `BEX_BUILD_NAMESPACE=bex-system` in [`manager.yaml`](../lego/operator/config/manager/manager.yaml). That namespace is labeled with Pod Security `privileged`. The tenant node/metadata egress CiliumNetworkPolicy is installed only in the tenant `default` namespace, while the `bex-system` policy is ingress-only. Build, pre-deploy, and static-publish workloads therefore execute tenant-controlled code or tenant-controlled images next to platform services without the tenant egress boundary.

The BuildKit Job in [`build.go`](../lego/operator/internal/build/build.go) puts the workspace label on the Job but not its Pod template and does not set `automountServiceAccountToken: false`. Static publish has the same token-mount problem. Pre-deploy disables the token, but still executes the tenant image in the shared platform namespace.

The BuildKit worker also uses `--oci-worker-no-process-sandbox` with an unconfined seccomp profile. Upstream BuildKit explicitly warns that in this mode build containers can kill and potentially ptrace arbitrary processes in the BuildKit container. The registry push credential is mounted into that same container. This does **not** prove that a credential has been extracted in production, but it invalidates the absolute claim that an adversarial `RUN` process cannot reach it. The minimum proven impact from the rendered policy is broader network reachability; process-level credential extraction is a credible risk that requires either elimination by design or a live adversarial reproduction.

O-01 is a release blocker for real multi-tenant source builds. Minimum closure evidence is:

1. a rendered Pod template with no service-account token and the correct workspace identity;
2. default-deny ingress and egress in a dedicated untrusted build boundary;
3. explicit DNS, source-host, registry, and required datastore allow rules only;
4. a failed attempt to reach metadata, kubelet, the Kubernetes API, bex-api, OpenBao, Prometheus, other tenant services, and another tenant's registry repository;
5. a failed attempt to read or infer registry/signing credentials from an adversarial Dockerfile; and
6. equivalent tests for pre-deploy and static publish paths.

Upstream evidence: [BuildKit rootless process-sandbox warning](https://github.com/moby/buildkit/blob/6dd06999d5d369a217c3f3259a420f507e2db2c7/docs/rootless.md#L98-L104).

### O-02 through O-06: reconciliation correctness

- **Webhook selection:** [`manifests.yaml`](../lego/operator/config/webhook/manifests.yaml) selects namespaces labeled `bex.co/workspace`; production's tenant namespace has no such label, and tenant Pods use `app.bex.co/workspace`. `failurePolicy: Ignore` turns a configuration error into silent bypass. The control is dormant in the current GitOps configuration because tenant signing is not enabled, but it is unsafe to enable as written.
- **Database finalization:** [`database_controller.go`](../lego/operator/internal/controller/database_controller.go) handles deletion in reconcile, but the controller's `GenerationChangedPredicate` can discard the deletion update because deletion does not increment generation. External Secrets Operator's current predicate is a useful reference: it explicitly admits changes to generation, labels, annotations, finalizers, and deletion timestamp. See [ESO's predicate](https://github.com/external-secrets/external-secrets/blob/fe392a2cb897270d376ac0ad91dc74e693cc95e1/pkg/controllers/externalsecret/externalsecret_controller.go#L1398) and [Kubebuilder finalizer guidance](https://kubebuilder.io/reference/using-finalizers).
- **Storage:** [`database_controller.go`](../lego/operator/internal/controller/database_controller.go) compares requested storage only with the new plan floor, not the previously allocated size. A `basic-1gb` to `basic-256mb` downgrade can therefore request `5Gi -> 1Gi`; CNPG rejects PVC shrink. Valkey only puts storage in a StatefulSet volume-claim template at creation and never patches the PVC. See [CNPG's existing-PVC reconciler](https://github.com/cloudnative-pg/cloudnative-pg/blob/e39ec6b4151236d142f7dbc6edc9b251ac24f156/pkg/reconciler/persistentvolumeclaim/existing.go), [CNPG shrink validation](https://github.com/cloudnative-pg/cloudnative-pg/blob/e39ec6b4151236d142f7dbc6edc9b251ac24f156/internal/webhook/v1/cluster_webhook.go#L1805), and [Kubernetes PVC expansion](https://kubernetes.io/docs/concepts/storage/persistent-volumes/#expanding-persistent-volumes-claims).
- **Type transition:** the backend Blueprint update path can rewrite `App.spec.type`, while [`app_controller.go`](../lego/operator/internal/controller/app_controller.go) returns from type-specific branches without deleting kinds emitted by the old branch. OpenChoreo avoids this class by making component type and immutable release snapshots immutable in its CRD validation.
- **Status events:** App applies one generation/deletion predicate to both the App and all owned resources. Deployment status changes normally do not change generation. KeyValue similarly filters its StatefulSet watch by generation while readiness checks only available replica counts. Prometheus Operator separately admits status-relevant StatefulSet updates; CNPG watches Pods and PVCs rather than depending only on parent generation. See [Prometheus Operator's StatefulSet predicate](https://github.com/prometheus-operator/prometheus-operator/blob/d7d70ebb896b1d655c1d7d88ae11a1ce6ba5b80c/pkg/operator/resource_reconciler.go#L541) and [CNPG's child watches](https://github.com/cloudnative-pg/cloudnative-pg/blob/e39ec6b4151236d142f7dbc6edc9b251ac24f156/internal/controller/cluster_controller.go#L1278).
...
### 3. Keep an OpenChoreo-first, Korifi-second evaluation as a future option
...
O-01 and O-02 are security activation blockers. O-03 through O-06 are correctness blockers. Their fixes apply to the direct engine and become acceptance tests for any replacement. O-07 through O-10 follow immediately after those blockers.
...
| --- | --- | --- |
| P0 | O-01 build/pre-deploy/publish isolation and O-02 webhook activation | Adversarial live matrix plus rendered-manifest tests; no credential/network escape; unsigned tenant image denied when feature is enabled. |
| P1 | O-03 finalizer events, O-04 storage, O-05 type immutability/pruning, O-06 child status watches | Envtests covering deletion, downgrade/growth, every type transition, crash-after-Ready, and failed StatefulSet rollout. |
