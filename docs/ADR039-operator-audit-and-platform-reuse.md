...
| O-07 | **Medium-high** | **Closed — w2/m61** | Cross-namespace build credential copies were not fully owned or deleted. | Clone, env/file, registry, kpack Secret, and per-App kpack ServiceAccount copies could outlive the App in the shared build namespace. | Every execution artifact now carries App name, UID, workspace, source namespace, and component identity; one inventory reclaims Jobs, Pods, Secrets, ServiceAccounts, NetworkPolicies, and kpack Images/Builds and proves a zero result before finalization. |
| O-08 | **Medium-high** | **Closed — w2/m61** | Finalizers acknowledged cleanup before cleanup succeeded. | Registry, static-object, backup, and TLS material could survive a successful API delete with no controller left to retry. Removed custom hosts could also leave TLS private keys. | Cleanup errors retain finalizers. Static and backup purge use restart-adoptable terminal Jobs; registry deletion requires a later empty read; TLS history covers removed hosts; named Kubernetes artifacts are deleted and observed absent. |
| O-09 | **Medium** | **Closed — w2/m61** | Mutating a KeyValue password Secret did not roll the StatefulSet. | Clients could receive a new password while the running Valkey process still accepted the old password. | The controller now owns an immutable `<id>-auth` Secret, stamps its one-way revision on the Pod template, and exposes connection data only when Ready status proves the current rollout uses that revision. Direct Secret mutation is rejected; user-driven rotation is not supported. |
| O-10 | **Medium** | **Closed — w2/m61** | Registry and Prometheus calls used unbounded default HTTP clients. | A stalled endpoint could hold scarce reconcile workers indefinitely and amplify the synchronous-build bottleneck. | One pooled client policy supplies connect, TLS, response-header, idle, and total deadlines plus a 1 MiB response cap. Reconcile contexts reach every audited registry/Prometheus call; deterministic stalled-connect/TLS/header/body and cancellation tests bound failure. |
...
The Dockerfile path is now `clone -> BuildKit OCI export -> Skopeo push -> optional Cosign sign`. Clone, private-base, App-repository, and signing credentials have different serial phase custody. BuildKit runs rootful only inside the Pod user namespace, keeps its default OCI process sandbox, and never mounts the output credential or signing key. Production explicitly enables the Kubernetes 1.34 `UserNamespacesSupport` gate and pins containerd 2.3.3/runc 1.5.1. The prior `--oci-worker-no-process-sandbox` flag is absent and regression-tested. The upstream [rootless process-sandbox warning](https://github.com/moby/buildkit/blob/6dd06999d5d369a217c3f3259a420f507e2db2c7/docs/rootless.md#L98-L104) remains the reason that mode is prohibited.
...
| --- | --- | --- |
| BuildKit, pre-deploy, static publish | Jobs, Pods, copied/derived Secrets, ServiceAccounts, execution NetworkPolicies | App finalizer inventories exact UID ownership in `BEX_BUILD_NAMESPACE`, issues idempotent deletes, and requires a fresh zero inventory. |
| kpack | Images, Builds, per-App ServiceAccount, App-specific registry/Git Secrets | Same inventory and UID rule; deterministic-name adoption rejects another App lifetime. |
| App edge and external stores | historical TLS Secrets/private keys, Zot manifests, static-site object prefix | TLS identities are persisted before host removal; Zot requires an empty/404 tag read after deletion; a persisted static purge Job must complete and then be observed absent. |
| Managed Postgres backups | per-database barman prefixes and purge Job/Pods | Database finalizer waits for CNPG disappearance, persists terminal purge completion on the Database, and then removes and observes the cleanup Job/Pods absent. |

The cleanup Job state machine persists a UID-scoped completion marker on its parent before deleting the Job. A manager restart therefore adopts a still-running Job, cannot mistake a different lifetime's Job for its own, and does not repeat already-proven object-store deletion. Failed or nonterminal Jobs and transient Kubernetes/registry errors retain the finalizer. `scripts/delete-audit.sh` now checks CR absence, the full execution inventory, kpack Builds, cleanup Jobs/Pods, historical TLS material, registry tags, object prefixes, and both KeyValue Secrets.

KeyValue credentials deliberately do not offer rotation. `<id>-auth` is the immutable password authority; the existing `<id>` Secret is an immutable connection-string projection. The StatefulSet reads only the authority and carries `app.bex.co/credential-revision`; status publishes only the SHA-256 revision, never the credential. REST/GraphQL connection info returns 409 while phase/revision/rollout do not agree, so the dashboard cannot reveal a split credential.

Registry resolution/probing/finalization, Prometheus autoscaling queries, image verification, and OpenSandbox calls use bounded clients and response reads. Tests separately stall connect, TLS, response headers, and response bodies and preserve caller cancellation, so protocol failures remain distinguishable from `context.Canceled`/`context.DeadlineExceeded`.
...
| --- | --- | --- | --- |
| **OpenChoreo** | Apache-2.0 Go controllers; separate control, workflow, data, observability, and experience planes; OpenAPI and MCP; component types that render Deployment/StatefulSet/CronJob/Job; immutable releases and bindings; Argo-based build workflows; ResourceTypes with outputs, secret references, readiness, garbage collection, and retention. The original source review covered `1.2.0-rc.1`; [ADR070](ADR070-openchoreo-evaluation.md) pinned and ran stable `v1.1.2`. | CNCF **Sandbox**, young project, public CRDs remain `v1alpha1`, and the target state adds a four-plane topology plus a compatibility/ownership adapter. The live stable build path regressed O-01, registry deletion left residue, storage readiness was too weak, and no production Valkey authority or Render compatibility existed. | **Reject current adoption; watch only.** ADR070 records 2 pass, 2 partial, and 5 failed gates plus the conditions for a future stable reevaluation. |
...
### 5. Keep the closed audit as the acceptance boundary for any future substrate evaluation

O-01 and O-02 were security activation blockers; O-03 through O-10 were correctness and lifecycle blockers. Their completed fixes apply to the direct engine and are acceptance tests for any replacement.
...
| P2 — **DONE 2026-07-19** | O-07 artifact inventory, O-08 durable cleanup, O-09 KeyValue consistency, O-10 HTTP deadlines | w2/m61: exact-UID inventory/delete-recreate and transient-failure tests, restart-adopted terminal cleanup Jobs, immutable credential/revision gating, deleting public status, and deterministic connect/TLS/header/body/cancellation deadline tests. |
...
| --- | --- | --- |
| Future A — **evaluated 2026-07-19; rejected** | OpenChoreo engine adapter | [ADR070](ADR070-openchoreo-evaluation.md): stable v1.1.2 scored 2 pass, 2 partial, 5 fail; no adapter or migration is authorized. |
...
| Future decision | Accept or reject substrate migration | OpenChoreo is **Reject** for current adoption in ADR070. Any Korifi result or future OpenChoreo reopening requires its own measured code/operations delta, migration/rollback plan, CRD version risk, and ownership map. |
