...
Existing pieces this builds on: the CRD already defines `PhaseHibernated` and `spec.idleTTLSeconds` ("sleep = free"); the opensandbox runtime already has real pause/resume snapshots (~80 ms wake); the kubernetes runtime rolls Deployments via `CreateOrUpdate` and gates `Running` on replica readiness.
...
- Resume is **manual** for an explicitly suspended service: it clears `spec.suspended` and is intentionally idempotent when that intent is already clear. It does not override a replica count chosen by auto-sleep or autoscaling. Auto-hibernate (`idleTTLSeconds` after no traffic) and wake-on-request are implemented by the traffic-aware activator at the public edge, so only web services with that wake path participate. Private services are excluded: their in-cluster traffic neither reaches the activator nor supplies its activity signal. For agent **sandboxes**, idle-hibernate + wake-on-connect remains the separate design in [ADR014-sandboxes.md](ADR014-sandboxes.md) (gateway-observed `autoPause` over opensandbox's real pause/resume).
- **What `idleTTLSeconds: 0` means (decided, w6/m116):** `0` — the value a free service is created with, and the CRD's unset default — means **the platform default idle window of 15 minutes**, resolved in the operator; a **positive** value overrides it, and any value up to the 7-day `MaxIdleTTLSeconds` ceiling is accepted. It does **not** mean "never sleep": a free web service always auto-hibernates (on the default if it sets nothing), matching Render's fixed, always-on free spin-down, so "off" is deliberately inexpressible for the free tier. The literal `0` is stored **unrewritten** — the API never substitutes the concrete default on write — so the platform default can change without migrating stored Apps, and the dashboard's "Platform default" option stays distinct from an explicit "15 min". 15 minutes anchors on Render's free spin-down window and is one of the presets the Settings select already offers. Paid tiers ignore the field entirely (always-on).
