...
**Encryption at rest** (required for the parity row): a sibling StorageClass `hcloud-volumes-luks` (same provisioner, LUKS parameters per the csi-driver's documented support); the operator mints a random per-disk passphrase Secret in the workspace namespace, referenced only by the CSI node-publish parameters. **Enabled in production 2026-09-02 (w2/m86)** via `BEX_DISK_STORAGE_CLASS` in the prod operator overlay (`lego/operator/config/prod/`), after live verification corrected this ADR's two original prerequisites: `cryptsetup` is bundled **inside** the hcloud-csi-driver image (v2.22.0 ships cryptsetup 2.8.6; the CAPH node image needs nothing), and the passphrase Secret is fetched by the **kubelet** under the node authorizer, not by the CSI ServiceAccount (no tenant-namespace RBAC exists or is needed — external-provisioner templates `${pvc.name}-luks` into the PV's `nodePublishSecretRef`). That template is a naming contract: `DiskLUKSSecretName` is defined as `DiskPVCName + "-luks"` so the kubelet's lookup can never diverge from the minted Secret (the original digest-inside-the-budget derivation diverged for App names ≥54 chars — every such disk would have failed to mount; fixed before any tenant disk existed). The driver formats LUKS1 and resizes the open mapping without re-authentication, so growth works online. Disks created before the flip stay on the default class (class is immutable; zero existed). Snapshots are independently age-encrypted (§ D5), so data at rest is covered on both the volume and the backup target.
...
- **Dashboard**: a **Disk** tab (Render's singular label) in the Manage sidebar group on eligible service pages (currently omitted as a non-goal). The add form mirrors the live-captured contract ([render-artifacts/disks.md](render-artifacts/disks.md)): mount path + size only — no name input (bex auto-derives the API-level `name`), `/var/data` placeholder, the five-bullet warning list (zero-downtime deploys lost, no multi-instance, one disk max, only mount-path files persist, no cross-service access), 10/50/100 GB quick-select chips with a free-text box defaulting to 10 (Render also offers 1 and 5; both sit below Hetzner's 10 GB volume floor — see § D8), and an empty-state card quoting bex's $0.175/GB-month rate. Post-create: usage-over-time graph (`kubelet_volume_stats_*` on the disk PVC, the datastore metrics path extended with a third kind, `service`, alongside `database`/`keyvalue` — it rides that verb rather than the App-metrics verb because what it measures is a PVC, and only `disk`/`disk_capacity` are valid for it; the other datastore metrics are refused by name, and a diskless service returns an empty series rather than an error so the tab can ask before it knows). **The claim name is derived, not spelled**: `appv1alpha1.DiskPVCName` moved into the leaf contract module in w1/m86 so bex-api matches the exact claim the operator created — a literal `^disk-<name>$` would silently graph nothing for any app name long enough to hit the truncate-plus-digest path, which is a failure with no error attached to it, grow, snapshot list + restore (confirmation phrase), delete.
- **REST shape decisions (w1/m88, from the m86 cross-surface audit)** — each of these was an accidental divergence until it was decided:
  - **Create-time attach is supported.** Render's `webServiceDetailsPOST` / `privateServiceDetailsPOST` carry `serviceDetails.disk` (schema `serviceDisk`), and because Render-matched routes decode strictly, its absence was not "ignored" but a 400 on `unknown field "disk"` — a Render client could not create a disk-bearing service at all. bex now accepts it and runs it through the same `validateCreateDisk` the Blueprint path uses, so eligibility, tier, replicas and the mount-path denylist cannot drift between the two ways in.
  - **`sizeGB` is required on `POST /v1/disks` and optional at create.** That looks inconsistent and is not: Render's `serviceDisk` requires only `name` + `mountPath`, while its `add-disk` body requires `sizeGB`. bex reproduces Render's asymmetry rather than smoothing it out, defaulting to 10 GB at create (matching the Blueprint default) and refusing an omitted `sizeGB` on the standalone route.
  - **`restore-snapshot` answers 200 with the disk**, not a bodyless 202. The work stays asynchronous — the operator scales down, restores, and brings the service back — but returning the resource names what is being restored rather than claiming it finished, and a Render client's generated code decodes a disk there. GraphQL and MCP return the disk too, so the three surfaces agree; GraphQL's previous `Boolean` told a caller only that nothing threw.
  - **`list-snapshots` answers 201**, which Render's own pinned spec documents for that GET even though it creates nothing. Matched deliberately: a generated client switches on the status code, so a tidier 200 would leave its 201 branch nil and its result empty. The oddity is Render's; propagating it is what compatibility means here.
  - **Every documented `list-disks` filter now filters.** `serviceId` (an array), `diskId`, `name`, and the `created*`/`updated*` windows all pass the OpenAPI gate because the pinned spec declares them, so an unimplemented one was silently ignored — the caller received an unfiltered list and could not tell. Multiple `serviceId` values are unioned by asking the authorized verb once per service, so naming more services can only narrow what a caller may read, never widen it; within a multi-value filter an id that is gone or invisible matches nothing rather than failing the whole request, while a single unknown id remains a 404.
...
| --- | --- | --- | --- |
| Service disk (new) | $0.25/GB-month, provisioned, prorated by the second | **$0.175/GB-month, provisioned, prorated by the second, minimum 10 GB** | 30% off |
| Disk snapshots (new) | no charge | no charge | — |

Mechanics:

- **Minimum size is 10 GB (corrected 2026-08-24, `.pm/w1/078.md`).** Hetzner's minimum Cloud Volume is 10 GB — recorded in § Context from the first draft — but the implementation shipped `diskMinSizeGB = 1` and the dashboard copied Render's 1/5/10/50/100 GB chips without re-checking them against that floor. A 1 GB request therefore produced a **10 GB volume**: bex billed $0.175 and paid Hetzner for a full 10 GB volume, selling its smallest disk below cost. Caught by reading a live disk's kubelet capacity (10464022528 bytes ≈ 9.75 GiB on a disk whose spec said 1 GB), not by any test. The floor makes displayed size, provisioned size and billed size the same number. Render still offers 1 and 5 GB; that is now a documented divergence, since bex discounts 30% and cannot absorb the difference.

  The floor is **API policy, not a CRD rule**: `spec.disk.sizeGB` keeps `Minimum=1` so that disks provisioned before the change stay valid. Raising it in the CRD would fail validation on their next write, and the grow-only rule leaves no way back into compliance. It is also deliberately not configurable — the constraint is Hetzner's, and a knob is speculative until a self-hoster on other infrastructure asks.
...
| --- | --- | --- |
| Hetzner volume (**measured**, w2/030) | **$0.0767/GB-mo net** (USD-billed prod account) | **~56.2% gross margin** |
...
**Decision (w2/034, 2026-09-08):** keep **$0.175** (Render $0.25 × 0.70) so disks stay on the same 30%-off-Render sheet as compute/Postgres/Key Value ([ADR030](ADR030-pricing.md)). Do **not** raise the rate to chase the earlier ~65–71% margin hypotheses — those were derived from Hetzner's EUR public list and do not apply to this USD-billed account. `pricing.yaml` unchanged.

_Superseded planning figures (pre-measurement):_ EUR list €0.0440 ≈ $0.050 → ~71%; April-2026 rise scenario €0.0572 ≈ $0.062 → ~65%. Neither matched the authenticated `GET /v1/pricing` read.
...
- **Snapshot pipeline verified end to end on a live cluster (w1/m87/t004, 2026-08-25).** A disk-bearing App produced a `Bound` 10 GiB PVC and its LUKS Secret; the operator created the backup CronJob (`45 2 * * *`, the staggered window); a triggered run wrote a real object at `disks/<workspace>/<app>/2026-08-25T06:55:44Z.tar.gz.age`; the disk was then mutated (marker flipped, a second file added) and restored, after which the marker held its **pre-snapshot** contents and the post-snapshot file was **gone**; detaching the disk ran the purge Job and left **zero** objects under the prefix, with the CronJob removed. The `SnapshotImageUnresolved` fail-closed guard also fired correctly before the helper image was resolvable — it refused to schedule rather than create a broken CronJob. Three provisioning-contract defects the drill exposed are recorded in [runbooks/disk-snapshot-setup.md](runbooks/disk-snapshot-setup.md) § Contract details and guarded in CI.
- **Arming snapshots in a deployment**: [runbooks/disk-snapshot-setup.md](runbooks/disk-snapshot-setup.md) — the bucket, the two scoped identities (the operator writes; bex-api only lists), the disk-dedicated age keypair, and the live checks that prove bex-api cannot write or delete a tenant's backups. Provisioned by `scripts/disk-snapshot-secret.sh`; its refusals are guarded by `scripts/disk-snapshot-secret.test.sh` in CI. Since w2/033 (2026-09-08) the credential pair is **projected by the operator** from the apps namespace into each disk-bearing App's own namespace at reconcile time (the datastore `BackupSourceNamespace` pattern), so a workspace namespace created after provisioning still takes its nightly snapshot — previously its first backup Job died in `CreateContainerConfigError` until the script was re-run, and the bootstrap apps namespace was never covered at all. A missing canonical pair sets the named `SnapshotCredentialUnavailable` condition instead of scheduling a doomed CronJob; envtest pins projection, drift refresh, the protected-from-tenant-mount label, and the fail-closed branch.
- **Snapshot pipeline armed on production + operator-path LUKS proof (w2/m86, 2026-09-02, later the same day).** The runbook's first production run surfaced and fixed four latent defects — the provision script's first-run pipefail abort (orphaning once-shown Wasabi keys), the never-implemented `bex-system/bex-disk-snapshot` secretKeyRef contract (no manifest-armed cluster ever created a backup CronJob), the backup Job's read-only claim source (EBUSY beside the app's rw LUKS mount, so a running app on the encrypted class could never be backed up), and the snapshot/restore container's non-root uid (permission denied on any real ext4 volume's root-owned `lost+found` — latent on **every** class; CAPD's 777 `local-path` dirs hid it). All four fixes are guard-/envtest-pinned; details in [runbooks/disk-snapshot-setup.md](runbooks/disk-snapshot-setup.md) § Contract details. A hand-applied App then provisioned its PVC on `hcloud-volumes-luks` through the operator (Recreate + eviction guard + CronJob + clean teardown). The end-to-end snapshot/restore re-run on a running LUKS App awaits the fixed operator image's rollout.
- **LUKS mechanism drilled on production (w2/m86, 2026-09-02).** In a scratch namespace with a hand-minted `${pvc.name}-luks` Secret, a 10Gi PVC on `hcloud-volumes-luks` bound and mounted in 32 s: the PV carried the templated `nodePublishSecretRef`, the pod's filesystem sat on `/dev/mapper/scsi-0HC_Volume_106770929`, and `cryptsetup status` in the node plugin reported **LUKS1, aes-xts-plain64, 512-bit key, active**. A 10→11Gi grow applied **online** (~30 s: volume, LUKS mapping 20.9M→23.0M sectors, and ext4 all grew with the pod running; data intact) — better than D4's documented worst case, which stays as the honest contract since the maintainers still do not claim online expansion. Namespace deleted; `reclaimPolicy: Delete` removed the Hetzner volume. `cryptsetup 2.8.6` was confirmed inside the running v2.22.0 node plugin (`alpine` image bundles it), and zero disk-bearing Apps existed at flip time. `scripts/gitops-validate.sh` § "persistent-disk LUKS class contract" pins the class shape, the template↔Secret-derivation agreement, and the prod-on/local-off env split.
- **Pre-GA cost check — COMPLETED 2026-09-08 (w2/030)**: authenticated `GET https://api.hetzner.cloud/v1/pricing` against the production Hetzner project (operator-custodied `HCLOUD_TOKEN`, never printed) answered the €0.0440-vs-€0.0572 question with a third number: this account bills in **USD**, and `volume.price_per_gb_month` is **$0.0767 net (VAT 0.00%, gross identical)**. Against the $0.175/GB-month sale rate that is a **~56.2% gross margin** — materially below both prior hypotheses (~71% / ~65%, § Pricing), which were derived from EUR price-page figures that do not apply to this USD-billed account. **Keep decision (w2/034, 2026-09-08):** retain $0.175 (Render × 0.70) for sheet consistency; § Pricing unit-economics table updated to the measured cost. `pricing.yaml` untouched; meter mechanics unaffected (provisioned-GB metering still matches Hetzner's hourly-billed, monthly-capped cost basis exactly). _Historical context of the earlier failed attempt (2026-08-23): no token existed in any bex-run environment then, and Hetzner's public price page renders client-side; the durable vendor confirmations from that read — hourly billing with a monthly cap, and verbatim "We do not provide Backups or Snapshots for Volumes" — stand._
