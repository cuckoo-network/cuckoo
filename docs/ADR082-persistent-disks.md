...
- **Arming snapshots in a deployment**: [runbooks/disk-snapshot-setup.md](runbooks/disk-snapshot-setup.md) — the bucket, the two scoped identities (the operator writes; bex-api only lists), the disk-dedicated age keypair, and the live checks that prove bex-api cannot write or delete a tenant's backups. Provisioned by `scripts/disk-snapshot-secret.sh`; its refusals are guarded by `scripts/disk-snapshot-secret.test.sh` in CI.
