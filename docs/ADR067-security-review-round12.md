...
| 8 | Mutable backup images + runtime `apk add age` in plaintext path | low | **Resolved 2026-08-21 (`w7/m85`)** — was deferred as the digest-pinning inventory (fifth report); see Re-confirmed residuals |
...
- **8 (low, deferred → RESOLVED 2026-08-21, `w7/m85`)** — the Key Value backup pipeline runs mutable Valkey/BusyBox/Alpine tags and installs `age` at runtime via `apk add` in a stage that reads the plaintext RDB. Fifth report of the digest-pinning inventory deferral (ADR061 1 → ADR063 12 → ADR066 7 → ADR064), now explicitly extended to cover the age stage's runtime package install; the durable fix is the reviewed first-party backup helper ADR061 already scoped. Snapshot-stage `REDISCLI_AUTH` exposure is bounded by the same supply-chain prerequisite (upstream compromise). **Resolution (`w7/m85`, 2026-08-21).** Closed at both halves, and the whole ADR055 F7 lineage (F7 → ADR061 1 → ADR063 12 → ADR066 7 → this → [ADR068](ADR068-security-review-round13.md) 9) closes with it.
  - _The plaintext-RDB stage._ The encrypt container no longer resolves anything at run time. `apk add age` had already been replaced by a checksum-verified release download (round-16 11); that download is now gone too — the stage runs `/backup-encrypt`, a first-party entrypoint of the bex image built from `filippo.io/age` v1.3.1 (`lego/operator/cmd/backup-encrypt`). Its output is a standard age file, so `scripts/restore-keyvalue.sh` decrypts it with the stock `age` binary unchanged. Snapshot and compress were pinned by round-14 5 and stay pinned, so no stage in the pipeline is tag-addressed or fetches code.
  - _Why not the separate helper image ADR068 9 named._ The operator image is already first-party, reviewed, cosign-signed, SBOM-attested and digest-pinned by the deploy write-back. Publishing a second artifact to carry three binaries would add a registry artifact to scan and sign, a sixth generated digest to the deploy transaction, and a bootstrap fallback path — for a stage the bex image can run itself. The operator resolves its own image off its Pod (`internal/selfimage`, `POD_NAME` downward API), so the encrypt stage always matches the digest the operator was rolled out with. With encryption configured and that image unresolved, the reconcile **fails closed** rather than uploading plaintext.
  - _The inventory, and the reason it kept coming back._ Every image reference bex builds from, runs, or ships now resolves by digest — including four this round found still floating: `axllent/mailpit` (two manifests), the `BEX_SANDBOX_IMAGE` default (`docker.io/library/alpine:3`), the `BEX_AGENT_SESSION_IMAGE` default (`:latest`), and CI's `postgres:17` + `openfga/openfga:latest`. The durable half is `scripts/image-pin-validate.sh`: it re-derives the inventory on every CI run across four syntaxes (Dockerfile `FROM`, Go string literals, manifest `image:`/kustomize/Helm values, workflow `docker run`), fails closed on anything it cannot classify, and carries enumerated exemptions with a reason each. `scripts/image-pin-validate.test.sh` proves it red on each syntax and green on the fixed tree.
  - _A defect the verification found._ `--verify-digests` asks each registry whether the pinned digest exists in the repository the reference names. `quay.io/skopeo/stable@sha256:c7d3c512…` did not: quay converts its stored OCI index to a Docker manifest list on request and reports the **converted** digest, which it never stores. The build plane's push stage was pinned to an image quay would not serve. Repinned to the stored index (`sha256:64ac45c5…`).
...
| digest-pinning inventory (incl. `apk add age`) | ADR055 F7 | re-confirmed (finding 8, 5th) — **closed 2026-08-21 (`w7/m85`)**, lineage retired |
