...
**Open, filed 2026-07-15 (w1/m37, discovered incidentally, not fixed by that milestone):** outbound webhooks (`lego/backend/internal/webhooks/worker.go`) POST to a tenant-supplied destination URL with no SSRF protection — `parseDestination` (`service.go:406`) validates URL syntax only (absolute, `http`/`https` scheme, non-empty host); the outbound `http.Client` (`worker.go:182`) uses the stock dialer, so a tenant can register `http://169.254.169.254/...` or an internal `10.x.x.x`/`192.168.x.x` address as a webhook endpoint and bex-api will POST to it. m37 added SSRF protection (dial-time IP validation via `net.Dialer.Control`, blocking loopback/private/link-local/unspecified ranges, no redirect-following) to the _new_ maintenance-mode custom-page fetch (`lego/operator/cmd/activator/main.go`) since that path was being introduced fresh; the identical, pre-existing gap on the outbound-webhook path was left as-is to keep that milestone scoped. Recommend porting the same guard (or extracting it to a shared helper) to `worker.go`'s `defaultClient`.
