...
| Web-shell ticket single-use is per-replica; ticket rides the URL query string | LOW | **fixed** (2026-07-20, `w1/done/042`) — shared `shell_ticket_nonces` atomic claim (single-use across replicas, fail closed); ticket moved to a `Sec-WebSocket-Protocol` entry (never the logged RequestPath; legacy query fallback for pre-roll tabs, then remove) |
