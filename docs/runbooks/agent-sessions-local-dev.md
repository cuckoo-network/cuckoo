...
- **Reap finished sandboxes to free the plan ResourceQuota.** The `<ws>-sandbox` namespace has a starter-plan `tenant-quota` whose `limits.cpu: 4` / `limits.memory: 8Gi` caps it at **exactly two** live sandboxes (2cpu/4Gi each). Old sessions' sandboxes aren't reaped promptly (30m idle TTL), so a third session sticks in **Creating / "Starting the sandbox…"** while the controller loops `failed to scale batch sandbox pods … is forbidden: exceeded quota: tenant-quota`. Free it with `kubectl -n <ws>-sandbox delete bsbx --all` (use the `bsbx` short name / `batchsandboxes.sandbox.opensandbox.io` — the singular `batchsandbox` intermittently fails API discovery here). Note the controller uses exponential backoff, so a sandbox already stuck won't retry the instant quota frees — nudge an immediate reconcile with `kubectl -n <ws>-sandbox annotate bsbx <id> bex.co/nudge="$(date +%s)" --overwrite`.
