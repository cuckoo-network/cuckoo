...
Cross-surface check: the dashboard's update payload sends only dirty fields over GraphQL `updateWebhookEndpoint` (`eventTypes` name — the GraphQL adapter accepts both it and Render's `eventFilter`, resolving precedence server-side), matching REST `PATCH /v1/webhooks/{id}` merge semantics and the MCP `update_webhook_endpoint` tool; the status toggle uses `setWebhookEndpointEnabled`, the same core verb REST PATCH's `enabled` maps to. No new drift introduced.
