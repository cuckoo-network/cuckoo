...
| Projects | Search plus All/Services/Env Groups filtering over resources | Project detail has environment cards, resource tables, and management actions, but no search/type filter | One missing interaction family | Real gap: [w5/020](../../../.pm/w5/020.md). `render-walk-projects.png` / `bex-walk-project.png`. |
...
| Team | Member table with search | Real accepted-member table and role actions, but no member search | One missing control | Real gap: [w5/019](../../../.pm/w5/019.md). `render-walk-workspace-team.png` / `bex-walk-workspace-team-and-audit.png`. |
