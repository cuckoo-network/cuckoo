...
- The separate multi-replica MCP residual observed during this walk is closed by w2/m58: Streamable HTTP POSTs are stateless, and selected-workspace state is shared through Postgres by MCP session plus authenticated subject. Follow-up requests may now reach any bex-api replica without an affinity retry.
