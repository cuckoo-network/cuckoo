...
| --- | --- | --- | --- | --- |
| Service root | Deploy history is the main content; Events is a separate tab. `render-walk-service-overview.png` | The root redirects to Events. Deploys exists as a separate complete page. `bex-walk-service-overview.png` | Divergent default route | Real gap: [w5/015](../../../.pm/w5/015.md). |
...
| Environment | Variables, secret files, group linking, add/edit actions, and Export | Variables, secret files, and group linking exist; Export is absent | One missing control | Real gap: [w5/017](../../../.pm/w5/017.md). Local secret-store unavailability is configuration, not a second gap. |
| Logs | Search, range selector, display actions, and log stream | Structured type/level/method/status/path/instance filters, search, and live tail exist; there is no user-owned time-range control | One missing control; bex has extra structured filters | Real gap: [w5/016](../../../.pm/w5/016.md). Host/path store support is already owned by `w3/m18`. `render-walk-service-logs.png` / `bex-walk-service-logs.png`. |
| Metrics | Time range, event filter/timeline, application CPU/memory/instances, and request/network controls | Time range, CPU/memory/instances, request percentile/status/grouping, and network charts exist; the event timeline/filter is absent | One missing section | Real gap: [w5/018](../../../.pm/w5/018.md). Functional host/path metrics are already owned by `w3/m18`, so no duplicate was filed. `render-walk-service-metrics.png` / `bex-walk-service-metrics.png`. |
