...
| `--runtime` | `serviceDetails.runtime` | Exact for native web and cron services. Readback also derives `runtime`/`env` for shapes created without the flag so a partial `services update` round-trips (w4/052, hardened w9/m93): a repo Dockerfile build (default/`auto`/`dockerfile` builder) reads back `docker`, and a prebuilt image (image set, no repo) reads back `image` (no `envSpecificDetails`). A buildpack build or static site has no bex runtime and reads back empty. |
