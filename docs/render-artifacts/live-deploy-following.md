...
- **Trigger-time clone-token remint (the `w1/031` fix; historical pre-m59 layout):** the deploy's trigger changed the `tea-…-agentmarketcap-1-clone` Secret's token bytes in **both** `default` and the then-current `bex-system` build namespace (sha1 `b08484e85c6d` → `1f418f4ad9bf`; the Secret object itself was updated in place, creationTimestamp still 2026-07-15). The then-current BuildKit remote-context step fetched the private repo for real (`#1 [internal] load git source …` → `From https://github.com/bex-co/agentmarketcap`)—no `could not read Username`. The four preceding `trigger=api` deploys of the same service (2026-07-16 → 17) had all failed exactly there; this one went `live`, the App reached `Running`, and the public URL answers 200. w2/m59 later moved builds to `bex-build` and made clone a credential-isolated init phase; this paragraph remains evidence for token reminting, not the current build shape.
...
**Re-confirmed 2026-07-18 (after the `8dcaf419` roll, w1/m49):** an independent second walk of the same checklist on the next bex-api deploy. Manual deploy `dep-d9dgcm09cdps73dmrhlg` (createdAt `04:51:36Z`, `live` at `04:55:52Z`, 4m06s): the clone Secret's token bytes changed again at trigger time (sha1 `d39db7b1ccd8` → `6ac264fa0b04` — the remint is per-trigger, not one-time), `load git source` completed in 3.0s, and the whole build → Zot push → serve cycle succeeded. `type=build` reads returned `==> Build queued` / `==> Building from …@fd0190c` / `==> Your service is live 🎉` interleaved with Loki-stored BuildKit lines; a 46-row bounded window read twice produced byte-identical ids and order; `type=app` and untyped reads over the same window carried zero `==>` lines; the dashboard deploy page rendered the closed window as a pure history read ending in the terminal narration line attributed to `[dep-d9dgcm09cdps73dmrhlg]` (screenshot `.playwright-mcp/m48-prod-deploy-history.png`).
