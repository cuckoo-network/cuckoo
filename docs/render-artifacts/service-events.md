...
**`disk_*` is no longer a non-goal.** The 2026-07-18 capture listed it as omitted for want of persistent disks; bex has had them since [ADR082](../ADR082-persistent-disks.md) and emits four types from the `apps.AddDisk` / `apps.UpdateDisk` / `apps.DeleteDisk` / `apps.RestoreDiskSnapshot` audit verbs. That line has been removed.

**Wire vocabulary closed (w8/m34, 2026-09-08).** bex now spells the family as Render does: `disk_created` / `disk_updated` / `disk_deleted`, with `disk_restored` kept as a labeled bex extension (Render has no restore-from-snapshot event). `GET /v1/services/{id}/events?type=disk_created` and `?type=disk_deleted` return 200 and push the matching audit-verb filters; the legacy bex spellings `disk_attached` / `disk_detached` are refused 400 by the pinned enum (same as every other bex-named type). Historical audit rows need no rewrite — types are projected from the verb at read time. Live webhook subscription filters carrying the old spellings are rewritten by migration 0106, and Create/Update still accept the aliases and store the canonical names. Outbound webhooks now advertise and deliver the three Render disk types (removed from the m70 anti-goal ledger once ADR082 reopened persistent disks).

**The API was never the defect; the dashboard was.** w6/m122 found the Events tab rendering `events ∩ dashboard catalog`, so five emitted types (`custom_domain_verified` and the four `disk_*`) were invisible and — the filter's option list coming from that same catalog — unselectable. The three API surfaces were correct throughout, and `TestServiceEventSurfaceCarriesDiskAndBexNamedTypes` pins that down by probe: REST, GraphQL and MCP all carry `custom_domain_verified` and the disk family for the same fixture. **No ADR018 row is warranted for the tab fix** — making it fail-open restores Render's behaviour (Render's Events page shows the types its API emits) rather than introducing a divergence.
