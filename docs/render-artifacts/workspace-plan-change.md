...
- **Catalog workspace fees, no licensed Stripe item yet** — Render inserts a "Payment Method" section (card-on-file, required for any paid plan) between plan selection and submit and charges $25 / $499 for Pro / Scale. bex's catalog (ADR030) lists those fees at 30% off ($17.50 / $349.30) on the plan picker; `changeWorkspacePlan` still flips immediately without attaching a licensed Stripe Price (usage meters remain the billed Stripe catalog).
...
| Payment Method step gates any paid plan | Card onboarding exists for paid **resource** intent (ADR046); workspace-plan licensed SKUs are catalogued at 30% off Render but not yet attached to the Stripe Subscription | Display/catalog is set; collection of the monthly workspace fee is a billing follow-up. |
