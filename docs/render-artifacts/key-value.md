...
- **Connections**: the on-page (not dropdown) connection info —
  - **Internal Key Value URL**: `redis://<slug>:6379` — a text field + Copy button, plus a separate **"Enable Internal Authentication"** link. **Notable: Render's internal URL has no username/password by default** — auth is opt-in. Bex's internal URL is **always** password-protected (`redis://:<password>@<name>.<ns>.svc:6379`) — a conscious superset, already recorded in `docs/render-parity.md`'s Key Value row. Don't change bex's posture to match; just confirm the wording of the UI doesn't claim parity it doesn't have.
  - **External Key Value URL** and **Valkey CLI Command**: when no IP is allow-listed at the instance level, both render as an info alert: "External traffic not allowed. Add IP addresses in the Networking section." — i.e. Render gates external connection info on a separate per-instance IP allowlist, not just a public/private toggle. Bex's model is a simpler `public: bool` (no IP allowlist) — `externalConnectionString`/`cliCommand` simply absent when not public. This is the existing documented `ipAllowList` omission; the UI should mirror bex's simpler contract (show/hide by `public`), not attempt to fake an IP allowlist.
...
- **Networking**: instance-level IP allowlist management (bex has no equivalent — omit, already filed as `ipAllowList` gap).
