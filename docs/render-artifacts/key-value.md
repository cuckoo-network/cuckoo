...
- **Key Value Instance**: current Instance Type card (name, RAM, connection limit) + an "Update"/"Upgrade" link to the plan page. Bex supports plan changes post-create via `PATCH /v1/key-value/{id}` / GraphQL `updateKeyValuePlan` / MCP `update_key_value_plan` (shipped `w2/m16`).
