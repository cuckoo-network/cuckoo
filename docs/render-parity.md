...
> **Headline.** bex has real parity on the **service lifecycle, env vars, custom domains, managed Postgres, logs, and metrics** core across all four surfaces, and is _ahead_ of Render on three AI-native verbs (API-key management over the API, deploy-from-chat, an inbound push webhook — [§ bex ahead of Render](#bex-ahead-of-render)). The open frontier is **deploys as first-class objects, config surfaces beyond plain env vars (env groups, secret files), autoscaling config, static sites, and advanced Postgres data-protection** — all mapped to owning milestones/notes in the [§ Gap backlog](#gap-backlog). (Background-worker and cron-job service types shipped in w1/m15; managed Key Value landed on REST/GraphQL/MCP in w2/m7, dashboard pending in w5/m12.)
