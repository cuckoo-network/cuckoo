...
| `PATCH /v1/services/{id}` | update `displayName`, `serviceDetails.plan`, `serviceDetails.maxShutdownDelaySeconds`, `serviceDetails.idleTTLSeconds` (bex extra), `rootDir`, `buildFilter`, `autoDeploy`, `notifyOnFail` (w4/m21 — default\|notify\|ignore, [docs/render-artifacts/notify-on-fail.md](render-artifacts/notify-on-fail.md)), `schedule`/`command` (cron_job only), `healthCheckPath`, and/or `preDeployCommand` — each a pointer/tri-state field, omitted ⇒ unchanged (row backfilled; several fields wired by earlier milestones were missing here) | 200 |
...
Verbs return the updated service object (the patch is accepted; the operator converges asynchronously — poll `GET` for `suspended`/`phase`). The service object carries Render's fields (`id`, `name`, `type` (the serviceType — see below), `suspended`, `dashboardUrl`, `createdAt`, `serviceDetails.url`, and `serviceDetails.maxShutdownDelaySeconds` for web/private/worker services) plus bex extras (`displayName`, `phase`, `replicas`, `revision`) — a superset Render clients safely ignore. bex has no build plans, regions or disks, so those Render fields are omitted.
