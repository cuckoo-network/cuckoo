...
The other direction — bex → you (`internal/webhooks`, [ADR018-render-parity.md § Platform events & integrations](ADR018-render-parity.md)): a workspace registers a destination URL + event-type subscription, and bex POSTs a signed, thin payload (`{type, timestamp, data: {id, serviceId, serviceName, status?}}`) when a subscribed transition happens. **No parallel event store**: the delivery worker tails the same two control-plane tables the events feed derives from (`deploys` + `audit_events`) through a durable watermark, fanning each event out as `webhook_deliveries` rows retried in place — so delivery survives restarts and fires no matter which path performed the transition. Managed-datastore writes with official, sourceable Render counterparts add fixed post-success audit effects: `postgres_created`, `postgres_restarted`, `postgres_credentials_created`, `postgres_credentials_deleted`, `postgres_backup_started`, plus generic `plan_changed` for services, Postgres, and Key Value. The audit row stores both the immutable `dpg-…`/`red-…` target and its non-secret display name, preserving Render's `serviceId`/`serviceName` payload fields across later renames. Render publishes no datastore suspend/resume/delete event types and no Key Value create type; observed availability/health and operation-completion types need a durable operator/status source, so bex omits them instead of inventing names or treating request acceptance as completion. The exact source vocabulary and mapping are pinned in [render-artifacts/datastore-webhook-events.md](render-artifacts/datastore-webhook-events.md).

Signing is Standard Webhooks (`webhook-id`/`webhook-timestamp`/`webhook-signature`, HMAC-SHA256 over `id.timestamp.body` with the `whsec_…` secret minted once at creation); Render's documented semantics apply: 2xx within 15s, 8 retries on an exponential backoff (final ~33h, override with `BEX_WEBHOOK_BACKOFF` for dev/verification), failure-notice email (to the endpoint's creator, over the shared SMTP relay) after 3 consecutive failures, auto-disable until manually re-enabled after the schedule is exhausted. Endpoint management is one Core, three adapters: Render-compatible REST `GET/POST /v1/webhooks`, `GET/PATCH/DELETE /v1/webhooks/{id}`, and `GET /v1/webhooks/{id}/events`, plus the original `/v1/webhooks/endpoints` and `/deliveries` aliases and `GET /v1/webhooks/event-types`; GraphQL `webhookEndpoints`/`webhookEndpoint`/`webhookDeliveries`/`webhookEventTypes` queries + create/update/enable/delete mutations; MCP `list_/create_/update_/delete_webhook_endpoint` (a bex superset — Render's own MCP has no webhook tools). Writes are admin-tier (`can_manage`), reads member-tier (`can_view`); the store-off mode 503s (`ErrWebhooksUnavailable`), never fakes. Proven end-to-end by `scripts/webhooks-verify.sh` and by datastore write → audit row → Render payload tests.
