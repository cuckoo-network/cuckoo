...
**Environment groups** (`lego/backend/internal/envgroups`, Render's `/v1/env-groups`) are a named, reusable set of env vars **and** secret files linkable to many services. A group (id `evg-…`) materializes to two Secrets, `<evg-id>-env` and `<evg-id>-files`; **linking** a group to a service appends those names to the service's `spec.envFromSecrets` / `spec.filesFromSecrets`, and the operator wires them into the container's `envFrom` (before the service's own set, so a service-level var wins on collision) and the shared `/etc/secrets` volume. Legacy item/replace writes retain their immediate-roll compatibility behavior. The staged content verb applies one sparse, opaque-preserving, revision-aware env-var + secret-file patch and then follows `save_only`, `deploy`, or `rebuild`; each linked service receives zero or one selected action, and rollout-only failures are retryable without replaying the secret write. A metadata-only rename preserves the id, contents, links, and running revision.

REST adds `POST /v1/env-groups/{id}/clone`, `PATCH .../contents`, and `PATCH .../environment` to the existing CRUD, replace/single-value, secret-file, and link/unlink routes. GraphQL mirrors them as `cloneEnvGroup`, `patchEnvGroupEnvironment`, and `moveEnvGroup`; MCP mirrors them as `clone_env_group`, `patch_env_group_environment`, and `move_env_group`. Create and single/batch updates accept the same mutually exclusive literal `value` or `generateValue` intent. Clone reads and writes contents only inside Core, copies no service links, and returns names/metadata only. List/detail reads expose keys, file names, Environment scope, timestamps, links, and an opaque revision; plaintext remains available only through a fresh-authorized per-value/file reveal.

Exact trimmed names are unique within one workspace. A non-secret KV-v2 CAS claim makes concurrent create/rename attempts resolve to one winner while identical names remain valid across workspaces. Every name resolver also sweeps legacy metadata and returns `ENV_GROUP_NAME_AMBIGUOUS` instead of choosing an arbitrary duplicate. `BEX_ENV_GROUP_NAME_CLAIM_AUDIT=dry-run|apply` reports or idempotently backfills only unambiguous missing claims at startup; duplicates and conflicting claims are reported for manual repair and are never merged or renamed.

The workspace dashboard renders a searchable Render-shaped table, explicit Environment scope, constrained link candidates, atomic Move/Clone actions, and one masked staged editor with Cancel, key/file rename, dotenv import, generated values, per-value copy, and fail-closed bulk copy/download. Apollo eviction removes a deleted group before collection navigation; intentional deletion is kept distinct from the generic dead-id redirect. The service Environment page remains link/unlink-only and reuses the complete create dialog with that service preselected. Create-time `serviceIds`, group-side link/unlink, owner/timestamp/link-count metadata, replace-all REST, and REST/GraphQL/MCP management remain deliberate Bex extensions. Render's official MCP has no env-group or secret-file tools. Live, redacted evidence: [Environment Groups parity](render-artifacts/env-groups-live-parity.md).
