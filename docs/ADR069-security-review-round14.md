...
**Why default-on is safe here.** The exemption class is exactly the class the audience flag's marker already identifies, and the effect on the official Render CLI is nil — its device-flow tokens carry no audience, so the round-14 rule never armed for them. `BEX_OAUTH_API_SCOPE` was overridable per deployment; empty resolved to `DefaultAPIScope`, and an unset `BEX_OAUTH_RESOURCE` left the _round-14_ `bex.api` rule inert. **w8/m27 retired that inert-when-unarmed contract:** the granular-capability rule arms for every non-platform human OAuth token regardless of `BEX_OAUTH_RESOURCE` and regardless of whether `aud` contains the resource (an audience-less third-party human token is 401). `BEX_OAUTH_API_SCOPE` is ignored as a second matrix.

**What this is not.** At round 14 it was not a per-operation least-privilege scope matrix (read-only vs. write vs. mint-gated scopes). A client that honestly requested `bex.api` and a user who approved it still delegated the user's full authority — ordinary OAuth consent semantics, now truthfully displayed and explicitly requested. **Follow-up landed in w8/m27:** the closed independent capabilities `bex.read` / `bex.write` / `bex.sensitive` now gate every `RelCan…` relation at the shared authorization seam; `bex.api` remains a platform-client compatibility alias only, and recorded audit events carry subject, OAuth client, audience, canonical scopes, relation, and outcome. Per-resource grants and an operation-level OAuth scope matrix beyond those three capabilities stay out of scope.

Tests (`lego/backend/internal/api/r14_scope_test.go`, `dashboard/src/common/server-fn/__tests__/hydra-consent.test.ts`): the third-party identity-scoped audience token is refused and the client record **is** consulted; the four legitimate caller shapes (compliant third party, platform-marked client without the scope, `client_credentials` API key, audience-less device flow) all pass with the Hydra lookup paid only where needed; the parser rejects substring and prefix near-misses; consent refuses on both GET and POST and exempts a platform-marked client. w8/m27 retired the free-form `OAUTH_API_SCOPE` / `BEX_OAUTH_API_SCOPE` override as a second matrix.
...
- **Operation-level OAuth scope matrix** (finding 1's remediation beyond what shipped): **implemented by w2/m78** — a checked-in classification of every REST route, GraphQL top-level field, and MCP tool as `read` / `write` / `sensitive` / `mint`, enforced at each surface's dispatch seam on top of w8/m27's relation-level `bex.read` / `bex.write` / `bex.sensitive` vocabulary ([ADR012](ADR012-auth.md) §7). Mint rides existing `AuthorizeMintClass` (not a requestable scope). Class **refusals** are audited as `api.ScopeClass` with client id, subject, audience, and normalized scopes; allows are not (volume). Finding 1's earlier "Follow-up landed in w8/m27" sentence still describes the capability vocabulary; this item was the remaining operation-level half.
