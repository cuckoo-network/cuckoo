...
In production, `bex-kv-sni-proxy-public` is a fixed `NodePort` (`31892`) with `externalTrafficPolicy: Local`; Terraform owns the shared Hetzner edge LB's `6379 → 31892` listener and TCP health check. This preserves the public client's source IP for allowlist enforcement and admits only workers with a local proxy endpoint. The production `allow-production-edge-proxies` NetworkPolicy opens only the PostgreSQL/Valkey proxy pods' public ports through the `bex-system` default deny; the proxy still enforces SNI routing and each resource's IP allowlist.
