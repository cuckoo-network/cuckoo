...
| --- | --- | --- | --- | --- | --- | --- |
| **etcd** | App/Database/KeyValue CRs (user deployments) | CronJob `kube-system/etcd-backup` → `etcd-snapshots/` in `bex-tfstate` | 03:15 UTC daily | 7 snapshots (rolling) | `scripts/restore-etcd.sh` → local throwaway etcd → sanitized CR manifests | 2026-08-04 (production decrypt + scripted extraction; CI key custody re-proven 2026-08-26 against the shared Tier-A recipient) |
| **OpenBao** | All tenant env-var secrets | CronJob `secrets/openbao-backup` → `openbao-snapshots/` in `bex-tfstate` | 03:45 UTC daily | 7 snapshots (rolling) | `scripts/restore-openbao.sh` → force-restore into a new isolated Raft target | 2026-08-26 (production encrypted-snapshot scripted restore, run `33007385050`) |
| **paid KeyValue** | Tenant Valkey data for every non-Free managed Key Value | Operator-owned `kvbak-<id>` CronJob → `keyvalue/<id>/<RFC3339-UTC>.rdb.gz` in `bex-tfstate` | One stable per-id slot from 03:20–03:39 UTC daily | 7 snapshots (rolling) | `scripts/restore-keyvalue.sh` → fresh PVC, AOF-safe seed/rewrite/restart | 2026-09-01 (production scheduled encrypted-snapshot scripted restore, run `33467191581`) |
...
| **bex-db** | Workspaces, members, audit log, usage, API keys, deploy history | Barman Cloud plugin → ObjectStore `bex-system/bex-db` → `bex-db/` in `bex-tfstate` + continuous WAL | 04:00 UTC daily (full base backup via plugin `ScheduledBackup`); WAL archiving is continuous | 7 days of base backups + WAL (ObjectStore retention) | `scripts/restore-postgres.sh` → new recovery Cluster | 2026-08-25 (production scripted restore, verification drill) |
| **kratos-db** | User identities, credentials, verification/recovery state | Barman Cloud plugin → ObjectStore `auth/auth-dbs`, server `kratos-db-pg18` → `auth-dbs/kratos-db-pg18/` + continuous WAL | 04:15 UTC daily base backup; WAL continuous | 7 days of base backups + WAL | `scripts/restore-postgres.sh` → new recovery Cluster | 2026-08-25 (production scripted aggregate restore, verification drill) |
...
### Paid KeyValue protection restored — 2026-08-26 through 2026-09-01 (w8/m30, production)

The `0644` ciphertext fix, migration-delete backup-preservation annotation, deliverable Alertmanager receiver, and OpenBao workflow decrypt-key fix shipped in `4fa97ce0`. A later successful descendant deployment rolled the production operator; both surviving paid-KeyValue CronJobs reference a rolled descendant containing that encrypt-stage fix.

OpenBao workflow run `33007385050` (2026-08-26) minted a fresh encrypted snapshot, decrypted it with the CI-custodied Tier-A key, force-restored and unsealed an isolated Raft target, verified the marker, removed the target namespace and live marker, and finished with the source StatefulSet 3/3. The 2026-08-25 recipient comparison had already proved etcd, OpenBao, and KeyValue encrypt to the same public recipient, so this also re-proved custody of the private half used by etcd; etcd's last full decrypt-and-extract restore remains the 2026-08-04 run.

By 2026-09-01 both paid KeyValues still in the estate had four consecutive scheduled successes (2026-08-29 through 2026-09-01), exactly seven retained `.rdb.gz.age` objects each, and no firing `BackupCronJobStale`. The third instance present during the incident was normally deleted after successful runs on 2026-08-28 and 2026-08-29; its CronJob and backup prefix were correctly purged, so it no longer belongs to the live paid estate. Provider telemetry also showed four 2026-08-31 `PlatformDatabaseBackupStale` notifications delivered to the operator inbox, proving the shared critical-alert route reaches a human-visible mailbox.

For the restore re-drill, a non-sensitive marker was written before the scheduled 2026-09-01 run. CronJob-owned Job `kvbak-red-da4086iii7bs73drbqh0-29803898` completed at `03:41:57Z` and uploaded `2026-09-01T03:39:20Z.rdb.gz.age`. CI-custodied workflow run `33467191581` selected that object, passed the dry run, decrypted and checksum-validated it, verified the marker with `aof_enabled:0`, enabled and rewrote AOF, restarted and verified it with `aof_enabled:1`, and deleted `restore-kv-m30-20260901`. The source marker was then removed and both production KeyValues remained `Ready`. See the [verification record](drills/2026-08-25-backup-verification.md#follow-up-resolution--2026-08-26-to-2026-09-01).

### Backup verification + KeyValue backup incident — 2026-08-25 (production)

A full-estate verification (hermetic gates, freshness, `DRY_RUN=1` sweep, live `bex-db` + Kratos scripted restores with teardown) passed for the CNPG estate, etcd, and OpenBao backup freshness — and found **paid KeyValue protection broken**: the 2026-08-21/22 namespace cutover's CR deletes purged the shared per-id prefixes (destroying all pre-cutover recovery points), and w7/m85's `/backup-encrypt` wrote `0600` ciphertext the capability-dropped upload stage cannot read, failing every scheduled kvbak Job from 2026-08-23. `BackupCronJobStale` fired correctly but went unactioned for 2 days. Fresh recovery points were minted same-session for all three instances via corrected one-off Jobs; the completed follow-up is [w8/m30](../.pm/w8/done/m30/README.md). See [2026-08-25-backup-verification.md](drills/2026-08-25-backup-verification.md).
...
| **OpenBao** (tenant env-var secrets) | ≤ 24 h | daily 03:45 UTC Raft snapshot, 7 rolling | ≤ 1 h to a restored, unsealed, verified instance | 2m44s workflow job for fresh snapshot, decrypt, scripted restore/unseal, verification, and teardown (2026-08-26) |
| **paid KeyValue** | ≤ 24 h — nightly snapshot only, **no PITR** | per-id RDB slot 03:20–03:39 UTC, keep 7 | ≤ 1 h to a verified fresh-PVC instance | 32 s restore/AOF transition; 2m44s workflow job including setup and teardown (2026-09-01) |
