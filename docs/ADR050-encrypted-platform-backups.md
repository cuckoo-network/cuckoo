...
**How Tier A gets its `age` binary (revised by w1/m66 F16, 2026-08-10).** The first cut installed it at run time (`apk add --no-cache age`) and said so in a comment — "swap for a pinned age image if runtime apk egress is undesirable". The 2026-08-10 security scan made the case concrete: that install resolves code from a mutable package repository **inside the stage holding the plaintext snapshot**, on a Pod that also mounts the S3 upload credentials. The encrypt step now downloads one pinned release artifact and verifies it against a SHA-256 committed in the chart before executing it; a mismatch fails the Job, so a tampered artifact means **no upload**, never an unencrypted one. Every image in both backup CronJobs is likewise `@sha256`-pinned, so a moved tag cannot change what runs beside the snapshot. `scripts/gitops-validate.sh` enforces both invariants. A dedicated prebuilt encryption image (removing the download entirely) and admission-time signature enforcement for platform images remain open — see `.pm/w1/046.md`.

**Revised again by `w7/m85` (2026-08-21) — the managed Key Value path only.** The download is gone there. The KeyValue backup CronJob's encrypt stage now runs `/backup-encrypt`, a first-party entrypoint of the bex image built from `filippo.io/age` v1.3.1 (`lego/operator/cmd/backup-encrypt`), so the stage holding the plaintext RDB installs nothing, downloads nothing, and resolves nothing at run time. The operator reads its own image off its Pod (`internal/selfimage`), which keeps that stage on exactly the digest the operator was rolled out with; with a public key configured and no image resolvable the reconcile fails closed rather than uploading plaintext. The output is a standard age file, so `scripts/restore-keyvalue.sh` is unchanged.

The two STATIC CronJobs (etcd, OpenBao) keep the checksum-verified download: they are Helm-templated manifests with no operator in the loop to resolve a first-party image for them, and their mitigation — a reviewed SHA-256 committed in git, mismatch ⇒ no upload — is unchanged and still enforced by `scripts/gitops-validate.sh`. Moving them onto the same entrypoint is the remaining piece of this thread.
...
- The [age key rotation runbook](runbooks/backup-age-key-rotation.md) covers the single-recipient cutover, two-generation restore identities, verification, and inventory-gated retirement.
