...
- **No network second layer (w1/m7 t001 not applied).** `allowed_ssh_cidrs` is still `0.0.0.0/0` and there is no `bex-app` firewall. Applying t001 adds the network allowlist, but interacts with the chain: CI (GitHub-hosted, dynamic egress) uses `:22` + the apiserver, and `:6443` is reached _via the LB_ (`:443`), so a node-`:6443` lockdown must not sever the LB→node hop. _Follow-up:_ decide the allowlist + CI egress model before applying.
...
- Apply the network second layer (w1/m7 t001) once the `allowed_ssh_cidrs` + CI-egress model is decided.
