...
- **Sandbox ingress:** the NamespaceReconciler now emits one narrow `allow-gateway-driver-ingress` policy for `<ws>-sandbox` (bex-system → driver port `8787` only); everything else stays default-deny.
