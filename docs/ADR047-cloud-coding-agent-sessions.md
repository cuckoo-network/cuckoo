...
**Revised by w5/m65 (2026-08-09) — the branch is the delivery; the PR is opt-in.** Pushing the `bex-agent/*` branch stays unconditional, but the Completer opens a draft PR **only when the session asked for one at create time** (`agentConfig.openPr`, default `false`, accepted identically on REST/GraphQL/MCP and persisted with the config so it survives every later turn). Opening a pull request writes to the tenant's repository and shows up for their whole team; doing that on every fire-and-forget session was noise the user never asked for, and an unwanted PR cannot be un-opened whereas a pushed branch can always have one opened from it. An unreadable persisted config is therefore read as opted **out**. A session that opted out still completes with its `headSha` recorded.

The same milestone retired the **evidence digest as a presentation surface**: the PR body is now session metadata only (session id, branch, head, API link), and neither the dashboard nor the mobile app renders the evidence at all. The digest was a phase-1 artifact from when a completed session had no conversation record; [ADR051](ADR051-agent-session-transcript.md)'s durable transcript now carries the real narrative, making the digest a lossy duplicate — and a misleading one, since the driver's extractor classifies any tool output as "test output" (a `grep` reporting "No matches found" rendered under a Test-output heading). The `evidence` **wire field is retained** on REST/GraphQL for API consumers and is still populated; only its renderers were removed.
