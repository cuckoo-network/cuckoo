...
**Render-parity check (w2/m72).** The operator change preserves the deploy-status wire vocabulary unchanged across REST/GraphQL/MCP — `BuildQueued`→`queued`, `Building`→`build_in_progress`, `Deploying`→`update_in_progress`, `Running`+active→`live`, failures→`*_failed` (`internal/store/reconciler.go` `observedDeployStatus`) — so no surface drifts and the dashboard renders no new status values. Because the operator now pins `status.releaseGeneration` to the in-flight build, the **latest** open deploy row progresses honestly (`build_in_progress`→`live`) instead of being force-closed early, and superseded pushes still resolve to `canceled` — which matches Render (a superseded deploy shows "Canceled"). **One deliberate, deferred divergence:** under a push faster than the build, the run-to-completion build rolls out an intermediate generation whose deploy row bex-api already `canceled` (the DB-only newest-wins in `store.go`'s `cancelOpenDeploys` fires at deploy-create, before that generation went live), so that intermediate briefly serves traffic while its row reads `canceled` rather than Render's live-then-superseded. Making the deploy history record a rolled-out-then-superseded generation as its own terminal state (a distinct `superseded` status, or letting an in-progress row reach `live` before the newer row supersedes it) is a **backend deploy-model refinement tracked as follow-up**, not part of this operator milestone — it needs a new non-terminal→terminal status in the `store.go` vocabulary and is not required for the forward-progress guarantee above.
