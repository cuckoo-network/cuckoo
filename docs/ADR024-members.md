...
`whoami` therefore also reads Kratos's `verifiable_addresses` and reports `Identity.EmailVerified`. Setting **`BEX_REQUIRE_VERIFIED_INVITE_EMAIL=1`** gates login-time redemption on a verified email (`tenantService.RequireVerifiedInviteEmail`), closing the window. It is **off by default** because this deployment's email-verification UX is not yet complete — the verification flow is enabled in Kratos (`kratos.values.yaml`, `verification.via: email`) but the dashboard has no `/auth/verification` route, so the emailed verification link dead-ends and no user can currently reach a verified state. Turning the gate on before that UX ships would block every invited teammate. **Sequencing:** ship the dashboard verification route, confirm users can verify, then set `BEX_REQUIRE_VERIFIED_INVITE_EMAIL=1`. The token path (below) is unaffected — its capability is the unguessable token, not an email claim.
