...
**Target realized** — the quota-gated m19 residue landed on 2026-07-15 and the fleet completed its supported minor-by-minor upgrade to Kubernetes v1.34.9 on 2026-07-27: 3 control-plane + 3 platform + one warm tenant baseline and a 0→2 burst pool. KCP is a healthy 3/3 quorum, OpenBao is 3/3 unsealed on distinct platform nodes, and the four platform CNPG clusters are 2/2 Ready. [`scripts/verify-substrate.sh`](../scripts/verify-substrate.sh) continuously re-checks the target shape, placement, data, scheduler, CSR, network, autoscaler, and no-bootstrap invariants; [`scripts/verify-elastic.sh`](../scripts/verify-elastic.sh) proves tenant scale-up, MostAllocated packing, and scale-down. The production bootstrap, upgrade, and rollback sequence is documented in [`infra/clusterapi/README.md`](../infra/clusterapi/README.md).
