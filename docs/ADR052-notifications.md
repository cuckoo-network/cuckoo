...
flowchart TB
  ...
  pushsvc["browser vendor push service (VAPID)"]
  device@{ shape: tri, label: "member's phone" }
  browser@{ shape: tri, label: "member's browser" }
...
**c) Native and browser push — the bex extension (at-least-once, durable).** No Render counterpart. `PushWorker` tails the same feed through `push_watermark`, evaluates each member's push policy (enabled, event filter, minimum urgency, IANA timezone, working/quiet hours, bounded deferral, per-service overrides), and materializes **one caller-scoped logical inbox row** (`push_notifications`) plus **at most one leased delivery per active destination** (`push_deliveries`). Destinations are Expo installations (`device_push_subscriptions`, CHECK-constrained `provider='expo'` / ios|android) and browser subscriptions (`webpush_subscriptions`, w2/m76). One policy row fans out to both; there is no second browser policy. Expo still uses tickets then receipts; web push completes on HTTP 201/202 (RFC 8291 `aes128gcm` + RFC 8292 VAPID). `DeviceNotRegistered` / HTTP 404/410 prune exactly that destination; 8-attempt retry; 90-day sweep. The payload is a closed four-field envelope (schema version, opaque notification id, closed event type, short status copy + allowlisted relative route) — never secrets, emails, or logs. MCP deliberately has no device-registration, web-push, or push-policy tool. Expo is constructed only from the optional out-of-band `bex-system/bex-push` Secret (`BEX_PUSH_PROVIDER=expo` + `BEX_EXPO_PUSH_ACCESS_TOKEN`, [runbooks/mobile-push.md](runbooks/mobile-push.md)); VAPID from `bex-system/bex-webpush` (`BEX_WEBPUSH_*`, [runbooks/web-push.md](runbooks/web-push.md)). Either Secret absent ⇒ that transport is not constructed (byte-identical when both unset). Native production release stays blocked until the w11/m5 physical-device qualification matrix is satisfied; browser push has no store gate.
...
bex exposes `GET`/`PATCH /v1/notification-settings` (+ `/push`, `/push/availability`, the `GET /v1/notifications` inbox + read-marking, `/v1/notification-device-subscriptions`, and `/v1/notification-webpush-subscriptions`) scoped to the **caller** in the workspace — like `/v1/usage`, and unlike Render's `/notification-settings/owners/{ownerId}`. This follows from D2's per-member tier: there is no owner-wide row to address. GraphQL and MCP mirror the same verbs (one `Service` core, thin adapters per ADR006) except MCP still omits push/device/web-push tools; the dashboard `/notifications` page and the Service-settings row are the only UI writers.

### D5 — Honest disabled states, everywhere

Every dependency is optional and degrades loudly-or-quietly but never wrongly: store nil ⇒ settings/webhook verbs 503 (`ErrEventsUnavailable`/`ErrWebhooksUnavailable`); mailer nil (either `BEX_SMTP_*` unset) ⇒ email paths no-op with a log line; Expo Secret absent ⇒ no Expo transport is constructed and `pushNotificationsAvailable=false`; VAPID Secret absent ⇒ no web-push transport and `webPushAvailable=false` (distinct from Expo so native stays byte-identical). The dashboard renders explicit “not configured” copy rather than a silently dead toggle. `BEX_WEBHOOK_BACKOFF` compresses the retry schedule for verification only (`scripts/webhooks-verify.sh`).
...
- **Web push / VAPID / service worker** — **resolved (w2/m76).** Browser push is a second transport of D3c ([runbooks/web-push.md](runbooks/web-push.md)); ADR048's complementary self-hoster path no longer gates native.
...
5. **Native Expo push is code-complete but not release-qualified** — the gate is operational (EAS project, APNs/FCM credentials, physical-device evidence; w11/m5/t007), not architectural. Browser web push (w2/m76) is the store-free path that can reach users without that gate.
...
9. **Resolved (w3/m82, 2026-09-08) — push covers managed datastores, additive and opt-in.** `projectPushEvent` maps `postgres_unavailable` / `key_value_unhealthy` (Critical, the `server_failed` shape), their `postgres_available` / `key_value_available` recoveries (Important, the `server_available` shape), and the failure half of the t002 lifecycle — `postgres_backup_failed`, `postgres_restore_failed`, `postgres_upgrade_failed` (Important). The successful backup/restore/upgrade facts and `postgres_upgrade_started` stay feed- and webhook-only: a nightly backup that worked is bookkeeping, not a page. Unlike `service_hibernated`/`service_woken`, none of these are ever excluded from push — an unreachable datastore is real downtime. The default policy is byte-identical, so existing policy tests pass unmodified. This is the second walk of m78's five mirrors, plus one new dimension: a datastore is **not** a service, so `push_notifications` gained the resource kinds `database` (`dpg-`, `/databases/<id>`) and `keyValue` (`red-`, `/key-values/<id>`) — the routes `mobile/src/features/notifications/deep-link.ts` already declared — through migration `0109_push_datastore_events`, which widens the `event_type`, resource, and `deep_link` CHECKs together. Destination eligibility is the datastore's **own** `can_view` (`can_view from workspace` on `type postgres`), which every member role holds, so the family is all-roles like the service one; the feed needs no change because the datastore fact arm of `ListWebhookEvents` already rides the shared watermark. Datastore pushes are workspace-wide rather than override-scoped, since `serviceOverrides` keys on `srv-` ids.
