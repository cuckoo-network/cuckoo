...
func (s *Service) RegisterMCP(srv *mcp.Server) {
	mcp.AddTool(srv, &mcp.Tool{
		Name:        "get_usage",
		Description: "Get month-to-date usage (instance-seconds by tier, egress bytes, build seconds, and managed-datastore storage GB-seconds) for the caller's workspace, plus an estimated cost in USD (estimate only — not an invoice; 30% below Render on compute/Postgres/KeyValue/build/Postgres storage, 90% below on bandwidth). When the workspace is billed through Metronome, the response also carries a `billing` object with the real current-period cost and finalized invoices (absent for estimate-only workspaces). Returns the same quantities, cost estimate, and billing as GET /v1/usage.",
