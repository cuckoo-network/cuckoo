...
// StripeWebhook verifies Stripe's signature before accepting events. The
// payment-failure callback is deliberately narrow: m50 establishes trusted
// intake, while ADR040's suspension ladder remains deferred. A nil callback
// records the verified failure for operators without mutating tenant state.
