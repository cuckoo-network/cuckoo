...
// the KeyValue sibling of postgres/metadata_rest_test.go and
// apps/metadata_rest_test.go. Every response body a KeyValue REST handler can
// write (list/get/create/update/suspend/resume) must carry the same nested
// `owner`/`region`/`dashboardUrl`/`updatedAt` shape (resourcemeta.Config,
// docs/ADR006-bex-api.md § Resource metadata contract). Decodes each response
// into a bare map[string]any and checks specific keys/shape (not a snapshot),
// so a handler that regresses to a bare KeyValueView (flat ownerId, no
// owner/region/dashboardUrl keys — the exact w6/016 bug class) fails here.
...
// TestRESTKeyValueMetadataParity_Present exercises create/list/get/update/
...
func TestRESTKeyValueMetadataParity_Present(t *testing.T) {
	svc, _ := newService()
	...
	createReq := httptest.NewRequest(http.MethodPost, "/v1/key-value", strings.NewReader(`{"name":"kv-meta","plan":"free"}`))
	...
	if err := json.Unmarshal(serveREST(svc, "GET", "/v1/key-value", "").Body.Bytes(), &list); err != nil {
		t.Fatalf("list decode: %v", err)
	}
	if len(list) != 1 {
		t.Fatalf("list = %d items, want 1: %#v", len(list), list)
	}
	item, ok := list[0]["keyValue"].(map[string]any)
	if !ok {
		t.Fatalf("list[0] has no nested keyValue object: %#v", list[0])
	}
	check("list", item)

	check("get", decodeMap(t, serveREST(svc, "GET", "/v1/key-value/"+id, "").Body.Bytes()))

	patchRec := serveREST(svc, "PATCH", "/v1/key-value/"+id, `{"name":"kv-meta-renamed"}`)
	if patchRec.Code != http.StatusOK {
		t.Fatalf("patch => %d: %s", patchRec.Code, patchRec.Body.String())
	}
	check("update", decodeMap(t, patchRec.Body.Bytes()))

	suspendRec := serveREST(svc, "POST", "/v1/key-value/"+id+"/suspend", "")
	...
	resumeRec := serveREST(svc, "POST", "/v1/key-value/"+id+"/resume", "")
...
// TestRESTKeyValueMetadataParity_OmittedWhenUnresolved is the omission half:
...
func TestRESTKeyValueMetadataParity_OmittedWhenUnresolved(t *testing.T) {
	...
	createRec := serveREST(svc, "POST", "/v1/key-value", `{"name":"kv-bare","plan":"free"}`)
	if createRec.Code != http.StatusCreated {
		t.Fatalf("create => %d: %s", createRec.Code, createRec.Body.String())
	}
	createBody := decodeMap(t, createRec.Body.Bytes())
	...
	patchRec := serveREST(svc, "PATCH", "/v1/key-value/"+id, `{"name":"kv-bare-renamed"}`)
	if patchRec.Code != http.StatusOK {
		t.Fatalf("patch => %d: %s", patchRec.Code, patchRec.Body.String())
	}
	check("update", decodeMap(t, patchRec.Body.Bytes()))
