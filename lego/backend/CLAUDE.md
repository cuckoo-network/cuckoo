...
- `internal/id/` — the **one place** typed resource ids are minted + validated (`id.New(kind)`, `id.WellFormed`, `id.KindOf`): `<prefix>-<xid>`, hyphen-separated so ids stay valid DNS/k8s names. A leaf (imports only `xid`); the store delegates to it. ADR + the format guard test: [docs/identifiers.md](../../docs/identifiers.md).
