...
type AppSpec struct {
	...
	// HealthCheckPath selects what "healthy" means for this service, in the two
	// modes Render offers:
	//
	//   unset — a TCP socket probe against the container port. The platform has
	//           not been told what healthy looks like, so it asks only whether
	//           the process is listening. This is Render's default.
	//   set   — an HTTP GET of the path; 2xx/3xx is healthy. The opt-in to a
	//           stricter check, for a service that can answer the question.
	//
	// Wired as both the startup and readiness probes (identical handler, see
	// deployment_projection.go). Applies only to HTTP-serving service types
	// (web/private) — workers and cron jobs expose no port, so it is unused
	// there.
	//
	// Deliberately carries NO +kubebuilder:default. It used to default to "/",
	// which forced the strict mode on every service and made an ordinary API —
	// one whose root is a legitimate 404 — impossible to deploy, since
	// Kubernetes scores an HTTP probe healthy only on 200 <= code < 400.
	// Because defaults are applied at write time, Apps stored before that
	// change still carry a persisted "/" and keep the HTTP check until an
	// owner clears the field; see docs/ADR004-app-deployment.md.
	// +optional
	...
	// Count-capped (codex-security round 18): every host fans out into routing,
	// certificate, and host-cache state, so the durable per-App host set must
	// stay bounded; bex-api enforces the same per-service cap (and a
	// per-workspace one) with CUSTOM_DOMAIN_LIMIT before any host reaches here.
	// +optional
	// +kubebuilder:validation:MaxItems=100
	...
	// Count-capped like Hosts (codex-security round 18) — each entry is its own
	// redirect router.
	// +optional
