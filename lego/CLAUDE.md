...
All Go is a workspace under `lego/` (`lego/go.work` over `types/` `operator/` `backend/` `cli/`). Note the Go-version split: `cli/` (and its pinned upstream `render-oss/cli`) requires Go 1.26, so `go.work` declares `go 1.26.0` and local workspace builds use the 1.26 toolchain, while the three platform modules stay on `go 1.25.7` and the shipped image still compiles per-module with `golang:1.25` (the Docker build has no workspace file). The `make` targets live in **`lego/operator/`** (codegen, manager build, image build with context `lego/`, deploy). Build/test bex-api from `lego/backend/` (`cd lego/backend && go build ./... && go test ./...`); the CRD types are in `lego/types/`. Run `make` below **from `lego/operator/`**:
...
- `make lint` / `make lint-fix` — golangci-lint over **all four** modules (operator + backend + types + cli); `make lint-backend` runs the backend module alone (its `.golangci.yml` depguard-enforces the id convention, [docs/ADR020-identifiers.md](../docs/ADR020-identifiers.md)). operator/backend carry their own `.golangci.yml`; types/cli lint with the default linter set. **CI-enforced** on every push/PR touching any `lego/` module (`.github/workflows/go-lint.yml`, which runs `make lint`).
