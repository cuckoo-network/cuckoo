 ...
 * hamburger glyph; usable by button alone (the left-edge swipe is the gesture
 * alternative). Screens host it in their own header/title row. Must be rendered
 * inside an {@link AppDrawerProvider}.
 */
export function AppDrawerButton({ color }: { color?: string }) {
  ...
  return (
    ...
    >
      <Ionicons name="menu" size={26} color={color ?? theme.foreground} />
    </TouchableOpacity>
  );
}
