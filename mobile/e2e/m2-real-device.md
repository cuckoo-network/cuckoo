...
- `scripts/auth-bootstrap-client.sh` has provisioned the public `bex-mobile` client with the exact `co.bex.mobile:/oauth2redirect` URI.
...
6. **Invalid callback:** open malformed `co.bex.mobile:/oauth2redirect/...` and generic `bex://oauth2redirect` links; confirm neither authenticates and no resource screen opens.
