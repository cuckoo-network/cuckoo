...
| pg-sni-proxy | `BEX_PROXY_ADDR`, `BEX_PROXY_METRICS_ADDR` | TCP listen address (default `:5432`) and internal Prometheus address (default `:9092`); the proxy handles PostgreSQL SSLRequest preamble, routes by SNI to CNPG backends using `BEX_DB_DOMAIN`, and meters backend→client bytes (docs/ADR009-postgresql-management.md §SNI proxy) |
| kv-sni-proxy | `BEX_KV_PROXY_ADDR`, `BEX_KV_PROXY_METRICS_ADDR`, `BEX_KV_DOMAIN` | public TLS/SNI front door (defaults `:6379` and `:9093`) and wildcard domain; routes exact public KeyValue intent to Valkey TLS port `6380`, enforces source allowlists, and meters backend→client bytes |
